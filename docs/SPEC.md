...
- ✅ **開催条件別の信頼度**: `reliability`は`standard|reference`を返す。7月の小倉芝は、データのある
  2022・2024・2025・2026年すべてで確定値同士の好走率差が負だったため`reference`とする。
  函館は年ごとに方向が変わり、一律抑制しない。`reference`は表示上の注意であり、有利度スコア・PAI・
  展開恩恵馬・統合順位の計算値は変更しない。原因は馬場・距離構成・開催時期のどれか未確定。
...
11. 🔎 `StyleAdvantageWeights`（勾配4.0/pt・増幅1.2・逃げ競合減点6.0/頭）の期間別安定性（3.4節）。
    広域標本の方向性確認と夏開催の複数年比較は完了。7月小倉芝は`reference`表示としたが、原因を
    馬場状態・距離帯・開催時期へ分解する追加検証と、福島の想定RPCI誤差の調査は未完了。
