...
最終更新: 2026-07-25 / 対象コミット `ed5d0ee` 以降（少人数ロケテスト認証を含む）
...
- 🧪 2026-07-25以降の事前予想は検証データを蓄積中。母数0件の段階では精度を保証せず、
  画面にも検証中であることを表示する。
...
- 監視対象は採用翌日の2026-07-25以降に確定したダートレースとする。
- 全体100件かつ実績ハイ20件へ到達するまでは`accumulating`とし、品質判定を行わない。
...
2. 🧪 **ダートRPCI v4の期間外監視条件は確定、初回判定は未実施**
   （`docs/DECISIONS.md` 2026-07-24 参照）。100件かつハイ20件から4条件を判定する実装は完了した。
   2026-07-25以降の確定レースは現時点で0件のため、初回の期間外レビューはデータ蓄積待ちである。
