...
## 3.6 統合順位予想（能力 ability-v1 × 展開 = integrated-v1）

ユーザー要望（2026-07-12「展開＋絶対能力の統合順位予想」）の Phase1。展開適性(PAI)だけでなく
馬の地力も加味した順位・分類を出す。**現データのみで実現**（人気・オッズ・賞金は未永続化のため未使用）。

- ✅ **能力指数 ability-v1**（`domain/pace/ability.py`）: 各近走のスコア =
  「出走頭数で正規化した着順評価 × クラス係数」を新しさ加重平均（直近5走）。0〜100の内部score
  ＋ `reasons` ＋ `model_version`。UIには実数を出さず統合層で言葉・記号へ翻訳。
- 🧪 **仮係数**（`AbilityWeights`）: 新しさ減衰(≤180日1.0/≤365日0.7/超0.4)、クラス係数
  (G1 1.5〜未勝利0.8)、score正規化基準1.5。実データ検証後に確定（独断で確定しない）。
- ⚠️ **クラス信号の限界（既知）**: `grade` コードは未永続化（`parse_ra` が None・§9-8関連）のため、
  クラス係数は `race_class`（レース名／条件名の文字列）のキーワードから best-effort で判定する。
  条件名（未勝利/1勝/2勝/3勝/オープン/G1-3）が読めれば効くが、ステークス名だけの重賞等は
  判別できず中立(1.0)にフォールバックする。**現段階の能力指数は事実上「クラス補正付きの近走充実度」**。
...
- ✅ **UI**: `/forecast` の `integrated_ranking` を「統合順位予想」セクションが記号・言葉で表示
  （順位・◎○▲△・能力上位/中位・展開が向く/向きにくい）。実数値は非表示。
- 🔭 **Phase2候補**: 人気・獲得賞金・馬体重を ingestion で永続化して能力指数を強化（`tasks/backlog.md` B節）。
...
16. 🧪 **能力指数 ability-v1 の仮係数**（`AbilityWeights`、§3.6）。新しさ減衰・クラス係数・score正規化
    基準は暫定。特に**クラス係数は `race_class` 文字列の best-effort 判定**で、重賞のステークス名
    のみのケースは中立化される（`grade` 未永続化のため。§9-8関連）。実データでの妥当性検証と、
    Phase2での市場・賞金指標の永続化検討が必要（`tasks/backlog.md` B節）。
