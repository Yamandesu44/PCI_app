...
最終更新: 2026-08-03 / 少人数ロケテスト認証と事前予想63件の初回レビューを含む
...
- ✅ 個別ユーザー認証・課金はMVP非対応。少人数ロケテスト用の共有認証だけを提供する。
- ✅ Web共有認証は`BETA_ACCESS_USER`・`BETA_ACCESS_PASSWORD`の両方を設定した場合だけ有効。
  片方だけなら503、両方未設定ならローカル開発用に無認証とする。
- ✅ `PUBLIC_API_TOKEN`設定時は`/api/v1/*`へBearer認証を要求し、Webの
  `API_ACCESS_TOKEN`から同じ値を送る。`INGEST_TOKEN`とは分離する。
...
- ✅ 展開予想の冒頭は、想定展開、統合順位の上位3頭、注意馬、展開信頼度を表示する。
  「中心候補」は展開適性単独ではなく、近走能力と展開適性を統合した順位を正とする。
- ✅ 統合順位は上位5頭を初期表示し、6位以下は必要時だけ展開する。
- 🧪 2026-07-25以降の事前予想は検証データを蓄積中。2026-08-03時点の照合は63件、
  評価対象に対するカバー率は3.8%であり、精度を保証せず画面にも検証中であることを表示する。
...
## 3.4 脚質別有利度（style-advantage-v4）

- ✅ **算出元**: 想定RPCIの「中立点からの乖離」を脚質別スコア（50=互角、0〜100）へ写す。
  中立点は `classify_pace`（rule-v4）と同じ閾値の中点（芝50.0／ダート43.0）から導出し、
  判定基準の二重定義を作らない。スロー寄り→逃げ・先行が加点／ハイ寄り→逃げ・先行が減点。
  **v4では差し・追込を採点せず常に50（互角）とする**（ADR-0010・下記🚨と実測根拠を参照）。
- ✅ **逃げ競合の減点**: 逃げ候補が2頭以上の場合、1頭増えるごとに逃げのみ減点（先行争いの消耗）。
- 🧪 **係数**（`StyleAdvantageWeights`）: 勾配4.0/pt・逃げ増幅1.2・先行1.0・競合減点6.0/頭・
  クランプ[5,95]は引き続き暫定（実データ検証後に調整する）。
  **差し0.0／追込0.0（常に互角）はADR-0010の実DB検証で確定済み**。
...
- 🚨 **2026-07-26発見: ダートの中立点43.0は`rpci_actual`の算出式が切り替わる前の分布で
  較正されており、現行データと整合しない**（`scripts/diagnose_rpci.py --by-track-year`）。
  - `aggregate_rpci`はレースラップ(`race_s3f`/`race_l3f`)があればTARGET準拠式を、
    無ければ全完走馬PCI平均のフォールバック（コード上「暫定」）を使う。この2経路は
    **ダートで平均5.4pt乖離する**（2025年1〜7月の同一期間内比較でラップ由来40.9／代替46.3）。
    芝は約1.6ptしか離れないため影響が小さい。
  - ラップ保有率は2022〜2024年が0.0%、2025年1〜7月が2.8%（ダート）、**2026年は100%**。
    2025-07-24以降のバックフィル境界と一致する。したがって年をまたぐRPCI比較は
    **式の違いを競馬側の変化と取り違える**。
  - 結果として同じ1〜7月で揃えたダート実績平均は2022〜2025年が45.9〜46.2で安定し、
    2026年だけ42.0へ動いた。中立点43.0に対する「スロー側」割合は82%前後→57.7%へ変化し、
    有利度ラベルの選別力が失われた（2026年5〜7月ダートの好走率差 -0.6%、
    前年同期は+18.8%）。芝は同じ窓で50.6〜51.4と動かず、実際に劣化していない。
  - **本文書の上記「2026年7月だけ-11.9pt・開催条件に局在」という2026-07-23時点の解釈は、
...
- 🚨 **2026-07-26発見: 有利度は「前付け馬」にしか効いていない**（式統一後の全期間検証、
  ...
  - 対ベース好走率（不利→有利の5帯）:
    - 芝・前付け 0.84x→0.89x→1.02x→1.07x→**1.18x**（単調・大きく分離）
    - ダート・前付け 0.94x→0.88x→0.96x→1.03x→**1.14x**（概ね単調）
    - 芝・差し追込 1.03x→0.99x→0.98x→1.01x→**0.98x**（**完全に平坦**）
    - ダート・差し追込 0.94x→0.99x→1.03x→1.08x→**1.02x**（**有利帯で逆行**）
  - 全体の好走率差+2.9%（芝・ダートとも）は**前付け馬だけが稼いでいる**。
    差し追込は出走頭数の70〜74%を占めるがシグナルがなく、ダートの相関-0.000、
    芝+0.037という低さの主因はここにある。
  - 解釈: 「スロー→前が楽に運べる」は実データで成立するが、
    「ハイ→差しに向く」は成立しない。ペースが速いことは前が苦しくなる理由にはなっても、
    特定の差し馬が届く理由にはならない（決め手は展開より個体差に依存する）。
  - 併せて判明: 前付けと差し追込では好走率のベースライン自体が約2倍違う
    （芝 33.9% vs 18.2%、ダート 33.9% vs 15.9%）。有利度スコアは全馬50中心のため
    この差を全く表現しておらず、スコア70の差し馬とスコア30の逃げ馬の実力差が逆転している。
  - **暫定対応（2026-07-26 実施・UI層のみ）**: ユーザー判断により、まず表示だけ実態へ合わせた。
    ドメインのスコア算出・係数は一切変更していない。
    - 差し・追込は「有利／不利」と断定せず「展開の影響は小さい」と表示し、
      スコア数値と進捗バーを出さない（文言より視覚が勝って「有利」と読まれるため）。
      理由文をリスト下に一度だけ添える。`styleAdvantageScores()`の`isDirectional`で判定。
    - 脚質別プロファイルでは差し・追込を淡色＋「（参考）」表示にする。
    - ハイペース時の文言から「差し・追い込みが届きやすい」等の裏付けのない主張を外し、
      裏付けのある「前に行く馬に厳しい」側の記述へ寄せた（スロー時の記述は実データが
      支持するため変更なし）。
- 🔎 **2026-07-26: 一次データ（実績ペース×確定脚質の素の好走率）を測定**
  （`--pace-style-matrix`、芝101,478頭・ダート107,828頭）。ルールの答え合わせではなく、
  ルールを作り直すための土台。スロー−ハイの差（自脚質平均に対する比）は以下。
  ...
  - **自在は先行とほぼ同等に反応するのに採点されていない**。しかも出走の34〜37%を
    占める最大グループで、現状は展開評価から丸ごと外れている。
  - 差し・追込の反応は前付けの1/4以下。現行の乗数-1.0/-1.2は過大。
  - **ADR-0010（Accepted・2026-07-26）**。独立2期間×芝ダートの4条件で候補比較した結果、
    **`back-neutral`（差し・追込を常に互角）を採用**し`style-advantage-v4`とした。
    好走率差は4条件すべてで改善（芝+8.1%/+4.0%、ダート+3.4%/+0.6%）。
  - **自在の採点は検証で否定され、取り下げ**。直近ダートで悪化（-0.8%/-0.6%）し、
    自在自身の帯別単調性も4条件中1条件でしか成立しなかった。
    「ペースに反応する」ことと「有利不利の分離に貢献する」ことは別だった。
    `entries`は4件のままで、API公開スキーマへの影響はない。
  - 差し・追込は係数をどう弱めても帯別好走率が単調にならず、
    **後方脚質には順序づけ可能なシグナルが存在しない**と確認された。
  - 未解決: ダートは現行でも前付けの帯別好走率が単調にならない。
    係数ではなくダート固有の別要因であり、本件とは切り離して追う。
...
- 🔎 **検証残**: 本番相当DBで期間を分けて比較を実行し、改善の再現性を確認する。
  2026-08-03の実COM 50件検証で合成予約位置は人気1件だけ偶然一致、本賞金0件だった。
  予約拡張はmykeibadb合成時だけ解析し、jvlinkでは欠損へ縮退する。実位置は未確定。
...
- ✅ `GET /api/v1/forecast-performance`はJST基準の直近30日・90日・180日（`days`、既定90日）、
  ...
  「変動注意（50%未満）」ごとの一致率・的中数・母数を返す。集計対象は保存根拠に
  `classification_margin`を持つ新方式の予想だけとし、旧固定値や異なる算出方式を混在させない。
  `confidence_cohort_groups`は同じ新方式について全体・芝・ダートの件数と一致率を返し、
  Webは芝・ダート各100件の再評価条件に対する蓄積状況を表示する。
  `confidence_review_target`はコース別の必要件数、`confidence_review_ready`は芝・ダート双方の
  条件到達を示す。Webは未達時に各コースの残り件数、到達時に「再評価可能」を表示する。
  モバイルの閉じた予想検証サマリーにも、行高を増やさず新方式の芝・ダート進捗または
  「再評価可能」を表示し、詳細を開かなくても次の検証時期を判断できるようにする。
  同サマリーの読み上げ名は、芝・ダートを略さず、進捗、展開一致、検証件数、カバー率、
  詳細を開く操作であることを一続きで伝える。視覚上の短縮表記をそのまま読み上げ名にしない。
  キーボードフォーカス時は、レイアウトを変えない内側リングでサマリー全体の位置を可視化する。
  LightGBM予想の`confidence`は、展開3分類の境界から予測値がどれだけ離れているかを
  0.40〜0.90へ変換した表示用の「読みやすさ」であり、的中確率の校正値ではない。
  2026-08-03以前に保存された予想の固定値0.75は、当時の予想記録を保つため遡及更新しない。
...
- ✅ **表示**: Web トップに `IngestStatusBanner`。正常時は控えめな表示、鮮度低下・失敗時のみ
  目立つ配色にする。警告の要点は常時表示し、失敗一覧、対象レース、重複レース、再同期・馬場補完
  コマンドは「詳細と復旧手順」1つの閉じた開閉領域へ集約する。正常時は開閉領域を表示しない。
  失敗一覧は最大5件、エラーの要約は200文字までとする。
...
- 次回採用評価の監視対象は、完全未使用期間として予約した2026-08-03以降に確定した
  ダートレースとする。
- 全体100件かつ実績ハイ・平均・スロー各20件へ到達するまでは`accumulating`とし、
  品質判定を行わない。
- 判定開始後は次の4条件をすべて満たす場合に`healthy`とする。
  - MAE `≤5.94`（採用時4.750へ25%の悪化余地）
  - 展開ラベル一致率 `≥60%`（既存の受入基準）
  - ハイ再現率 `≥60%`（重要区分の最低受入基準）
  - 絶対バイアス `≤4.62`（採用時2.619へ約2.0の悪化余地）
- 1条件でも外れた場合は`retraining_review`とする。ただし本番モデルの自動置換・自動降格は行わず、
  同一期間で現行モデルと再学習候補を比較する。
- 実行コマンドは次のとおり。
...
- `lgbm-dirt-v4-lap-history`をmartへ保存するため、migration 006で`predicted_pace`と`pace_fit`の
  `model_version`を20文字から64文字へ拡張する。API起動前に`alembic upgrade head`が必要である。
...
2. 🧪 **ダートRPCI v4の期間外監視条件は確定、初回判定は未実施**
   （`docs/DECISIONS.md` 2026-08-02 参照）。100件かつハイ・平均・スロー各20件から
   4条件を判定する実装は完了した。
   2026-08-03以降の確定レースは現時点で0件のため、次回の期間外レビューはデータ蓄積待ちである。
...
16. 🧪 **能力指数 ability-v3 の仮係数**（`AbilityWeights`、§3.6）。新しさ減衰・クラス係数・score基準に
    ...
    `--compare-ability-weights`へ`recent10`候補を追加した（実装のみ）。
    **実DB比較の結論（不採用）**: ユーザーのWindows実行機で独立2期間を比較
    （直近200レース: 1位勝率+1.5%/1位好走率-1.0%/TOP3捕捉率-0.3%、
    〜2025-12-31以前200レース: +1.5%/+1.5%/-0.5%）。TOP3捕捉率が両期間で悪化し、
    1位好走率も符号が不安定なため、RuleWeights/PaiWeightsと同じ採用基準
    （両期間で全指標が悪化しないこと）を満たさず不採用。`recent_races=5`を本番維持する。
    `recent10`候補はCLIに残し、データ蓄積後の再検証に利用できるようにする。
    なお同じ2回の実行で成分ブレンド候補`form-only`/`form-heavy`が両期間で
    1位勝率・1位好走率・TOP3捕捉率の全3指標を改善しており、2026-07-22の同候補比較
    （2025-07-01〜12-31・212レース／2026-01-01〜07-21・97レースでは「両期間で
    全指標改善する候補なし」と結論、成分ブレンドは現行維持で決定済み）と食い違ったため、
    2026-07-22と同じ日付区切りで`--limit 300`により再検証した。母数は当時より増加
    （2025-07-01〜12-31: 212→300件、2026-01-01〜07-21: 97→300件）。
    - `form-only`: 2025年後半 +0.7%/+2.7%/+0.9%、2026年前半 +2.3%/+0.7%/+1.3%。
      **両期間・全3指標が改善**し、RuleWeights/PaiWeights/AbilityWeights共通の
      採用基準を満たす。
    - `form-heavy`: 2025年後半のみ1位勝率が-0.3%と僅かに悪化し基準未達。
    - `market-aware`: 2026年前半が-1.3%/-4.0%/+0.0%と明確に悪化し不採用（既存結論を維持）。
    - `recent10`: 両期間ともTOP3捕捉率が悪化し、上記の不採用判断を再確認した。
    データ増加後は`form-only`が基準を満たす結果となったが、母数拡大に伴う結論の反転は
    今回が初めてであり安定性が未確認のため、**2026-07-26時点ではユーザー判断により
    採用を見送り、本番の成分ブレンド（form0.55/本賞金0.30/人気0.15）は維持**する。
    追加データでの再現性を確認したうえで改めて採用を判断する（`tasks/backlog.md`参照）。
