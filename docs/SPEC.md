...
- 🧪 **2026-07-20修正（未検証）**: `mykeibadb_client._build_se_record()` は、mykeibadbのテーブルが
  `DATA_KUBUN`列を持つ場合その値をそのまま確定コード（'4'/'7'）として使っていたが、この列の値が
  JV-Data本来の確定コードを正しく反映しない環境では、着順・タイム・上り3Fが実際は揃っている
  レコードでも「出走前」のまま扱われ、`parse_se_result`が全件`None`を返し続ける
  （ユーザー報告: 2026-07-12以降の確定成績が1週間以上反映されない一方、特別登録は正常に反映）。
  着順・タイム・上り3Fが全て揃っていれば`DATA_KUBUN`列の値に関わらず確定扱いするよう修正。
  **静的なコードリーディングのみに基づく仮説的な修正で、実DBでの検証はできていない**。
  ユーザーが再同期して直るか確認が必要（`docs/DECISIONS.md` 2026-07-20）。
...
15. 🔎 **`mykeibadb_client._build_se_record()`のDATA_KUBUN修正（2026-07-20）は未検証**。
    2026-07-12以降1週間以上、確定成績が一切反映されない（特別登録・出走表は正常）という
    ユーザー報告を受け、コードリーディングのみで「DATA_KUBUN列の値を無条件に信用しており、
    その値が確定コードを正しく反映しない環境では着順・タイム・上り3Fが揃っていても
    出走前扱いのまま」という仮説を立てて修正（§6参照）。実DBでの検証はできておらず、
    ユーザーが再同期して直るかどうかの確認待ち。直らない場合は別原因（mykeibadb.exe/wmykeibadb
    側の同期失敗など、このリポジトリのコード外の要因）を疑う必要がある。
