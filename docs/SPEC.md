...
15. 🩺 **確定成績が2026-07-12以降1週間以上反映されない件は原因究明中（診断待ち）**。
    経過: (a) `_build_se_record()`のDATA_KUBUN扱いを修正 → 再同期しても改善せず**空振り**。
    (b) 同期ログが exit code のみで原因層を特定できなかったため、`batch.py`に件数ログを常設し、
    切り分け診断ツール`ingestion.diagnose_results`を新設（§6参照）。
    **次アクション**: ユーザーが `python -m ingestion.diagnose_results --date 20260712 --date-to 20260719`
    を実行して出力を共有する。判定は3択:
    - (A) SE行が0件 → mykeibadb未取得（mykeibadb.exe/JV-Link設定側。このリポジトリのコード外）。
    - (B) SE行はあるが着順/タイム/上り3F列が拾えない → `mykeibadb_client`の列候補不足（実列名を追加）。
    - (C) 3項目は揃うのにparse失敗 → 合成レコードのバイト配置/値変換の不整合。
    出力が出るまでは(A)/(B)/(C)いずれとも断定しない（独断で確定しない原則）。
