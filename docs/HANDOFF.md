...
### 次にやること（要ユーザー判断・ADR起票が必要）

ドメイン側の是正はルールの前提に関わるため、独断で進めない。選択肢:

1. **脚質グループで非対称にする**: 前付けには現行の加点則を維持し、差し追込側は
   中立寄りへ圧縮する（実データに合わせる最も素直な案）。
2. **ベースライン差を別項目として持つ**: 有利度は展開のみを表し、脚質そのものの
   好走率差は統合順位・PAI側で別途扱う。

いずれの場合も、採用前に`--validate-style-advantage`の脚質グループ別帯集計で
前付け・差し追込の両方が単調になるかを確認すること。

なお、自在（FLEXIBLE）は`_SCOREABLE_STYLES`から除外されており有利度の対象外だが、
出走頭数の33〜38%を占める。前が苦しくなった分の恩恵が自在馬へ向かっている可能性があり、
ドメイン是正を検討する際は自在の扱いも併せて見直す価値がある（未調査）。
