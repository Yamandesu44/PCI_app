...
- [ ] **ラベル構成比の確認。** 55/45 は同じ規則を当てただけで**構成比は未測定**。
      `--diagnose-pai` に構成比表を足したので、「合致」が極端に少ない/多い、
      合致と不利の好走率が逆転していないかを見て調整する。
- [ ] **オフセットの期間外確認。** −1.17 / +0.47 は 2026-06-01以降の500レースから
      取った当てはめ値。別期間で解き直して安定しているか見る。
      `--compare-pai-weights` に `uncentered` を常設したので寄与は継続監視できる。
- [ ] **`sortByPai` が ADR と矛盾している。** `apps/web/src/lib/pace.ts` が馬を PAI 降順で
      並べており、「脚質をまたいだ順位付けにPAIを使わない」に反する。表示順の設計変更に
      なるため今回は触っていない。**要対応。**
...
- **認証・CORS**: `app.py:37` が `allow_origins=["*"]`。ユーザー認証なし。`INGEST_TOKEN` の運用も未確定。
- **デプロイ基盤**: Dockerfile も vercel.json も無い。CIはあるがCDが無い。
