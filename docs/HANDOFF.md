...
- [ ] **「判断材料が少ない」を別情報として出す。** UIは「中立」を展開の判定として
      見せているが、実際にはかなりの割合が「この馬は分からない」を意味している。
      `HorseFitOutput` にフラグを足して UI で区別する案を推奨。**API契約の変更を
      伴うため要判断。** ラベル閾値をいじっても解決しない（原因が閾値ではない）。
