...
## 2026-08-08 (Claude Code) 作業区切り その8 — 合致閾値を脚質別へ（pai-v5）

- 更新日時: 2026-08-08 JST
- 作業担当: Claude Code
- 引き継ぎ先: OpenAI Codex
- ブランチ: `claude/sweet-einstein-ilnaov`
- 最新コミット: `c47e69c feat(api): give each course and running style its own fit threshold (pai-v5)`
- 作業目的: 「向く」が多すぎる問題を**測ってから**直す。設計判断は
  `docs/DECISIONS.md` の 2026-08-08 の項。

### 先に読むこと: CI が15コミット連続で赤かった

`1d42a35`（2026-08-06）以降、**統合テストが全30件エラーのまま**だった。
`alembic/env.py` が `alembic.ini` の `sqlalchemy.url` を見ないようにしたところ、
`tests/integration/conftest.py` が testcontainers の接続先をそこへ渡していたため、
全部が既定の localhost:5432 へ向かっていた。`7fad61d` で修正済み。

**気付くのが遅れた構造のほうが重要。** 常用コマンド
（`python -m pytest tests/unit/ tests/contract/`）はずっと緑で、統合テストは
Docker が要るため手元で回らない。**CI を見に行かない限り分からない。**
CLAUDE.md は「世代を上げたら統合テストも確認」と書いているが、
実際には**世代と関係なく壊れうる**。push のたびに CI を確認すること。

### 測った結果（500レース・6,575頭）

```
コース   レース数   中央値   90%点   半数以上   合致頭数
芝        296     30.0%   70.0%   31.1%     4.0
ダート     204     53.8%   86.7%   54.4%     7.0
```

芝は使える絞り込みになっている。**ダートは過半数のレースで半数以上が「向く」。**
一方で **芝の自在は602頭中0頭**が合致で、構造的に到達できていなかった。

### 原因は閾値の値ではなく、閾値が1つしかないこと

PAI は pai-v3 以降**脚質内の相対量**で、大小を脚質間で比較できない（ADR-2026-08-04）。
`matched_threshold` はそれを全脚質・両コース共通の絶対値 55.0 と比べていた。
**「脚質をまたいで比べない」と言いながら、暗黙に脚質を順位付けていた。**

閾値を（コース×脚質）別の10個へ分けた。各セルの合致割合を30%へ揃える最小の閾値を
実測から解いている。**目標30%は芝の現状**——問題が出ていない側を基準に置き、
新しい恣意的な数字を持ち込まない。

### 実装中に見つけて直したもの（いずれも黙って壊れる型）

1. **「注目」が10セル中3セルで到達不能になっていた。** 閾値を上げたのに上積みが
   固定 +10点 のままだったため。ダート先行は合致72.0に対し PAI 上限78.25で、
   余地が6.25点しかない。**到達不能な線は例外を出さず、ただ何も出さなくなる。**
   「合致からPAI上限までの余地の半分」へ変え、`max_pai_for` を追加した。
   到達可能性は全10セルのテストで固定してある。
2. **web の `PAI_MATCHED = 55`** を削除した。閾値が脚質別になった以上、
   **単一の値では近似にすらならない**（芝の自在49.5 対 ダートの先行72.0）。
   サーバの `top_fit_strength` だけを使い、判定できないときは強調しない。
3. **一覧のキャッシュ経路が脚質を持っていなかった。** 強さを判定できないので
   `RaceBoardForecastRecord` に `top_running_style` を足した。

### 残る穴: ダートの差しは閾値では解けない

**ダートの差しだけ 55.0 に据え置いた（48.1%のまま）。** 推奨値は 75.5 と出るが、
適用すると 0.0% になる。理由は式にある:

```
感応度0の脚質（差し・追込）は base_pai が中立の50で固定される
  → PAI = 0.5×50 + 0.5×affinity
affinity は「その馬自身の最良レベル」を100へ正規化する
  → PAI の上限は 75.0 ちょうど
ダートは予測RPCI平均46.55で、境界47.0を下回るためほぼ全レースが同じペース区分
  → 多くの馬の最良レベルと一致し、一律 75.0 に並ぶ（48.1%・その上は0頭）
```

**affinity が「どれだけ向くか」ではなく「一番得意か否か」を答えているのが、
ダートの合致過多の正体。** 診断側は `分割不能` と表示して推奨値を出さないようにした
（答えに見えるだけの数字を返さないため）。

### 適用後の実測（確認済み・同じ500レース）

```
        中央値        90%点        半数以上      全頭合致     合致頭数
芝      30.0→25.0%   70.0→60.0%   31.1→21.6%   0.3→4.0%    4.0→3.0
ダート   53.8→33.3%   86.7→66.7%   54.4→30.4%   3.9→2.0%    7.0→4.0
```

**元の問題は解決した。** ダートの「半数以上が向く」レースが 54.4% → 30.4%。
脚質を固定した行でも、芝先行・ダート逃げ・ダート自在・ダート追込が
`逆転` → `順当` へ改善している。

**残った副作用: 芝の全頭合致が 0.3% → 4.0%（296中12レース）。** 芝の自在だけ
閾値が緩んだ（55.0 → 50.5）ため、自在の多い少頭数レースが丸ごと合致になる。
**中立点への引き上げ（49.5 → 50.5）では戻らなかった**——その区間に馬が1頭もおらず、
対象集団が変わらないため（合致26.6%で不変）。表示層の `CROWDED_SHARE = 0.5` が
拾う範囲であり、芝の「半数以上」自体は減っているので、追加の特例は入れていない。

**もう一つの弱点: 芝の自在は合致(25.0%)が中立(26.2%)を上回らない。** 脚質内の
効きも +2.0%（±8.7%）で全10セル中もっとも弱く、**PAI が芝の自在を判別できていない。**
閾値では直らない（セル全体に信号が無い）。多くが `low_evidence` に該当するため
UI では「材料薄」が併記される。pai-v6 で扱う。

### 次の担当がやること

- [ ] **mart の再生成**（precompute）。既存 `pace_fit` は pai-v4 のままで、
      新しいラベルは流すまで画面に出ない。
- [ ] **再生成の前に `prune_mart_versions` を流さない。** 現行世代が pai-v5 へ
      変わったため pai-v4 が削除対象に入る（`--active-days` で保護はされている）。
- [ ] `tests/integration/`（Docker が要る。世代を上げて取り残しCIを落とした前例あり）。
- [x] `--diagnose-pai` で適用後を確認済み（上記）。
- [ ] **pai-v6: affinity の正規化を母集団基準へ。** 上限と塊が消え、ダートの差しも
      閾値で切れるようになる。芝の自在に信号が無い件も、ここで見直す価値がある。
- [ ] 閾値は2026-06-01以降の500レースからの当てはめ値。**期間外で再確認すること。**

### 診断の実行について

1レースあたり数百クエリを投げるので、**DB がリモートだと往復が支配的**になる。
Supabase 経由で約45分、ローカルの開発DBなら約4分。進捗は25レースごとに stderr へ出る。

```powershell
$env:DATABASE_URL = $null   # Supabase 向きが残っていると45分かかる
.venv\Scripts\python -m scripts.backtest_forecast --diagnose-pai --limit 500
```

診断は**読み取り専用**。`.env` が本番を指していてもデータは変わらない。

### テスト状況

```
apps/api: 848 passed (tests/unit/ tests/contract/)
apps/web: 187 passed (24 files) / tsc --noEmit 通過
Ruff 0 / mypy --strict 0（66 files）/ lint-imports 2 contracts kept
OpenAPI 変更なし（再生成して差分ゼロを確認）
```
