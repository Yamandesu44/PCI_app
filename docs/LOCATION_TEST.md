...
- 日付ストリップの各日付（56×56px）
- 「他の日程を探す」、月カレンダーの年月移動・開催日（44px以上）
- 同一開催ナビの前後ボタン（44×44px、`MobileRaceNavigation`）
- 競馬場タブ・詳細タブの各ボタン（行の高さ44〜48px）
- ヘッダーロゴ、取り込み対象レース、検証期間切替、検証詳細の折りたたみ、同一開催の各R（44px以上）
...
- 開催日・競馬場タブ・レース行・詳細タブ・隊列内の馬・同一開催レース移動で、
  緑色2pxの`focus-visible`リングが視認できるか（PCブラウザ390pxで確認済み）
- ヘッダーのアイコンリンクが「予想検証」「レース一覧」と読み上げられ、
  44×44pxの操作領域とフォーカスリングを保つか（PCブラウザ390pxで確認済み）
- OSの「動きを減らす」を有効にした状態で、ページ内移動・開閉アイコン・ローディング表示が
  不要にアニメーションしないか（CSSと本番ビルドでは対応済み、実機確認待ち）
- 取り込み警告の見出し、一覧の件数・説明、非選択の競馬場タブが、屋外や低輝度でも
  背景に埋もれず読めるか（PCブラウザ390pxの自動コントラスト監査では不足0件）
...
PCブラウザ390px幅の事前監査では、ホーム・出走前詳細・確定後詳細の重複ID、壊れたARIA参照、
名前のない表示中操作要素は0件で、ホームの見出し階層も`h1 → h2 → h3`へ整理済み。
以下では端末固有の読み上げ文言とスワイプ順を重点的に確認する。
