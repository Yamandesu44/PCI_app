...
- 各タブ（`role="tab"`）はTab+Enter/Spaceで切替できる。矢印キー（←→）での切替はWAI-ARIAの
  Tabsパターンの一部だが未実装。これは既知の簡略化であり不具合ではない
...
- 日付ストリップ: 選択中の日付が「選択中」と読み上げられるか（`aria-current="date"`実装済み）
- 同一開催ナビ: 現在のレースが読み上げられるか（`aria-current="page"`実装済み）
- 競馬場タブ・詳細タブ: 選択中のタブが選択済みと読み上げられるか（`aria-selected`実装済み）
- 既知の制約: 各タブグループはアクティブな1パネルだけをDOMへ描画する実装のため、
  非アクティブなタブボタンの`aria-controls`は現在DOM上に存在しないパネルIDを参照する。
  スクリーンリーダーや自動監査（axe等）が警告を出す可能性があるが、実際の読み上げが
  破綻していなければ実害は無い。実機で読み上げに支障が出た場合のみ修正を検討する
