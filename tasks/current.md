# tasks/current.md — 進行中タスク

## 2026-08-08 完了: 「絞りにくい」が多すぎる問題（pai-v6・目標30%→20%）

実機で「展開では絞りにくいレースです」が頻出すると指摘され、測り直した。
**約4分の1のレースで出ていた**（芝21.6% / ダート30.4%）。詳細は `docs/DECISIONS.md`。

- [x] pai-v5 の目標30%が緩かった。「芝の現状」というだけで検証ではなかった。
- [x] 目標を20%へ。**芝 21.6% → 10.8%**、**ダート 30.4% → 22.1%**。
      重み付きで 25.2% → 15.4%（4回に1回 → 6.5回に1回）。
- [x] `MODEL_VERSION` を pai-v6 へ。全10セルで合致・注目とも到達可能を確認。
- [x] 診断のバグを修正。スウィープが `splittable` を見ずに解を適用し、ダートの差し
      757頭を上限外 75.5 で切って**丸ごと合致から消していた**（30.4% → 6.9% と誤表示）。
      目標30%の行が実測と一致するか突き合わせて発見。

### 分かったこと: ダートは閾値では直らない

- [ ] **目標10%まで絞ってもダートは17.2%までしか下がらない。** 差し（757頭・28%）が
      分割不能で48.1%のまま残り、**この1セルが床を決めている**。
- [ ] **芝は20%が底。** 15%以下では差し・逃げが PAI 上限 75.0 に当たって分割不能になり、
      据え置きの分かえって悪化する（10.8% → 12.2%）。
- [ ] **裾が広いのは構造。** `pace_deviation` はレース単位の定数で、同じ脚質の馬は
      揃って閾値を跨ぐ。馬ごとの絶対閾値ではレース単位の頭数を制御できない。
- [ ] **pai-v7 が本丸**: affinity の正規化を母集団基準へ。上限75.0と塊が消える。

### 反映に必要な操作（未実施）

- [ ] Cloud Run へ再デプロイ（pai-v6）。
- [ ] mart の再生成（`run_batch.ps1 -Step forecasts`）。
- [ ] 再生成後に `--diagnose-pai` で 芝10.8% / ダート22.1% を確認する。

## 2026-08-08 修正: CI が15コミット連続で赤かった（統合テスト）

**気付くのが遅れたこと自体が問題。** 常用コマンド（`tests/unit/ tests/contract/`）は
ずっと緑で、Docker が要る統合テストだけが落ちていた。手元で回せないため、
**CI を見に行かない限り分からない**構成になっている。

- [x] 原因は `1d42a35`。`alembic/env.py` が `alembic.ini` の `sqlalchemy.url` を
      一切見ないようにしたが、`tests/integration/conftest.py` は
      `Config.set_main_option` で testcontainers の接続先をそこへ渡していた。
      **全30件が既定の localhost:5432 へ向かい、接続できず全部エラー。**
- [x] 危険なのは「設定ファイルに残った古い値が黙って勝つ」ことであって、呼び出し元が
      その場で指定することではない。`alembic.ini` は空のまま（テストで固定済み）なので、
      **非空の値はプログラムからの明示だけ**という切り分けにした。
- [x] 判定を `resolve_migration_url` として `session.py` へ出した。
      **`env.py` はテストから import できない**（読み込むだけでマイグレーションが走る）ため、
      中に書いてある限りソースの grep でしか確認できない。
      どのDBへ書くかを決める分岐が2回とも振る舞いテスト無しで通っていた。
- [ ] **世代を上げたときだけでなく、push のたびに CI を確認すること。**
      手元の常用コマンドに統合テストが含まれていないので、緑でも何も保証しない。

## 2026-08-08 完了: Claude Code — 合致閾値を（コース×脚質）別にした（pai-v5）

「向く」が多すぎる問題を実測してから決めた。詳細は `docs/DECISIONS.md` の
2026-08-08 の項。

### 測った結果

| コース | レース数 | 中央値 | 90%点 | 半数以上 | 合致頭数 |
|---|---|---|---|---|---|
| 芝 | 296 | 30.0% | 70.0% | 31.1% | 4.0 |
| ダート | 204 | **53.8%** | 86.7% | **54.4%** | **7.0** |

- [x] **原因は閾値の値ではなく、閾値が1つしかないこと。** PAI は pai-v3 以降
      脚質内の相対量なのに、全脚質共通の絶対値（55.0）と比べていた。
      実測でダートの差しは48.1%が合致、**芝の自在は602頭中0頭**。
- [x] 閾値を（コース×脚質）別の10個へ分けた。各セルの合致割合を30%へ揃える
      最小の閾値を実測から解いた。**目標30%は芝の現状**（問題が出ていない側）。
- [x] 芝の自在が 0% → 26.6% になり、到達不能が解消した。
- [x] `PaiWeights.__post_init__` で合致と不利の重なりを構成時に落とすようにした。
- [x] `MODEL_VERSION` を pai-v5 へ上げた。OpenAPI は変わっていない。

### 実装中に見つけて直したもの

- [x] **「注目」が10セル中3セルで到達不能になっていた。** 固定 +10点 のままでは
      ダート先行（合致72.0・上限78.25）に余地が6.25点しかない。
      「合致からPAI上限までの余地の半分」へ変え、`max_pai_for` を追加して
      全10セルで到達可能であることをテストで固定した。
- [x] web の `PAI_MATCHED = 55` を削除した。**単一の値では近似にすらならない**
      （芝の自在49.5 対 ダートの先行72.0）。サーバの `top_fit_strength` だけを使い、
      判定できないときは強調しない。
- [x] 一覧のキャッシュ経路が脚質を持っておらず強さを判定できなかったので、
      `RaceBoardForecastRecord` に `top_running_style` を足した。

### 適用後の実測（確認済み）

| | 中央値 | 90%点 | 半数以上 | 全頭合致 | 合致頭数 |
|---|---|---|---|---|---|
| 芝 v4 → v5 | 30.0% → 25.0% | 70.0% → 60.0% | 31.1% → **21.6%** | 0.3% → **4.0%** | 4.0 → 3.0 |
| ダート v4 → v5 | 53.8% → **33.3%** | 86.7% → 66.7% | 54.4% → **30.4%** | 3.9% → 2.0% | 7.0 → 4.0 |

- [x] **元の問題は解決した。** ダートの「半数以上が向く」レースが 54.4% → 30.4%。
- [x] 脚質を固定した行で、芝先行・ダート逃げ・ダート自在・ダート追込が
      `逆転` → `順当` へ改善した。
- [x] 芝の自在を 49.5 → 50.5（中立点の上）へ。合致は 26.6% のまま変わらない
      （49.5と50.5の間に馬がいないため）。`分割不能` → `採用可` になった。

### 残っている穴と次の一手

- [ ] **ダートの差しだけ 55.0 に据え置いた（48.1%のまま）。** 閾値では分割できない。
      感応度0の脚質は `PAI = 0.5×50 + 0.5×affinity` で、affinity は自分の最良レベルを
      100へ正規化するため **PAI の上限が 75.0**。ダートはほぼ全レースが同じペース区分に
      入るので多くの馬がそこへ並ぶ（75.0に48.1%・その上は0頭）。
- [ ] **pai-v6: affinity の正規化を母集団基準へ変える。** これが根治。上限と塊が消え、
      10セルすべて閾値で切れるようになる。**affinity が「どれだけ向くか」ではなく
      「一番得意か否か」を答えているのが、ダートの合致過多の正体。**
- [ ] 閾値は2026-06-01以降の500レースからの当てはめ値。**期間外で再確認すること。**
- [ ] **芝の自在は合致(25.0%)が中立(26.2%)を上回らない。** 脚質内の効きも
      +2.0%（±8.7%）で全10セル中もっとも弱く、**PAI が芝の自在を判別できていない。**
      閾値では直らない（セル全体に信号が無い）。pai-v6 で扱う。
- [ ] **芝の全頭合致が 0.3% → 4.0%（296中12レース）へ増えた。** 自在の閾値が
      緩んだため、自在の多い少頭数レースが丸ごと合致になる。
      **中立点への引き上げでは戻らなかった**（49.5と50.5の間に馬がいない）。
      表示層の `CROWDED_SHARE` が拾う範囲で、芝の「半数以上」自体は 31.1% → 21.6%
      へ減っているため、**追加の特例は入れない**判断。
- [x] `CROWDED_SHARE = 0.5`（表示層）はこのまま。実測で発動対象は
      芝 21.6% / ダート 30.4% へ下がった（v4 は 31.1% / 54.4%）。

### 運用（未実施・DBが要る）

- [ ] **mart の再生成。** 既存の `pace_fit` は pai-v4 のまま。新しいラベルは
      precompute を流すまで画面に出ない。
- [ ] **再生成の前に `prune_mart_versions` を流さないこと。** 現行世代が pai-v5 へ
      変わったため、pai-v4 が削除対象に入る（`--active-days` で保護されてはいる）。
- [ ] `tests/integration/` の確認（Docker が要る。世代を上げて取り残しCIを落とした前例あり）。
- [ ] 再デプロイ後に `--diagnose-pai` を回し、合致割合が目標へ寄ったか確認する。

## 2026-08-07 進行中: Claude Code — 公開後の画面と出力を直した

公開まで通ったので、**訪問者から見た画面**を見直した。個人ツールだった頃の
作りがそのまま残っていた部分を直している。

### 表示の方針: 自己否定を看板にしない（2026-08-07）

「近走内容による能力の並び（参考）」を見出しにし、前置きで
**「この並びは単勝人気の順より当たりません」**と断っていた。実測に忠実ではあったが、
利用者が最初に目にする情報がそれになっていた。

- [x] 見出しを **「この展開が向きそうな馬」** へ変更。展開が向く馬を主役に置いた。
- [x] 能力の並びは畳んだ先へ移し、そこで「勝ち馬を当てるための順位ではありません」と述べる。
      **限界は消していない。** 置き場所を変えただけで、予想検証ページの実測もそのまま。
- [x] 向く馬は**馬番順**に並べる。能力順だと先頭が推奨に見えるが、それは実測で
      否定された使い方そのもの（ADR-2026-08-04）。

**製品の価値は展開の読みと、それが向く馬を挙げられること。** そちらを前に出す。

### 公開画面の見直し（2026-08-07・P1/P2/P3）

個人ツールだった頃の画面が、公開後もそのまま残っていた。

- [x] **運用者向けの情報を訪問者から分離**（`SHOW_OPERATOR_DETAILS`）。内部パス・
      例外文言・再同期の PowerShell コマンドが公開画面に出ていた。
      **隠すのは手順であって状態ではない**——遅延自体は伝え続ける。
- [x] 同一画面で「出走前」が 0 と 36 に分かれていた（ピルは今週末を除く件数）。
      リンク先の節名に合わせて「その他」へ改名。
- [x] `候補: リングスター / 不利` の矛盾を解消。**向くときだけ**出す。
- [x] 一致率に基準を併記（3区分なので当てずっぽうで約33%）。
- [x] 「合致」を画面から外し、語彙を「向く／向きにくい／影響は小さい」へ統一。
- [x] `PAI 62` の生の指数値を表から削除。非専門家向けという前提に反し、
      **脚質をまたいだ比較（実測で否定された使い方）を誘う**。
- [x] タグ（注目・妙味・波乱注意）の凡例を追加。「妙味」は馬券用語。
- [x] 正常時の緑バナーを訪問者に出さない。問題がないことを一等地で報告しない。
- [x] 一覧の切り詰めをレース名だけに限定（「芝20…」で距離が欠けていた）。
- [x] **免責フッターを追加**。的中を保証しない・推奨ではない・JRA/JRA-VAN とは
      無関係であることを常設で示す。公開サービスとして最低限必要。

### 「向く」と「評価を下げたい」の矛盾を修正（2026-08-07）

実機で発見: PAI 57 の馬が「この展開が向きそうな馬」と「評価を下げたい馬」の
両方に出ていた。

- [x] 原因は割引側の条件 `fit_label === "不利" || pai < 60` が「合致」の下限
      （PAI 55）と重なっていたこと。pai-v4 で振れ幅を25→10へ縮めた際、
      `60` という値だけ古いスケールのまま取り残されていた。
- [x] `sortDiscountCandidates` を `fit_label === "不利"` だけに限定。
      合致と不利はドメイン上排他なので、構造的に重複しなくなる。
- [x] 並びも `sortByPaceBenefit` と対称にした（脚質有利度が第一キー、PAIは
      同一脚質内の同順位決めのみ）。**PAIを脚質をまたいで比べない**方針を維持。
- [x] `forecastDecisionChecklist` の独自フィルタも同じ条件へ統一。
- [x] 恩恵側・割引側それぞれのカードから生の `PAI 62` 表示を削除。
- [x] 同じ矛盾が再発しないよう、両関数を突き合わせる回帰テストを追加。

### 「向く」が多すぎる問題（2026-08-07・手法A + 実測の土台）

実機で16頭中15頭が「向く」になり、絞り込みに使えない状態を確認。

**原因は構造。** 合致の判定は馬ごとの絶対閾値（PAI >= 55）で、**レース内で何頭が
該当するかを制御していない**。想定ペースが強く傾くと、脚質ボーナスだけで前に行く
馬が丸ごと閾値を超える（振れ幅10 × 逃げ1.0 = 60、先行0.65 = 56.5）。
**展開の恩恵は相対的な価値で、全員に向く流れは誰の武器でもない。**

- [x] **手法A（表示層）を実装。** 合致が出走馬の半数以上なら、見出しを
      「展開では絞りにくいレースです」へ切り替え、頭数を明示して一覧を畳む。
      絞れないと分かること自体が手がかりになる。
- [x] **分布を測る診断を追加**（`summarize_fit_crowding`）。`--diagnose-pai` に
      「レースあたりの合致割合」（中央値・90%点・半数以上のレース比率・全頭合致の比率）
      を追加。既存の `summarize_fit_label_shares` は全頭混ぜた構成比で、
      **1レース内で何頭が合致するかが見えなかった**。
- [ ] **実測してから手法Bを決める（測定待ち・実行は所有者のWindows機）。**
      閾値 `CROWDED_SHARE = 0.5` は暫定値。分布を見て、合致の段階化
      （強い合致/合致）をサーバ側へ入れるか、閾値55の再較正で足りるかを判断する。
      **UI側に閾値を写経しない**（80/70・60の教訓）。
- [ ] レース内相対化（上位N頭だけ合致）は**採らない**。PAIを脚質をまたいで
      比べることになり ADR-2026-08-04 と衝突する。

診断は**読み取りのみ**（`scripts/backtest_forecast.py` と `application/backtest.py` に
`commit()` / `session.add` / INSERT / UPDATE / DELETE は無い）。`.env` が Supabase を
指したままでも本番データを壊さない。確定済みレースのみが対象。

再開コマンド:

```bash
cd apps/api && python -m scripts.backtest_forecast --diagnose-pai --limit 500
```

見るのは出力末尾の「レースあたりの合致割合」。**半数以上のレース比率**と
**全頭合致の比率**が手法Bの要否を決める。

### 触ってはいけないこと

- [ ] **マイグレーションをデプロイへ自動で組み込まない。** 複数インスタンスが同時に
      起動すると競合する。スキーマ変更時は事前に手で `alembic upgrade head` を流す。
- [ ] **`prune_mart_versions.py` の「新しいN世代を残す」だけに戻さない。**
      `predicted_pace` は芝とダートで別世代が同時に現役のため、書き込み頻度の
      低い側を現役のまま消す。現行世代の固定と `--active-days` の二重で守っている。

### リリースに向けて残っているもの

- [ ] **【最優先】規約確認が済むまで公開を閉じる。** 手順は
      `docs/design/public-data-surface.md` の「閉じ方」。**入口は2つ**で、
      web（`BETA_ACCESS_*`）だけ塞いでも Cloud Run の API から同じ JSON が取れる。
      **`PUBLIC_API_TOKEN` の入れ替えは、いまや衛生ではなく閉じるための必須手順。**
- [ ] ユーザー単位の認証（現状はサーバ間トークンのみ）。
- [ ] JRA-VAN 規約確認（コードでは解決できないため早めに）。
      取得経路は `JRA-VAN → mykeibadb → 本アプリ` の2段で、確認すべき規約が2つある
      可能性がある。焦点は区分B（馬名・着順・上がり3F等の公表事実の再掲）。
- [ ] GitHub へ `PUBLIC_API_BASE_URL` / `PUBLIC_API_TOKEN` を設定する
      （未設定の間、毎日の鮮度監視はスキップされる）。
- [ ] 土曜11:00 の実行で**日曜の出馬表**が入ることを確認する（新スケジュールの検証）。

取り込みの自動化そのものは完了している（`PaceLab_Sync_Mykeibadb`・火木金土日の8回）。

## 2026-08-06 完了: Claude Code — 公開の足回りを固めた（Cloud Run + Supabase）

ドメインのコードは触っていない。「動くが公開できない」状態を解消する作業。

- [x] デプロイ先を **Cloud Run**（東京）に決め、CD を用意した（`7d7f724`・手動実行のみ）。
- [x] 起動時にモデルを読むようにした。ゼロスケールのため、放置すると
      **コールドスタート直後の利用者が毎回読み込み待ちを被る**。
- [x] `MODELS_DIR` を設定項目へ出した。ソース相対のままだと、パッケージとして
      入れた瞬間に**例外も出さずルールベースへ落ちる**。CI に検出を追加した。
- [x] 接続プールをサーバーレス向けに絞った（`77c113d`・5本/プロセス）。
      `pool_pre_ping` と `pool_recycle=1800` も併せて入れた。
- [x] 容量を実測した（`420df71` / `77a2d3f`）。全体123MB・約4.7年分・コアは年21%増。
- [x] DB を **Supabase**（東京・無料枠 DB 500MB）に決めた。Aiven は無料プランで
      リージョンを選べず（東京なし）、東京には有料プランが要るため見送った。
- [x] pg8000 が `sslmode` を受け取れない問題を吸収した（`e08775b`）。
      マネージドDBの接続文字列をそのまま貼れる。
- [x] 移行の突き合わせと世代掃除のスクリプトを追加した（`1f25369`）。
- [x] API 816テスト、Ruff、strict mypy、import-linter を完了した。

### 追記: 設定漏れが原因を名乗るようにした

デプロイの設定漏れは「落ちる」形では出ない。**動いたまま静かに壊れる**ため、
どちらも「原因が分かる形にする」ことで対処した。

- [x] Web の案内文を原因別に分けた（`3469962`）。`API_BASE_URL` 未設定は
      開発用の 127.0.0.1 へ落ち、`API_ACCESS_TOKEN` 不一致は全リクエスト401。
      どちらも旧コードは「APIが起動しているか確認」と案内していた。**APIは動いている。**
- [x] デプロイ直後の確認スクリプトを追加した（`scripts/check_deployment.py`）。
      外から `PUBLIC_API_TOKEN`・CORS・モデルのフォールバックを検出する。

### 次にやること（コンソール作業・このセッションでは実行できない）

- [x] **Supabase（東京）へ移行を完了した。** 9テーブル・65万行超が完全一致。
      接続はプーラー経由・ポート5432（直結は IPv6 のみで Cloud Run から届かない。
      6543 は transaction モードで pg8000 の prepared statement と衝突する）。
- [x] 移行の過程で `alembic` の2つの不具合を直した（`4e9d9ae` / `1d42a35`）。
      SSL翻訳を通らない／接続先が `.env` を読まない。**どちらも黙って別のDBを見る。**
- [x] Supabase 上の実容量を確認した。**111MB / 500MB（22%）。** 入れ直しで不要領域が
      整理され、ローカルの123MBより小さくなった。コアの増加は年26MB程度。
- [x] ローカルAPIを Supabase 向きで起動し、画面まで確認した。モデル読み込み・
      `/ready`・レースボード・予想検証すべて正常。**DB移行は完了。**
- [x] `db_size.py` の不具合を直した（`fcb00c0`）。`pg_stat_user_tables` は public 以外も
      含むため、Supabase の `auth.users` をスキーマ名なしで数えて落ちていた。
- [x] **Cloud Run へ公開した**（`pci-api` / asia-northeast1）。Cloud Shell から手動で。
      環境変数は `RATE_LIMIT_TRUSTED_PROXIES=1` を含めて設定済み。
- [x] `check_deployment` の6項目すべてが意図どおり。モデルは `lgbm-dirt-v6-pci-v3`。
- [x] **web を Vercel へ公開した**（`https://pciapp.vercel.app`）。3段が繋がった。
- [x] `vercel.json` の廃止された Secrets 参照を削除した（`52d315e`）。
      **これがデプロイ0件の原因。**ビルド開始前に弾かれるため作り直しても直らない。
- [ ] **公開範囲を絞るか判断する。** JRA-VAN 規約確認が未了のまま公開状態にある。
      `BETA_ACCESS_USER` / `BETA_ACCESS_PASSWORD` で閉じられる。
- [ ] CD ワークフロー（`deploy-cloudrun.yml`）を使うなら Workload Identity 連携を用意する。
      手動デプロイで足りているうちは後回しでよい。

### 取り込みの自動化（`docs/design/ingestion-automation.md`）

- [x] 設計を起こした。実行・記録・**検知**の3層。要点は「動かす」ことより
      **「止まったと気付く」**こと。止まっても画面は壊れず古くなるだけで、
      公開後に最も気付きにくい。
- [x] 検知層を実装した。GitHub Actions が毎日 `/api/v1/ingest-status` を確認し、
      異常ならワークフローを失敗させる（GitHub が所有者へ通知する）。
      ローカルの実APIに対し、401・正常・鮮度不足の3経路を確認済み。
- [x] タスクを登録した（`PaceLab_Sync_Mykeibadb`・火金土日の6回）。手動実行で成功。
      **成功は「どこへ書いたか」を保証しない**ので、公開API側で鮮度を確認すること。
- [ ] ワーカーの `.env` を Cloud Run 向きにする（`API_BASE_URL` / `INGEST_TOKEN`）。
- [ ] リポジトリへ `PUBLIC_API_BASE_URL`（Variables）と `PUBLIC_API_TOKEN`（Secrets）を設定する。
      未設定の間、監視はスキップされる（毎日通知が飛ぶのを避けるため）。

### 表示名の統一

- [x] 利用者に見える名前を **PACE LAB** に統一した。タブ名・API のタイトルを変更。
      指標名（PCI）は名乗りに出さない。理解していなくても使えることが value のため。
- [ ] Vercel のドメインを変更する（プロジェクト名を変えると `.vercel.app` も変わる）。

### 公開データの棚卸し（`docs/design/public-data-surface.md`）

- [x] 公開APIが返す項目を「独自指標／公表事実／元データの識別子」に区分した。
      規約確認は法務判断だが、**判断材料としての実際の公開範囲は技術側でしか出せない**。
- [x] `ketto_num`（血統登録番号）を公開レスポンスから外した。JRA-VAN のマスターキーで、
      **元データへの突き合わせを容易にする**。web は未使用だった。取り込み側では引き続き使う。
- [ ] **公表事実（馬名・着順・上がり3F・馬場状態）の扱いを規約で確認する。**
      内容は JRA が公表しているが、取得経路は JRA-VAN の配信データ。ここが焦点。

## 2026-08-04 完了: Claude Code — PAIを中心合わせ＋実効サイズへ（pai-v3 → pai-v4）

pai-v3 の感応度を確定しようとして、もっと手前に2つの問題が見つかったので直した。

- [x] 振れの中心ずれを実測した（芝 −6.2点 / ダート +4.3点 の定数シフト）。
- [x] `pace-off` の芝/ダート符号逆転が、この定数シフトで全て説明できることを確認した。
- [x] 平均 deviation を0にするオフセットを二分法で解き、診断へ「推奨offset」として出した。
- [x] 中心を合わせると swing 25/10/5/0 が全体相関 +0.073〜+0.074 で並ぶことを確認した。
- [x] 中心合わせ（芝 −1.17 / ダート +0.47）と振れ幅 25 → 10 を本番既定へ採用した。
- [x] ラベル閾値を 65/40 → 55/45 へ引き直した。
- [x] `MODEL_VERSION` を pai-v4 へ上げた。
- [x] pai-v3 で入れた表示バグを直した（最も不利な馬に「持ち味を出しやすい流れ」と出ていた）。
- [x] web からPAIの実数閾値（80/70/45/60）を除き、`fit_label` 判定へ変えた。
- [x] API 751テスト、web 153テスト、Ruff、strict mypy、import-linter を完了した。

### 次にやること

- [x] ラベル構成比を脚質別に測った。**合致 > 中立 は合致の存在する全8セルで成立。**
- [x] 「不利 > 中立」の逆転の真因を特定した（脚質構成ではなく `pace_affinity` の
      フォールバック。過去データが無い馬が中立へ集中する）。
- [x] `自在` が合致へ構造的に到達できない欠陥を修正した（フォールバックに山が無かった）。
- [x] **「判断材料が少ない」を別情報として出した**（`b4798fd`）。 `PaiResult.low_evidence` を
      DTO・APIスキーマ・OpenAPI・TS型・UI まで通した。「材料薄」を併記し、材料が
      薄い馬は「軸候補」「評価下げ」と断定しない。ラベル閾値は変えていない
      （原因が閾値ではないため）。mart へは持たせていない（再計算可能なため）。
- [ ] 自在のフォールバック（山の位置と高さ）を実データで検証する。暫定値のまま。
- [ ] オフセットの期間変動を監視する。芝は期間外で +0.42 ずれたが、振れ幅10なら
      定数シフト3.8点に収まる（pai-v3の25なら9.5点だった）。
- [x] **脚質をまたいだ PAI 順位付けを全廃した**（`fd60398`）。

### CI（未着手・仕様判断が要る）

- [x] **CI の mypy 失敗を解消した**（`95e4067`）。型検査ジョブを Python 3.11 へ。
- [x] **`ruff format` をツリー全体へ適用した**（`78cceaa`・整形のみ・独立コミット）。

### 触ってはいけないこと

- [ ] **UI で馬を PAI 順に並べない。** ダートの追込は PAI 50（ペース非依存）だが好走率は 0.47x。
- [ ] **PAIの閾値をUI側へ写経しない。** ドメインの `fit_label` を使う。pai-v4 のスケール変更で
      web の `pai >= 80` が黙って機能を止めるところだった。
- [ ] **世代（model_version）を上げたら `tests/integration/` も確認する。** 常用コマンドに
      含まれておらず Docker が要るため、pai-v3 で実際に取り残されCIが落ちた。

### 運用待ち・未確認

- [ ] 2026-08-04以降が100レース・ラベル別20レースに達したら `--monitor-dirt` で品質判定する。

### リリースに向けた未着手項目

- [x] **CORS を許可オリジン制へ変えた**（`a811e40`）。既定はローカル開発の2つのみ。
      `PUBLIC_API_TOKEN` 未設定時に警告を出す。
- [x] **レート制限を追加した**（`a7e252d`）。`/api/v1/*` のみ・既定120回/分・クライアント単位。
      プロセス内カウントなので、複数インスタンスでは実効上限が台数倍になる。
- [ ] ユーザー単位の認証は未着手（現状はサーバ間トークンのみ）。
- [x] **API の本番用 Dockerfile を追加した**（`aee0370`）。PaaS 非依存（PORT で受ける）。
      非 root・lightgbm 用に libgomp1 のみ・モデル同梱。CI がビルドと `/health` 応答を検証する。
      `vercel.json` は既存。docker-compose に `api` サービスを profile 付きで追加した。
- [x] **デプロイ先を Cloud Run に決定し、CD を用意した**（`7d7f724`・手動実行のみ）。
- [ ] **PostgreSQL の置き場所が未決定。** Cloud SQL に無料枠が無いため、東京リージョンの
      マネージド Postgres を併用する想定。**これが最後の未決定事項。**
- [ ] GCP 側の準備（プロジェクト・Artifact Registry・Workload Identity・環境変数）。
      手順は `deploy-cloudrun.yml` 冒頭に列挙済み。
- [ ] 取り込みの自動化（Windows機での手動 PowerShell 実行のまま）。
- [ ] JRA-VAN 規約確認（コードでは解決できないため早めに）。

### 次の候補

- [ ] ダートの「平均」帯を広げるか検討する（帯3.4ポイントに対しMAE 2.404で比1.4倍）。
- [ ] 芝モデルの過学習を抑える（検証誤差が [50] 2.64959 → [100] 2.65502）。

## 2026-08-04 進行中: Claude Code — ペース補正が判別に寄与しているかの検証

- [x] `swing-light` を棄却した（芝の上位帯リフト 2.34x は頭数 94 → 18 の縮小によるもの）。
- [x] `--compare-pai-weights` に上位帯の頭数を併記した。
- [x] 脚質内比較の判定を標準誤差ベース（差が2SE以上か）へ変えた。
- [x] `pace-off`（感応度を全て0）を候補へ常設した。
- [x] `summarize_pace_centering` を追加した（ペース補正の中心ずれを測る）。
- [x] pai-v2 の式のままだった `adaptability.py` の docstring を差し替えた。
- [x] 統合テストの `pai-v2` 直書きを修正した（CIが落ちていた）。

### 次にやること: 中心ずれの確認と、必要なら再センタリング

`pace-off` が全体相関で current を上回った（+0.073 対 +0.069）。ただし芝 +0.016 /
ダート −0.016 と符号が逆。脚質内では感応度の高い脚質ほど判別できていない
（逃げ 芝+4.1% ダート+2.6% でいずれも誤差内）。

- [ ] `--diagnose-pai` の「ペース補正の中心ずれ」を実測する。概算では
      芝 +4.8点・ダート +6.2点 の底上げが起きているはず。
- [ ] ずれが確認できたら、deviation の中心を閾値中点（`neutral_rpci`）ではなく
      **予測RPCIの実測平均**へ置き直す。これで補正が脚質内だけに効くようになる。
- [ ] 再センタリング後に `pace-off` と比べ直す。そこで初めて「ペース補正は要るか」
      を公平に判定できる。今の比較は補正が定数シフトを含むため公平でない。
- [ ] 判定が付いたら感応度を確定し、`docs/DECISIONS.md` の ADR-2026-08-04 へ追記する。

### CI（未着手・仕様判断が要る）

- [x] **CI の mypy 失敗を解消した**（`95e4067`）。型検査ジョブを Python 3.11 で走らせる。
      3.12 だと numpy 2.5 系（3.12以上専用）が入り、その型スタブの PEP695 構文が
      `python_version = "3.11"` の解析で落ちる。`requires-python` の下限で検証するのが筋。
- [x] **`ruff format` をツリー全体へ適用した**（`78cceaa`・整形のみ・独立コミット）。

### 触ってはいけないこと

- [ ] **UI で馬を PAI 順に並べない。** ダートの追込は PAI 50（ペース非依存）だが好走率は 0.47x。
- [ ] **世代（model_version）を上げたら `tests/integration/` も確認する。** 常用コマンドに
      含まれておらず Docker が要るため、pai-v3 で実際に取り残されCIが落ちた。

### 運用待ち・未確認

- [ ] 2026-08-04以降が100レース・ラベル別20レースに達したら `--monitor-dirt` で品質判定する。

### リリースに向けた未着手項目

- [ ] 認証・CORS制限・レート制限（`app.py:37` が `allow_origins=["*"]`、ユーザー認証なし）。
- [ ] デプロイ基盤（Dockerfile・vercel.json が無い。CIはあるがCDが無い）。
- [ ] 取り込みの自動化（Windows機での手動 PowerShell 実行のまま）。
- [ ] JRA-VAN 規約確認（コードでは解決できないため早めに）。

### 次の候補

- [ ] ダートの「平均」帯を広げるか検討する（帯3.4ポイントに対しMAE 2.404で比1.4倍）。
- [ ] 芝モデルの過学習を抑える（検証誤差が [50] 2.64959 → [100] 2.65502）。

## 2026-08-04 完了: Claude Code — PAIをコース相対へ作り直し、評価軸も直した

その2で「PAIは展開適性ではなく脚質を符号化している」と確定したことへの対処。

- [x] `preferred_*`（絶対RPCI）を `sensitivity_*`（コース相対の感応度）へ置き換えた（pai-v3）。
- [x] 感応度を21万頭の実測比から較正した（逃げ1.0 / 先行0.65 / 自在0.6 / 差し・追込0）。
- [x] 両コースとも中立点で PAI=50 になることを確認した。ダートでの順序反転は構造的に解消。
- [x] `pci.py` と `rpci_forecast.py` の逆向きの記述（「スロー → 差し・追込有利」）を訂正した。
- [x] `--diagnose-pai` の脚質をまたいだ「不一致」判定を削除し、脚質内の上位1/3対下位1/3へ変えた。
- [x] `--compare-pai-weights` に上位帯の頭数を併記した。
- [x] 「PAI帯 → 好走率」が 40-60 帯で沈む理由を特定した（感応度0の脚質が積み上がる・仕様どおり）。
- [x] API 729テスト、Ruff、strict mypy、import-linter を完了した。

### 最優先の未完了タスク: 感応度の確定

`sensitivity_*` と `pace_swing` は比から目分量で置いた**暫定値**。実測では候補間に
決定的な差が出ていない（相関は ±0.007 の範囲）。

- [ ] `bed4de6` 後の `--compare-pai-weights` を再実行し、**上位帯の頭数を見てから**採否を決める。
      swing-light の芝 2.34x は頭数が減っただけの可能性が高い（振れ幅を狭めると base_pai の
      上限が 75 → 65 へ下がり、80-100帯へ届くのが `pace_affinity` の高い少数だけになる）。
- [ ] `--diagnose-pai` の脚質内の好走率差を主指標として判断する。相関を従、上位帯リフトは単独で使わない。
- [ ] 確定した値を `docs/DECISIONS.md` の ADR-2026-08-04 へ追記する（現状は暫定と明記済み）。

### 触ってはいけないこと

- [ ] **UI で馬を PAI 順に並べない。** ダートの追込は PAI 50（ペース非依存）だが好走率は 0.47x。
      同じ誤りを一度入れて撤回している（`5d9e01f`）。脚質をまたぐ提示は `style_advantage` を使う。

### 運用待ち・未確認

- [ ] 2026-08-04以降が100レース・ラベル別20レースに達したら `--monitor-dirt` で品質判定する。

### リリースに向けた未着手項目

- [ ] 認証・CORS制限・レート制限（`app.py:37` が `allow_origins=["*"]`、ユーザー認証なし）。
- [ ] デプロイ基盤（Dockerfile・vercel.json が無い。CIはあるがCDが無い）。
- [ ] 取り込みの自動化（Windows機での手動 PowerShell 実行のまま）。
- [ ] JRA-VAN 規約確認（コードでは解決できないため早めに）。

### 次の候補

- [ ] ダートの「平均」帯を広げるか検討する（帯3.4ポイントに対しMAE 2.404で比1.4倍）。
- [ ] 芝モデルの過学習を抑える（検証誤差が [50] 2.64959 → [100] 2.65502）。

## 2026-08-04 完了: Claude Code — 製品価値を測り、順位予想を看板から外した

「完成度を上げるには何をすべきか」への回答として、まず製品価値そのものを測った。
統合順位を「全馬平均」としか比べていなかったため、売り物として成立するか判断できていなかった。

- [x] 市場（単勝人気）との比較を実装した。同一レース集合へ揃えてから比べる。
- [x] 統合順位が市場に大きく劣ることを確定した（1位勝率 20.2% 対 36.9%・500レース・カバー率100%）。
- [x] 成分を切り分けた。PAIを順位付けへ使うと2.4ポイント悪化し、外しても市場に14.3ポイント届かない。
- [x] 順位予想を看板から外した。印を非表示、見出しを「参考」へ、配置を展開解説より後ろへ。
- [x] PAIが展開適性ではなく脚質を符号化していることを実測で確定した。
- [x] 自分が同日に入れた「展開が向く馬（PAI上位3頭）」を撤回した（ダートで0.47xの馬を推すため）。
- [x] API 720テスト、web 153テスト、Ruff、strict mypy、import-linter を完了した。

### 最優先の未完了タスク: PAI の再設計

**preferred をコース別へ引き直すだけでは不十分。** 実データは両コースとも
「逃げ > 先行 > 自在 > 差し > 追込」という同じ順序を示しており、
「脚質ごとに理想ペースがある」というPAIの前提そのものが支持されていない。

- [ ] `--pace-style-matrix` で素データ（ペース×脚質の好走率）を取り直す。
- [ ] 「脚質の定数効果」と「ペース依存の変動効果」を分離する設計を検討する。
      ADR-0010（前付けだけ採点・後方は常に互角）と整合させること。
- [ ] pai-v3 として実装し、PAI帯の単調性と好走率順の一致を検証する。
- [ ] 検証が済むまで、PAIを根拠にした個別馬の提示を増やさない。

### 運用待ち・未確認

- [ ] API を再起動し、実画面を確認する（pci-v3 移行後・順位降格後とも未確認）。
- [ ] 2026-08-04以降が100レース・ラベル別20レースに達したら `--monitor-dirt` で品質判定する。

### リリースに向けた未着手項目

- [ ] 認証・CORS制限・レート制限（`app.py:37` が `allow_origins=["*"]`、ユーザー認証なし）。
- [ ] デプロイ基盤（Dockerfile・vercel.json が無い。CIはあるがCDが無い）。
- [ ] 取り込みの自動化（Windows機での手動 PowerShell 実行のまま）。
- [ ] JRA-VAN 規約確認（コードでは解決できないため早めに）。

### 次の候補

- [ ] ダートの「平均」帯を広げるか検討する（帯3.4ポイントに対しMAE 2.372で比1.4倍）。
- [ ] 芝モデルの過学習を抑える（検証誤差が [50] 2.64959 → [100] 2.65502）。

## 2026-08-04 完了: Claude Code — レースRPCI式の誤りを修正し、全面移行を完了

利用者から「アプリのPCIがJRA公式(TARGET)と合わない」と指摘を受けて調査。
個馬PCI・PCI3は完全一致で、レースRPCI式だけに独立した2つの誤りがあった。

- [x] UIの「算出の根拠」「予測の根拠」から指数の実数値を除去し、回帰テストで固定した。
- [x] 同一画面でヘッダーと本文のペース区分が食い違う不具合を修正した。
- [x] レースRPCI式を修正した（中間区間を捨てる誤り／前半3Fを常に600m扱いする誤り）。
- [x] `formula_version` を pci-v2 → pci-v3 へ更新した。
- [x] 展開3分類の閾値を3分位へ再較正した（芝 49.7/54.0・ダート 44.8/48.2）。
- [x] 全15,332レースの `rpci_actual` を再計算した（12,196件更新）。
- [x] 芝・ダート両モデルを再学習し、同一条件比較で全指標の改善を確認して採用した。
- [x] `RuleWeights` の基準補正を新分布から引き直した（+5.0→+2.0 / −9.75→−3.5）。
- [x] 既知課題「芝『平均ペース』再現率0%」を解決し、docstring の誤った原因分析を差し替えた。
- [x] 監視ポリシーを新しい区分定義へ引き直した。
- [x] `.gitignore` の認証情報パターンを打ち間違いも拾うよう強化した（`.env]` が未追跡だった）。
- [x] API 699テスト、web 149テスト、Ruff、strict mypy、import-linter を完了した。

### 最優先の未完了タスク

- [ ] **PAI帯の非単調性を調査する**（新規発見）。好走率が 20-40帯 1.16x → 60-80帯 0.84x →
      80-100帯 1.29x とV字になる。芝・ダート共通で式の修正前から存在した。最上位帯の
      リフトは良好（1.25〜1.34x）だが中間帯の順序付けが機能していない。
      `apps/api/src/pci/domain/pace/adaptability.py` の見直しが要る。
- [ ] API を再起動し、実画面でペース表示がTARGETと整合することを確認する（未実施）。

### 次の候補

- [ ] ダートの「平均」帯を広げるか検討する。帯3.4ポイントに対しMAE 2.372で比1.4倍。
      芝（1.8倍）より判別が苦しい。ただし3分位（等頻度）を崩す判断になる。
- [ ] 芝モデルの過学習を抑える（検証誤差が [50] 2.64959 → [100] 2.65502 と悪化）。
      early stopping で実害はないが木の深さ・葉数の調整余地がある。
- [ ] 統合順位を監視する。ダートは v5→v6 で 50.6%→46.3%、芝は v1→v2 で 47.4%→49.1%。
      偶然の変動の可能性が高いが確認したい。
- [ ] 2026-08-04以降が100レース・ラベル別20レースに達したら `--monitor-dirt` で品質判定する。

## 2026-08-04 完了: Claude Code — 想定RPCIの安全弁下限を較正し、バイアスの原因を特定

- [x] 予測誤差を「クランプ端に張り付いた群」と「内側の群」へ分ける診断を追加した（`2a2acb0`）。
- [x] `--clamp-min` / `--clamp-max` で本番設定を変えずに較正を実測できるようにした（`db4d538`）。
- [x] ダートのバイアス+2.5が、モデルではなく安全弁の下限35.0由来（寄与95%）と確定した。
- [x] 下限を学習ラベル範囲の下端 20.0 へ較正した（`7875c02`、ADR-2026-08-04）。
- [x] 芝は本番設定でも上下限とも張り付き0件で、下限引き下げの影響が無いことを確認した。
- [x] API 658テスト、Ruff、strict mypy、import-linter を再実行した。

### 最優先の未完了タスク: ダートRPCI v5の本番採用

見送りの根拠だった「バイアスが縮まらない」は測定側の欠陥だった。新しい下限では
採用条件4項目すべてをv5が満たす（バイアス +0.012 < +1.653 / MAE 2.356 < 3.719 /
的中率 78.6% > 71.2% / PAI最上位帯リフト 1.27x > 1.19x）。以下は未着手。

- [ ] `models/rpci_lgbm_dirt_v5_full.txt` と `.meta.json` をリポジトリへ追加する
      （現状ユーザーのWindows機のみ。リポジトリに存在しない）。
- [ ] `.meta.json` へ `"model_version": "lgbm-dirt-v5-lap-history"` を追記する。
      **無いとmart層へv4と記録される**（v4と同じ特徴量数39のためフォールバックする）。再学習は不要。
- [ ] `_DEFAULT_DIRT_MODEL_PATH`（`lgbm_forecaster.py:51`）をv5へ切り替える。
- [ ] `rpci_monitoring.py` の `RpciMonitoringPolicy` をv5用に更新する
      （`expected_model_version` と、旧下限時代由来の閾値 MAE≤5.94 / |bias|≤4.62）。
      同時に更新しないと `MODEL_MISMATCH` で監視が止まる。

### 判断待ち（独断で確定しない）

- [ ] ADR-2026-08-02 が予約した評価期間（2026-08-03以降・100R以上・ハイ/平均/スロー各20R以上）の
      到達を待つか、測定欠陥の判明をもって 2026-06-01以降257R での再評価を採用根拠にするか。
      ユーザーからは採用指示が出ている。方針を確定してからモデルを差し替える。

### 次の候補

- [ ] 上限65.0（学習ラベル上限90と不一致）が拘束する期間がないか監視する。現状は張り付き0件。
- [ ] 芝の下限引き下げを長期間で検証する（2026-06以降348Rで非拘束を確認しただけ）。
- [ ] 「バイアスを主指標」として下した過去の採否（ADR-2026-08-02 の2件）を新しい下限で測り直す。

## 2026-08-03 引き継ぎ準備完了: OpenAI Codex → Claude Code

- [x] `5385ac5`以降のGit差分と実装範囲を自己レビューした。
- [x] 診断用一時JSON、生JV-Linkレコード、不要なデバッグコードが残っていないことを確認した。
- [x] ingestion-worker全260テスト、Ruff、strict mypyを再実行した。
- [x] `docs/HANDOFF.md`へ停止位置、暫定値、既知課題、具体的な再開手順を記録した。
- [x] `docs/DECISIONS.md`の既存決定で今回の設計判断が記録済みであることを確認した。

### Claude Codeの最優先タスク

- [ ] `diagnose_jv_result_offsets.py`の`_load_references()`、`save_references()`、`load_references()`へ血統登録番号を追加し、実JV-Link SEの同一馬対応を再検証する。
- [ ] `test_diagnose_jv_result_offsets.py`へ血統登録番号一致・不一致・旧JSON互換のテストを追加する。
- [ ] 同一馬対応と複数レースでの一意な固定長位置を確認できた場合だけ、`jv_spec.py`を更新してworker全テストを再実行する。

### 運用待ちタスク

- [ ] 枠順確定後、通常同期を再実行して18レースの事前予想を確定出馬表で更新する。
- [ ] 8月8〜9日分の結果同期後、新方式の芝・ダート照合件数と一致率を再評価する。
- [ ] iOS VoiceOverとAndroid TalkBackでサマリー・詳細タブの読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 実JV-Linkの人気・本賞金予約位置を検証

- [x] 実SEとmykeibadbの同一レース・馬番を最大50件照合する診断CLIを追加した。
- [x] 生レコード・馬名・期待値を表示せず、位置候補と支持件数だけを出力するようにした。
- [x] 32bit COMと64bit MySQLを一時JSONで安全に橋渡しし、実COM 50件を診断した。
- [x] 合成予約位置は人気1/50・本賞金0/50で、実JV-Link用には採用できないと確認した。
- [x] 予約拡張をmykeibadb合成時だけ解析し、jvlinkの偶然一致を取り込まないようにした。
- [x] worker全260テスト、Ruff、strict mypyを完了した。

### 次の候補

- [ ] 血統登録番号をアンカーに実JV-Link SEの対応を再検証し、確定着順を含む全体配置を再校正する。
- [ ] 枠順確定後に通常同期を再実行し、18レースの事前予想を確定出馬表で更新する。
- [ ] 8月8〜9日分の結果同期後、新方式の芝・ダート照合件数が増えることを確認する。

## 2026-08-03 完了: Codex — mykeibadb同期のAPI接続先を実行単位で切替

- [x] `run_batch.ps1`へ`-ApiBaseUrl`を追加し、`.env`読込後に明示値を優先するようにした。
- [x] `run_mykeibadb_full_sync.ps1`へ同オプションを追加し、事前疎通と全5バッチ工程へ転送した。
- [x] HTTP(S)の絶対URLだけを許可し、URL内の認証情報を拒否するようにした。
- [x] 8998番の最新APIで事前疎通と予想18件の生成を実行し、`.env`を変更せず完走した。
- [x] worker全253テスト、Ruff、mypy、PowerShell構文を確認した。

### 次の候補

- [ ] 枠順確定後に通常同期を再実行し、18レースの事前予想を確定出馬表で更新する。
- [ ] 8月8〜9日分の結果同期後、新方式の芝・ダート照合件数が増えることを確認する。
- [ ] iOS VoiceOverとAndroid TalkBackで、サマリーと詳細タブの読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 所有PID不明のAPI待受を安全に検出

- [x] 8000番の待受接続と所有プロセスを別々に保持するよう再起動スクリプトを修正した。
- [x] 待受あり・PIDなしを`ORPHANED`として検出し、停止も新規起動も行わないようにした。
- [x] 再起動時は2秒後に再確認し、残留中なら安全に中断するようにした。
- [x] STOPPED、BLOCKED、ORPHANED、READYの各経路とUTF-8・構文を検証した。

### 次の候補

- [ ] 枠順確定後に通常同期を再実行し、18レースの事前予想を確定出馬表で更新する。
- [ ] 8月8〜9日分の結果同期後、新方式の芝・ダート照合件数が増えることを確認する。
- [ ] iOS VoiceOverとAndroid TalkBackで、サマリーと詳細タブの読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 通常同期と新方式の事前予想生成を再開

- [x] mykeibadb.exeの差分取得を実行し、exit 0を確認した。
- [x] 2026-07-24〜2026-08-17の出馬表144レース、馬場情報、確定成績140レースを同期した。
- [x] 2026-08-08〜2026-08-09の特別登録18レースを取り込んだ。
- [x] 同18レースの事前予想を最新APIで生成し、生成18・スキップ0を確認した。
- [x] 新方式の照合件数は芝0・ダート0のままで、確定後に評価対象へ入ることを確認した。

### 次の候補

- [ ] 8月8〜9日分の結果同期後、新方式の芝・ダート照合件数が増えることを確認する。
- [ ] iOS VoiceOverとAndroid TalkBackで、サマリーと詳細タブの読み上げ順を実機確認する。
- [ ] `confidence_review_ready=true`になった時点で、3区分の母数と一致率を再評価する。

## 2026-08-03 完了: Codex — ローカルFastAPIの安全な再起動と最新版診断

- [x] 既存8000番がPCI AppのUvicornの場合だけ停止する再起動スクリプトを追加した。
- [x] 別プロセスがポートを使用している場合は停止せず`BLOCKED`で中断するようにした。
- [x] `-CheckOnly`で停止・準備未完了・旧API・最新APIをプロセス変更なしで診断可能にした。
- [x] Windows PowerShell 5.1向けUTF-8、構文、停止状態、別プロセス保護、最新API＋実DBを検証した。
- [x] 最新API＋実DBでは新方式が芝0件・ダート0件、目標各100件、`ready=false`と確認した。

### 次の候補

- [ ] 通常同期後の事前予想生成を継続し、新方式の芝・ダート件数を蓄積する。
- [ ] iOS VoiceOverとAndroid TalkBackで、サマリーと詳細タブの読み上げ順を実機確認する。
- [ ] `confidence_review_ready=true`になった時点で、3区分の母数と一致率を再評価する。

## 2026-08-03 完了: Codex — モバイル予想検証サマリーのフォーカスを可視化

- [x] 180日APIを確認し、現時点の照合が63件で再評価条件未達であることを確認した。
- [x] 閉じた予想検証サマリーへ、キーボードフォーカス時の内側2pxリングを追加した。
- [x] レイアウト寸法を変えず、白背景上で判別できるエメラルド色を既存配色から採用した。
- [x] フォーカス可視化クラスを対象回帰テストで固定した。

### 次の候補

- [ ] 稼働中APIを最新コミットで再起動し、新方式の芝・ダート件数を確認する。
- [ ] iOS VoiceOverとAndroid TalkBackで、サマリーと詳細タブの読み上げ順を実機確認する。
- [ ] `confidence_review_ready=true`になった時点で、3区分の母数と一致率を再評価する。

## 2026-08-03 完了: Codex — モバイル予想検証サマリーの読み上げを明確化

- [x] 閉じたサマリーへ、進捗・展開一致・検証件数・カバー率・操作内容をまとめた読み上げ名を追加した。
- [x] 省略表示の「ダ」は読み上げ時に「ダート」とし、進捗値へ「件」「目標」を付けた。
- [x] 未蓄積時と再評価可能時にも、状態に合う読み上げ名へ切り替えるようにした。
- [x] 390px向けの省略・縮小防止クラスと読み上げ文言を回帰テストで固定した。
- [x] 対象5テストとWeb型チェックを完了した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、サマリーと詳細タブの読み上げ順を実機確認する。
- [ ] `confidence_review_ready=true`になった時点で、3区分の母数と一致率を再評価する。

## 2026-08-03 完了: Codex — モバイル予想検証サマリーへ再評価進捗を表示

- [x] 閉じたモバイルサマリーへ「新指標 芝X/100 ダY/100」を表示した。
- [x] 条件到達後は同じ位置を「新指標 再評価可能」へ自動切替するようにした。
- [x] 展開一致率・検証件数・カバー率を維持し、サマリーの行高を増やさない構成にした。
- [x] 新旧API両方のフォールバック判定を閉じたサマリーにも適用した。
- [x] 対象回帰テストとWeb型チェックを完了した。

### 次の候補

- [ ] `confidence_review_ready=true`になった時点で、3区分の母数と一致率を再評価する。
- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 信頼度指標の再評価到達を自動判定

- [x] 100件基準をAPIの`_CONFIDENCE_REVIEW_TARGET_PER_TRACK`へ移し、方針の正本を一元化した。
- [x] 予想検証APIへ`confidence_review_target`と`confidence_review_ready`を追加した。
- [x] 芝・ダート双方が100件以上の場合だけ再評価可能とする回帰テストを追加した。
- [x] Webは未達時にコース別の残り件数、到達時に「再評価可能」を自動表示するようにした。
- [x] 旧APIでは目標100件と件数から画面側で補完し、更新ずれでも画面を継続表示する。
- [x] OpenAPI・api-client型、API全体検査、Web148テスト・型チェック・本番ビルドを完了した。

### 次の候補

- [ ] `confidence_review_ready=true`になった時点で、3区分の母数と一致率を再評価する。
- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 新しい信頼度指標のコース別蓄積進捗を可視化

- [x] 予想検証APIへ`confidence_cohort_groups`を追加し、新方式の全体・芝・ダート件数と一致率を返した。
- [x] 信頼度別欄へ芝・ダートそれぞれの`現在件数/100`と「各100件で再評価」を表示した。
- [x] 旧固定値は蓄積件数へ含めず、全体一致率だけには従来どおり含める契約を維持した。
- [x] API契約テスト、OpenAPI、api-client型、Web回帰テストを更新した。
- [x] API全体検査とWebテスト・型チェック・本番ビルドを完了した。

### 次の候補

- [ ] 新方式の芝・ダートが各100件へ到達した時点で、3区分の母数と一致率を再評価する。
- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 信頼度別検証から旧固定値を分離

- [x] `predicted_pace.factors`に保存される`classification_margin`を使い、信頼度算出方式を
  `classification-margin-v1`として検証用レコードへ伝搬した。
- [x] 全体・芝・ダートの一致率は全保存済み予想を維持し、信頼度別一致率だけを新方式の予想へ限定した。
- [x] 旧固定0.75が全体集計には残り、信頼度別集計からは除外される回帰テストを追加した。
- [x] Webへ「新しい読みやすさ指標で保存された予想のみを集計」と明示した。
- [x] DBマイグレーションや保存済みデータの書き換えなしで、新旧方式を識別可能にした。

### 次の候補

- [ ] 新方式の事前予想が芝・ダート各100件に達した時点で、3区分の件数と一致率を再評価する。
- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — LightGBM予想の固定信頼度を読みやすさ指標へ変更

- [x] LightGBM予想が常に`confidence=0.75`となり、検証63件がすべて「読みやすい」へ
  集約されていた原因を特定した。
- [x] 芝・ダート別の既存展開閾値からの距離を0.40〜0.90へ変換し、境界付近を低く、
  区分中央または境界から十分離れた予測を高くする表示用指標へ変更した。
- [x] 理由へ`classification_margin`を追加し、的中確率ではなく展開区分の読みやすさであることを明示した。
- [x] 境界・中央・外側の算出と、予想ごとに値が変わることを回帰テストで確認した。
- [x] 保存済み事前予想は当時の記録として維持し、遡及更新しない方針を仕様と設計判断へ記録した。

### 次の候補

- [ ] 新方式の事前予想が芝・ダート各100件に達した時点で、3区分の件数と一致率を再評価する。
- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 事前予想63件の初回不一致傾向レビュー

- [x] 180日集計で事前予想の照合が63件となり、初回レビュー条件の30件へ到達したことを確認した。
- [x] 一致率は全体61.9%（39/63）、芝60.5%（23/38）、ダート64.0%（16/25）だった。
- [x] 不一致24件を方向別に確認し、芝は実際が速め7件・遅め8件で均衡、ダートは
  実際が速め6件・遅め3件だった。
- [x] ダート不一致は9件と小標本で、評価対象1,640件に対する照合カバー率も3.8%のため、
  係数・閾値・モデルを変更せず蓄積を継続すると判断した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。
- [ ] コース別の照合が各100件を超えた時点で、不一致方向と信頼度区分を再レビューする。

## 2026-08-03 完了: Codex — ダートRPCI監視を3ラベル最低件数へ整合

- [x] 2026-08-03以降の実DB監視を実行し、現時点の評価対象が0件であることを確認した。
- [x] 最新ADRの「全体100件かつハイ・平均・スロー各20件」に対し、監視実装が
  ハイ20件だけを確認していた不整合を修正した。
- [x] 各ラベル件数を監視結果・CLI表示・JSONへ追加し、いずれか1区分でも20件未満なら
  `accumulating`を維持するようにした。
- [x] 対象テスト13件、API非統合630 tests、Ruff、mypy strict、import-linterを完了した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。
- [ ] 条件到達後のみ、完全未使用期間で現行v4と開催月候補v6を同一条件で再評価する。

## 2026-08-03 完了: Codex — 2022〜2025年ラップ原本とアプリDBの整合性を再確認

- [x] mykeibadbを読み取り専用で診断し、2022-01-01〜2025-12-31の芝・ダート全距離帯で
  S3/L3保有率が100%であることを確認した。
- [x] PostgreSQLを`diagnose_rpci.py --by-track-year`で再診断し、2022〜2026年の芝・ダートが
  全年ともラップ由来100%、`rpci_actual`外れ値0件であることを確認した。
- [x] 2025年の約77レース欠損と、2022〜2025年の原本確認を未完了としていた古い記録を訂正した。
- [x] データ再同期やDB更新は不要と判断し、読み取り専用の診断だけで終了した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。
- [ ] 2026-08-03以降の確定ダートが100レース以上、かつ各展開ラベル20レース以上に
  達した時点で、現行v4と開催月候補v6を完全未使用期間で再評価する。

## 2026-08-03 完了: Codex — 確定成績の部分送信失敗を終了コードへ反映

- [x] `ingest_results()`が返す成功・失敗件数を`main()`で全日付チャンク分集計するようにした。
- [x] レース単位の処理継続は維持し、全チャンク終了後に失敗が1件以上あれば
  RuntimeErrorへ集約して、取り込みログ`status=error`と終了コード1へ反映するよう修正した。
- [x] `run_batch.ps1`が非ゼロ終了を検出できるため、既存の最大3回再試行と最終失敗通知へ接続された。
- [x] API送信失敗を模擬し、終了コード1・エラー取り込みログ・失敗件数を確認する回帰テストを追加した。
- [x] ingestion-worker 251 tests、ruff、mypy strictを完了した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-03 完了: Codex — 同期プリフライトのWindows実地確認とUTF-8回帰修正

- [x] `run_mykeibadb_full_sync.ps1 -PreflightOnly`をWindows実行機で動かし、
  FastAPI／PostgreSQLと読み取り元MySQL（83テーブル）の疎通確認がexit 0で完了することを確認した。
- [x] 新しいMySQL疎通チェックのPython出力だけ文字化けする回帰を発見し、同期ラッパーへ
  Windows PowerShell 5.1向けUTF-8入出力設定を追加した。
- [x] PowerShellスクリプトを既存規約のCRLFへ正規化し、日本語コメント直後の初期化が
  Windows PowerShell 5.1で欠落しないことを実行確認した。
- [x] UTF-8設定の静的回帰テストを追加した。
- [x] ingestion-worker 250 tests、ruff、mypy strictを完了した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-02 完了: Codex — モバイル詳細タブのキーボード操作と警告要約を改善

- [x] 出走前・確定後で重複していたモバイル詳細タブを`MobileTabList`へ共通化した。
- [x] WAI-ARIA Tabsパターンに合わせ、選択中だけを`tabIndex=0`とするroving tabindex、
  左右矢印・Home・Endによる選択切替とフォーカス移動を実装した。
- [x] 取り込み警告へ「要対応」「成績未取込」「馬場情報」「重複」などのモバイル要約チップを追加し、
  復旧手順とコピー操作を44px以上のタップ領域にした。
- [x] Web 141 tests、型チェック、本番ビルドを完了した。
- [x] 390pxの実ブラウザで横はみ出しがなく、サマリーから隊列へ矢印キーで移動し、
  `aria-selected`・フォーカス・表示内容が同期することを確認した。
- [x] 非選択パネルを空の`hidden`要素として保持し、全タブの`aria-controls`参照先が
  常にDOM上に存在するよう改善した。非選択内容は描画せず、初期表示の軽さを維持する。
- [x] モバイル・デスクトップの隊列表示が同時にDOMへ存在する場合の`formation-heading`重複を解消した。
- [x] 開催日ストリップを「開催日の選択」ナビゲーションランドマークへ変更した。
- [x] 競馬場タブへroving tabindexと左右矢印・Home・End操作を追加し、390px実ブラウザで
  札幌から新潟へフォーカス・選択・表示レースが同期することを確認した。
- [x] 開催日、競馬場タブ、レース行、詳細タブ、隊列内の馬、同一開催レース移動へ
  明示的な`focus-visible`リングを追加し、キーボード操作時の現在位置を見失いにくくした。
- [x] 390px実ブラウザで競馬場タブと詳細タブの緑色2pxリングを実測し、選択状態との同期と
  ページ全体に横スクロールが発生しないことを確認した。
- [x] スマホヘッダーで名前を失っていたアイコンリンクへ「予想検証」「レース一覧」の
  読み上げ名を付け、44pxの操作領域と明示的なフォーカスリングを確保した。
- [x] 390px実ブラウザで両リンクが44×44px、読み上げ名あり、横はみ出しなしと確認した。
- [x] OSの`prefers-reduced-motion`設定を尊重し、対象利用者にはスムーズスクロール、
  遷移、ローディングアニメーションを抑制するグローバルスタイルを追加した。
- [x] 回帰テストを追加し、本番ビルド後の圧縮CSSにもモーション抑制ルールが残ることを確認した。
- [x] 390px実ブラウザで表示中テキストのコントラストを監査し、取り込み警告見出し、
  一覧の補助文字、非選択競馬場タブをWCAG AA相当へ改善した。修正後の検出不足は0件。
- [x] 月カレンダーの「他の日程を探す」、年月移動、開催日を44px以上のタップ領域へ統一した。
- [x] 390px幅のホームと確定後詳細を操作領域監査し、ヘッダーロゴ、取り込み対象レース、
  検証期間切替、折りたたみ見出し、競馬場タブ、同一開催R移動を44px以上へ統一した。
- [x] 修正後は両画面とも表示中の44px未満の操作対象0件、横はみ出し0件を確認した。
- [x] 390px幅でホーム・出走前詳細・確定後詳細のアクセシビリティ構造を監査し、
  重複ID、壊れたARIA参照、名前のない表示中操作要素がすべて0件であることを確認した。
- [x] 取り込み警告内のラベルを見出しからセクションのアクセシブル名へ変更し、
  モバイル予想検証へ非表示`h2`を追加した。ホームの見出し階層を`h1 → h2 → h3`へ揃えた。
- [x] Web 146 tests、型チェック、本番ビルドを完了した。

### 次の候補

- [ ] iOS VoiceOverとAndroid TalkBackで、詳細タブの選択状態と読み上げ順を実機確認する。

## 2026-08-02 完了: Codex — 新規29レースでダートRPCI候補を予備評価した

- [x] これまで未使用だった2026-07-24以降の確定ダート29レース・365頭を抽出した。
- [x] 現行v4と開催月候補v6を同一条件で比較した。

| 指標 | 現行v4 | 開催月候補v6 |
|---|---:|---:|
| MAE | 3.283 | **3.003** |
| バイアス | **+2.014** | +2.045 |
| 展開ラベル的中率 | 62.1% | **69.0%** |
| PAI最上位帯リフト | 1.14x | **1.24x** |
| 統合1位馬の好走率 | 48.3% | **51.7%** |

- [x] **判断: 本番昇格なし。** 副指標の改善方向は再現したが、29レースでは母数不足で、
  一次指標のバイアスも`+2.014→+2.045`と改善しなかった。
- [x] 2026-07-24〜2026-08-02を予備評価済み期間とし、次の完全未使用期間を
  2026-08-03以降に予約した。

### 次の候補

- [ ] 2026-08-03以降の確定ダートが100レース以上、かつハイ・平均・スロー各20レース以上に
  達するまで候補の採否を再判定しない。蓄積中は現行v4を維持する。

## 2026-08-02 完了: Codex — ダートRPCIの開催月特徴量を独立評価し、本番採用を見送った

- [x] 実DB 7,431レースの分布を診断した。月別平均RPCIは`35.7〜44.3`と差が大きく、
  馬場状態別は`41.7〜42.6`と差が小さかったため、開催月だけを候補特徴量に選んだ。
- [x] `feature-set=v5`としてv4へ開催月one-hot 12列を追加した。学習SQLと推論の
  `RaceContext.race_month`で同じ順序を使い、v1〜v4モデルの互換性を維持した。
- [x] 2026-06-01より前の7,431レースで`lgbm-dirt-v6-month`候補を学習した。
  学習内テストはMAE 2.345、バイアス -0.576。
- [x] 同一の独立226レースで現行v4と比較した。

| 指標 | 現行v4 | 開催月候補v6 |
|---|---:|---:|
| MAE | 4.592 | **3.896** |
| バイアス | **+2.571** | +2.574 |
| 展開ラベル的中率 | 71.7% | **77.4%** |
| PAI最上位帯リフト | 1.18x | **1.24x** |
| 有利−不利の好走率差 | +1.4% | **+2.0%** |

- [x] **採用判断: 見送り。** MAE・分類・PAIは改善したが、事前に主指標とした期間外
  バイアスが`+2.571→+2.574`で縮小しなかった。本番は`rpci_lgbm_dirt_v4.txt`を維持する。

### 次の候補

- [ ] 月だけでは残った系統バイアスについて、開催月×競馬場などの交互作用を検討する。
  実装前に最低標本数と採用基準を定め、今回と同じ独立期間を再利用し過ぎないこと。

## 2026-08-02 完了: Codex — ダートRPCI再学習候補を評価し、本番採用を見送った

- [x] 初回`v5_full`の学習来歴で旧フォールバック式が0.3%混在していることを検出した。
- [x] mykeibadb側は2022-01-01〜2026-05-31のS3/L3が100%であることを診断した。
- [x] アプリDBの欠損を追跡し、2025-06-28〜2025-07-20の旧形式重複キー76件が原因と特定した。
- [x] 4週末・計280レースを再同期し、安全ガード付きで重複77組を統合した。
  最終確認はS3/L3欠損0件、同期間の重複0組。
- [x] 全7,431レースで再学習し、`lap_derived_ratio=1.0`を確認した。
  テストセットはMAE 2.330、バイアス -0.550。
- [x] データ統合後の同一226レースで現行v4と候補v5を再比較した。

| 指標 | 現行v4 | 候補v5_full |
|---|---:|---:|
| MAE | 4.592 | **3.881** |
| バイアス | **+2.571** | +2.628 |
| 展開ラベル的中率 | 71.7% | **79.2%** |
| PAI最上位帯リフト | 1.18x | **1.26x** |
| 有利−不利の好走率差 | +1.4% | **+2.6%** |

- [x] **採用判断: 見送り。** MAE・分類・PAI・展開有利度は改善したが、主指標のバイアスが
  `+2.571→+2.628`とわずかに悪化した。ADRの事前基準を変更せず、既定モデルはv4を維持する。
- [x] 同じv4特徴量でも再学習世代を識別できるよう、学習来歴JSONへ`model_version`を保存し、
  推論時に読み取る仕組みを追加した。来歴がない旧モデルは従来の特徴量世代へフォールバックする。

### 次の候補

- [ ] `tasks/backlog.md`のダート固有特徴量追加を再検討する。単純な再学習だけでは
  系統バイアスが縮まらなかったため、現行v4を維持したまま独立候補として進める。

## 2026-08-02 完了: Claude Code — 同期プリフライトへ読み取り元MySQLの疎通確認を追加

- [x] 事象: MySQL80停止時、プリフライトは「ready」で通過し、`mykeibadb.exe`が
  exit 0を返し、`batch.py`が3回リトライして初めて接続拒否が判明していた。
  プリフライトが書き込み先しか見ていなかったのが原因。
- [x] `ingestion/check_mykeibadb.py`を新規追加。`batch.py`と同じ設定で実接続し、
  `SHOW TABLES`まで確認。エラーコードでサービス停止/認証失敗/DB名違いを分類し、
  MANUAL_SYNC_GUIDEの該当節へ誘導する。テーブル0件も失敗扱い。
- [x] `run_mykeibadb_full_sync.ps1`のプリフライトへ組み込み（`mykeibadb.exe`起動前）。
  stderrで終了エラーにならないよう`$ErrorActionPreference`を一時的にContinueへ。
- [x] `MANUAL_SYNC_GUIDE.md`を更新（単独実行コマンド、§6.1へservices.msc手順）。
- [x] ingestion 249 tests（新規10件）・ruff pass。新規ファイルはmypy --strict 0エラー。
- [x] Windows実行機で`-PreflightOnly`を実行し、API／PostgreSQLと読み取り元MySQLの
  両方を確認してexit 0で終了することを確認した（2026-08-03、Codex）。

## 2026-07-26 (14) ✅ 完了: Claude Code — ADR-0010採用（style-advantage-v4）

- [x] 独立2期間×芝ダートの4条件で候補を比較し、**`back-neutral`（差し・追込を常に互角）を採用**。
  好走率差は4条件すべて改善（芝+8.1%/+4.0%、ダート+3.4%/+0.6%）で全候補中最良。
- [x] **自在の採点は取り下げ**。直近ダートで悪化し、自在自身の単調性も4条件中1条件のみ。
  → `entries`は4件のままで、API公開スキーマとWeb表示への影響なし。
- [x] ADRの採用条件を改訂。「差し追込が単調」は後方を順序づけない候補には原理的に
  満たせないため、単調性は候補が順序づけを主張するグループにのみ課す。
- [x] 比較ツールの欠陥2件を修正（自在がどのグループにも属さない／`stalker_gain=0`が
  差しを1帯へ潰す）。修正前の判定は信頼できなかった。
- [x] `stalker_gain=0.0`/`closer_gain=0.0`を既定化、`MODEL_VERSION`を`style-advantage-v4`へ、
  `reasons`から「ハイ→後ろが有利」を削除。OpenAPI/api-client再生成。
- [x] API 616 tests・ruff・mypy --strict・lint-imports、Web 136 tests・typecheck・build成功。
- [ ] **残課題: ダートは現行でも前付けの帯別好走率が単調にならない。**
  係数ではなくダート固有の別要因。本件と切り離して追う。

## 2026-07-26 (13) 🚨 要判断: Claude Code — 展開有利度は「前付け馬」にしか効いていない

- [x] (12)のバックフィルをユーザーが完了。`rpci_actual`が全期間ラップ由来へ統一され、
  ダートの1〜7月平均が2022〜2026年で41.1〜42.0へ収束。外れ値も96件→0件になった。
- [x] 統一後に再測定した結果、**旧「ダートで+8〜12pt」は較正ずれによる見かけの数字**と確定。
  実際は芝・ダートとも**+2.9%**（旧: 芝+4.6%・ダート+8.9%）。
- [x] `--validate-style-advantage`へ表示ラベル別の帯集計（commit `f4ff23e`）と
  脚質グループ別集計（commit `34ee306`）を追加。帯境界はwebの`styleVerdict`と一致させ、
  境界一致を検証するテストを付けた。API 601テスト・ruff・mypy --strict・lint-imports成功。
- [x] **核心的発見**: 有利度は**前付け（逃げ・先行）にしか効いていない**。
  - 芝・前付け 0.84x→1.18x（単調）／ダート・前付け 0.94x→1.14x（概ね単調）
  - 芝・差し追込 0.98〜1.03x（**完全に平坦**）／ダート・差し追込は有利帯で**逆行**
  - 差し追込は出走頭数の70〜74%を占めるがシグナルなし。相関の低さの主因。
- [x] **併せて判明**: 前付けと差し追込で好走率ベースラインが約2倍違う
  （芝33.9% vs 18.2%、ダート33.9% vs 15.9%）が、有利度スコアは全馬50中心で
  この差を表現していない。展開恩恵馬・統合順位への波及の可能性あり。
- [x] `docs/SPEC.md` §3.4・`docs/HANDOFF.md`へ記録。
- [x] ユーザー判断で「(3) UI表示だけ先に直す」を選択し実施（ドメイン層は無変更）。
  - `styleAdvantageScores()`へ`isDirectional`/`note`を追加。差し・追込は
    「展開の影響は小さい」と表示し、スコア数値と進捗バーを出さない
    （文言より視覚が勝って「有利」と読まれるため）。理由文はリスト下に一度だけ。
  - 脚質別プロファイルは差し・追込を淡色＋「（参考）」表示に。
  - ハイペース時の文言から裏付けのない「差しが届きやすい」系の主張を外した
    （スロー時は実データが支持するため変更なし）。
  - Web 136 tests（新規2件・既存1件を新仕様へ更新）、typecheck、build成功。
    Playwrightで PC/スマホ/プロファイルの3箇所を実コンポーネントで目視確認済み。
- [x] ドメイン是正の判断材料として`--pace-style-matrix`を追加（commit `baad549`）。
  有利度スコアを介さず、実績ペース×確定脚質の素の好走率を集計する。
  自在は有利度の対象外だが出走の3割超を占めるため集計に含めた。
- [x] 実測（芝101,478頭・ダート107,828頭）でスロー−ハイの差を測定:
  逃げ +0.33/+0.15、先行 +0.19/+0.16、**自在 +0.17/+0.11**、差し +0.05/-0.04、
  追込 -0.05/-0.09（芝/ダート）。**ペース感応度は脚質間で最大6倍以上違い、
  最大グループの自在が採点されていない**と判明した。
- [x] **ADR-0010（Proposed）を起票**。脚質ごとの独立係数化＋自在の採点対象化を提案。
  係数の具体値はADR内でも確定せず、独立2期間の検証後に採用する方針を明記した。
- [x] ADR-0010は後続タスクで採用判断を完了し、`back-neutral`を`style-advantage-v4`として採用した。
  自在の採点追加は独立評価で悪化したため取り下げ、公開スキーマは4件のまま維持した。
- [x] 2025年のラップ欠損は、旧形式の重複キー76件を統合して解消した。
  2026-08-03の再診断でも、2025年の芝・ダートともラップ由来100%を確認した。

## 2026-07-26 (12) ✅ 完了: Claude Code — 展開有利度のダート崩壊＝rpci_actual の算出式切替が原因

- [x] 発端: 一般公開の完成度評価中に、展開有利度が2026年前半で逆相関と判明
  （直近300レース混合 -10.7pt）。中核指標のため公開可否に直結する問題として切り分け。
- [x] `--validate-style-advantage --style-breakdown year`で2022〜2026年を検証。
  **ルール自体は5年間・両コースで一貫して有効**（芝+3.1〜+6.1pt、ダート+7.7〜+11.9pt）。
  「ルールが壊れている」という初期判断は誤りだった。
- [x] 同一月窓（5〜7月）で前年比較。芝は問題なし（+2.5%→+4.4%）、
  **ダートのみ+18.8%→-0.6%**。季節性ではないと確定。
- [x] `scripts/diagnose_rpci.py`へ`--by-track-year`/`--month-from`/`--month-to`を追加
  （commit `7e10be2` / `1d33c3f` / `962a369`）。RPCI分布・確定脚質構成比・算出経路の3表を出力。
- [x] **原因確定**: `aggregate_rpci`のラップ由来式とフォールバック式が**ダートで平均5.4pt乖離**し、
  ラップ保有率が2022〜2024年0% → 2026年100%へ切り替わっていた（2025-07-24のバックフィル境界）。
  ダートの中立点43.0は旧フォールバック分布での較正値のため新分布と合わず、
  「有利」判定馬が24%→42%へ増えて選別力を失った。芝は2経路の差が小さく無傷。
- [x] `docs/SPEC.md` §3.4・`docs/HANDOFF.md`へ記録。2026-07-23時点の
  「2026年7月だけ悪い＝開催条件に局在」という解釈は見直しが必要と明記した。
- [x] mykeibadbが2022〜2025年のラップを保持していることを確認し、全期間バックフィルを完了した。
  2026-08-03の再診断でも、2022〜2025年の全距離帯でS3/L3保有率100%を確認した。
- [x] 暫定的な`reliability=reference`への降格は不要と判断した。算出経路をラップ由来へ統一し、
  後続の独立評価で現行v4維持を決定した。
- [x] ダートRPCI学習データの旧式混在を解消し、再学習来歴で`lap_derived_ratio=1.0`を確認した。
- **後続判断**: 算出経路統一後の独立評価でも主指標のバイアスが改善しなかったため、
  ダート閾値40.0/46.0・中立点43.0と現行v4モデルを維持している。

## 2026-07-26 (11) 完了: Claude Code — 成分ブレンド重み再検証（母数拡大） → 採用は様子見

- [x] (10)の副次発見（`form-only`/`form-heavy`が2026-07-22の結論と食い違う）を、
  2026-07-22と同じ日付区切り（2025-07-01〜12-31、2026-01-01〜07-21）・`--limit 300`
  でユーザーが実DB再検証。
- [x] 母数は2026-07-22時点より増加（212→300件、97→300件）。
  `form-only`は両期間・全3指標（1位勝率・1位好走率・TOP3捕捉率）が改善し採用基準を満たした。
  `form-heavy`は前半期間の1位勝率のみ僅かに悪化し基準未達。`market-aware`は後半期間で
  明確に悪化し不採用。`recent10`は両期間ともTOP3捕捉率が悪化し(10)の不採用判断を再確認。
- [x] ユーザーに`form-only`採用可否を確認 → **「もう少し様子見」**を選択。
  母数拡大での結論反転が今回初見のため、安定性を見てから判断する方針。
  `AbilityWeights`のデフォルト（form0.55/本賞金0.30/人気0.15）は変更しない。
- [x] `docs/SPEC.md §9-16`・`docs/HANDOFF.md`に結果と保留理由を記録した。
- [ ] **次回への引き継ぎ**: データがさらに蓄積された時点で同じ日付区切りまたは新しい
  直近期間で`form-only`が引き続き両期間・全指標を改善するか再確認する。安定して
  再現するなら`weight_form=1.0`/`weight_prize=0.0`/`weight_popularity=0.0`への
  デフォルト変更を検討する。

## 2026-07-26 (10) 完了: Claude Code — 地力(ability)参照走数10走の実DB検証 → 不採用

- [x] (9)で追加した`recent10`候補をユーザーがWindows実行機で実DB比較（独立2期間）。
  - 直近200レース: 1位勝率+1.5% / 1位好走率-1.0% / TOP3捕捉率-0.3%
  - 〜2025-12-31以前200レース: +1.5% / +1.5% / -0.5%
- [x] TOP3捕捉率が両期間で悪化、1位好走率も符号不安定なため、
  RuleWeights/PaiWeightsと同じ基準（両期間で全指標が悪化しない）を満たさず**不採用**。
  `AbilityWeights.recent_races=5`を本番維持（コード変更なし、`recent10`候補はCLIに残置）。
- [x] `docs/SPEC.md §9-16`・`docs/HANDOFF.md`に結論を記録した。
- [ ] **副次発見（未着手・要ユーザー判断）**: 同じ実行で成分ブレンド候補`form-only`/
  `form-heavy`が両期間・全3指標で改善していたが、2026-07-22の同候補比較
  （別の日付区切りで「改善する候補なし」と結論済み）と食い違う。日付区切りを
  2026-07-22と揃えて再現するか確認してから着手するかどうかはユーザー判断待ち。

## 2026-07-26 (9) 完了: Claude Code — 地力(ability)の参照走数10走候補を準備（本番は5走のまま）

- [x] 背景: ユーザーから「統合順位予想の地力は5走だと短く、得意なペースで走った
  6走目以降を見落とす恐れがあるので10走へ変更すべき」との提案。
- [x] 調査して回答: 「得意なペース」判定（`affinity.py`のPAI適性プロファイル）は
  実はすでに最大12走（好走のみ抽出）、前付けペース傾向は10走を参照済みで、
  5走に限定されているのは「地力(ability)」と「脚質判定」の2つだけと判明。
  ユーザーの提案は「地力」を対象と再確認した。
- [x] 重要な副次発見: `AbilityWeights.recent_races`を変えても、呼び出し元
  （`forecast_use_cases.py`の`_build_ability_score`）が`history[:5]`と別途
  ハードコードしており、設定を変えても反映されない不具合があった。今回修正した
  （呼び出し元の切り詰めを撤去し、`AbilityScorer`側の`recent_races`だけに委ねる）。
- [x] 検証方針をユーザーに確認し、「比較ツールに10走候補を追加（推奨・実装のみ、
  本番デフォルトは変更しない）」を選択された。
- [x] `backtest.py`の`DEFAULT_ABILITY_WEIGHT_PROFILES`へ`recent10`候補
  （`AbilityWeights(recent_races=10)`）を追加した。既存の
  `--compare-ability-weights`が自動的にこの候補も比較対象に含める。
- [x] 上記のハードコード不具合を検出する回帰テストを追加
  （直近5走を不振・6〜8走前をG1好走という履歴で、`recent_races=10`なら
  `sample_size=8`・地力スコアが上がることを確認）。
- [x] API 598 tests（新規1件）、ruff、mypy --strict、lint-importsすべて成功。
- [x] 実DB検証・採用判断は完了。結論は上の(10)を参照（不採用、5走を維持）。

## 2026-07-26 (8) 完了: Claude Code — スマホに統合順位予想（展開×能力）を追加

- [x] 背景: ユーザーが「PC版では展開・能力を鑑みた全馬の総合予想順位が表示されるが、
  スマホでは見れない。仕様か」と質問。確認したところ`IntegratedRankingView`が
  `RaceForecastDashboard.tsx`（PC版）にしか組み込まれておらず、スマホ版
  `MobileRaceForecastDashboard.tsx`には統合順位予想Phase1/2実装時に反映漏れが
  あったと判明（仕様ではなく実装漏れ）。
- [x] 追加先をユーザーに確認し、「『注目馬』タブへ追加（推奨）」を選択された
  （既存の展開恩恵馬TOP5・評価を下げたい馬と同じ「馬のランキング系」タブへ集約。
  サマリータブの短さは維持）。
- [x] 「注目馬」タブの先頭（展開恩恵馬TOP5より前）へ`IntegratedRankingView`を追加した。
- [x] 副次対応: 枠色統一作業（(5)(6)）で見落としていた`IntegratedRankingView.tsx`自身の
  重複した枠色定義（独自の`FRAME_CLASS`）を発見し、共有の`frameColorClass()`へ統一した
  （テスト0件だったため今回`IntegratedRankingView.test.tsx`を新規追加）。
- [x] 新規6 tests（`IntegratedRankingView.test.tsx`5件・枠順未確定時の表示統一1件）を
  含むWeb 134 tests、typecheck、production buildすべて成功。Playwrightで
  「注目馬」タブ相当（統合順位予想＋展開恩恵馬TOP5を同一ページに再現）を
  390px幅で確認し、横はみ出しなし・タグの折り返しも自然であることを確認した。

## 2026-07-26 (7) 完了: Claude Code — ダートレースの展開速度誤判定を修正（重要バグ）

- [x] 背景: ユーザーがスマホ画面を提示し、「画面上部は『平均ペース』なのに、詳細タブの
  『展開』は『かなり速い流れ』になっている。この違いは何か」と報告。
- [x] 原因を特定: ペース速度判定が**バックエンドとフロントエンドで別々に実装**されており、
  食い違っていた。
  - バックエンド`classify_pace()`（rpci_forecast.py）は芝・ダートで別閾値を使う
    （芝: 高49.0/低51.0、ダート: 高40.0/低46.0。ダート実績平均43.0が芝53.1と
    大きく異なるための専用閾値、rule-v4）。上部の「想定展開」「読みやすい％」は
    この判定結果（`pace_label`）をそのまま表示しているため常に正しい。
  - フロント`paceSpeedFromIndex()`（lib/pace.ts）は**トラック種別を区別しない
    固定閾値**（47/50/52/55、芝寄り）を使っていたため、ダートの想定RPCI
    （ダートとしては「平均」域の40〜46でも）が47未満なら「かなり速い流れ」と
    誤判定していた。この関数は「詳細」タブの展開チェックリストだけでなく、
    展開予想ヒーロー・確定後分析の各馬ペース傾向・PCI3表示など7ファイルで
    使われており、**ダートレース全般で発生し得る不具合**だった。
- [x] 修正方針をユーザーに確認し、「3段階へ簡素化（推奨）」を選択された
  （既存5段階〈超ハイ/ハイ/平均/スロー/超スロー〉を、バックエンドと同じ3段階
  〈ハイ/平均/スロー〉へ統一し、新しい閾値は一切発明せずバックエンドの
  既存閾値をそのまま使う）。
- [x] `paceSpeedFromIndex(value, trackType)`をトラック種別必須の3段階判定へ再設計し、
  呼び出し元7ファイル（`PaceHeadline`・`RaceForecastDashboard`・
  `MobileRaceForecastDashboard`・`RaceHero`・`MobilePaceAnalysisDashboard`・
  `PaceAnalysisTable`・`pace-analysis/page.tsx`）すべてで`race.track_type`を渡すよう修正。
  未使用だった`paceSpeedLabel`/`paceSpeedSymbol`ラッパーは削除した。
- [x] `pci=44・ダート`が「平均」、`pci=44・芝`が「ハイ」になる、という不具合の
  直接的な再現テストを`lib/pace.test.ts`と`PaceAnalysisTable.test.tsx`へ追加。
  Web 129 tests（新規9件）・typecheck・production buildすべて成功。
  Playwrightでダート想定RPCI=44のレースを再現し、ヒーローの「平均」表示に
  変化が無いこと（元々正しかった部分に回帰が無いこと）を確認した。

## 2026-07-26 (6) 完了: Claude Code — 確定後分析にも馬番バッジの枠色を拡張（バックエンド対応）

- [x] 前タスクで保留にした`PaceAnalysisTable.tsx`（確定後分析デスクトップ表）へ、
  ユーザーの指示（「お願いします」）を受けて着手した。
- [x] `HorsePaceAnalysisOutput`（dto.py）・`HorsePaceAnalysisSchema`（schemas.py）へ
  `frame_no`を追加し、`GetPaceAnalysisUseCase`（既存の`RaceEntry.frame_no`から供給）→
  OpenAPI再生成→`packages/api-client`型再生成の順で結線した。
- [x] `PaceAnalysisTable.tsx`（デスクトップ表）と、あわせて発見した
  `MobilePaceAnalysisDashboard.tsx`の`MobilePaceResultRow`（スマホ「全馬」タブ、
  同じく色が付いていなかった）の両方を`frameColorClass()`へ統一した。
- [x] 完全に未使用になった`globals.css`の`.horse-no`/`.horse-no.sm`定義を削除した
  （旧`HorseFitTable.tsx`と`PaceAnalysisTable.tsx`がそれぞれ最後の参照元だったが、
  前回・今回の変更でどちらも参照しなくなったため）。
- [x] 新規4 tests（`PaceAnalysisTable.test.tsx`新規2件、`test_pace_analysis_use_cases.py`
  1件、`MobilePaceAnalysisDashboard.test.tsx`のアサーション強化）を含む
  API 597 tests・Web 125 tests、ruff・mypy strict・lint-imports・typecheck・
  production buildすべて成功。OpenAPIスナップショット契約テストも再生成後に合格を確認した。
  Playwrightで確定後分析のデスクトップ表・スマホ行の両方が正しい枠色で表示され、
  横はみ出しが無いことを確認した。

## 2026-07-26 (5) 完了: Claude Code — 馬番バッジの枠色を全画面で統一

- [x] 背景: ユーザーがスマホ画面4枚を提示し、「隊列予想では馬番ごとに枠色が塗られているが、
  他の画面（サマリー/注目馬/詳細タブ）では色が塗られていなかったり表示形態が異なる。
  隊列予想と同じにしてほしい」とフィードバック。
- [x] `FormationView.tsx`にだけ枠色定義（`FRAME_CLASS`）があり、他画面は未使用・不統一
  だったと確認。`lib/pace.ts`へ`frameColorClass()`として一本化し、
  `FormationView.tsx`・`MobileRaceForecastDashboard.tsx`（サマリー/注目馬タブの
  馬番バッジ）・`HorseFitTable.tsx`（詳細タブ）の計4箇所を同じ配色へ統一した。
- [x] `HorseFitTable.tsx`は表示形態も合わせ、バッジ内は隊列予想と同じ「馬番のみ」へ変更
  （従来は「馬番16」という文言がバッジ内にあった）。枠順未確定（frame_no=0）時は
  色を付けず「登録」と表示し、「登録順N（馬番未確定）」の文言は行内テキストへ残した
  （確定時に文言を付け足すと1行に収まらなくなる例をPlaywrightで確認したため、
  確定時はバッジのみ・未確定時だけ文言を残す形にした）。
- [x] 新規9 tests（`frameColorClass`単体4件、`MobileRaceForecastDashboard`1件、
  `HorseFitTable`新規ファイル2件、既存2件の枠色アサーション強化）を含むWeb 123 tests、
  typecheck、production buildが成功。Playwrightで18頭・複数枠のモックデータを用い、
  サマリータブの黒/緑/白いずれのカード背景でも枠色バッジが視認できること、
  横はみ出しが無いことを確認した。
- **未対応（ユーザーへ確認待ち）**: 確定後分析の`PaceAnalysisTable.tsx`（デスクトップの
  確定済み馬別PCI表）にも同じ「枠色が付いていない」箇所がある。ただしこちらが使う
  `HorsePaceAnalysisSchema`には`frame_no`が含まれておらず、APIのdto/schema/OpenAPI再生成を
  伴うため、フロントだけの今回の変更より一段大きい。ユーザーの指示があれば着手する。

## 2026-07-26 (4) 完了: Claude Code — 開催日選択UIの改善（年表示・日付ストリップ絞り込み・月カレンダー展開）

- [x] 背景: ユーザーがJRA-VAN公式スマホアプリの画面（開催日チップ＋「別日程で検索」→
  年選択＋開催日一覧）を提示し、「開催日を選ぶ操作が煩わしい。何年何月何日かわからない上、
  遡るのに大量スライドが必要」とフィードバック。コードを確認し原因を特定した。
  - `RaceDateCalendar.tsx`のスマホ見出し・各チップが年を一切表示していなかった。
  - `GET /api/v1/races/dates`は全開催日を無制限に返す設計で、スマホの日付ストリップも
    それを丸ごと1本の横スクロールに並べる実装だったため、開催日数が多いほど
    遡る操作が重くなっていた（JRA-VANは直近6件程度＋別画面という2段構え）。
- [x] 「遠い日付へのジャンプ手段」の実装方針をユーザーに確認し、「既存のPC版月カレンダーを
  流用（新規ページ・新規API無し）」を選択された。
- [x] スマホ見出しへ年を追加（`2026年7月26日（日）`）。
- [x] スマホの日付ストリップを全件表示から「選択日の前4件＋本人＋後1件（最大6件）」への
  絞り込みへ変更した（`STRIP_WINDOW_BEFORE=4`/`STRIP_WINDOW_AFTER=1`）。
- [x] ストリップ下に「他の日程を探す」トグルを追加し、タップで既存PC版月カレンダー
  （開催日ドット付き）をスマホでもインライン展開できるようにした。
- [x] 月カレンダーへ年ジャンプ（前年/翌年）を追加した。従来は月送りのみで、
  半年以上遡る操作自体が別途重かったため。
- [x] 新規3 tests（年表示・絞り込み件数・トグル既定閉＋年ジャンプ属性）を含むWeb 116 tests、
  typecheck、production buildが成功。Playwrightで390px（折りたたみ・展開の両状態）と
  実際のデスクトップサイドバー幅272pxで横はみ出し・年月ナビの折り返しが無いことを確認した。

## 2026-07-26 (3) 完了: Claude Code — 実端末ロケテストの準備確認

- [x] `apps/web/scripts/run_location_test_tunnel.ps1`を再読し、直近のモバイルUI変更
  （取り込み警告のモバイル要約化等）と内容面で衝突しないことを確認した。スクリプトは
  認証・トンネル・readinessのみを扱い、画面固有の文言依存は「レースボード」表示と
  既知エラー文言の不在確認のみのため、更新不要と判断した。
- [x] 対象コンポーネント（`RaceDateCalendar`・`MobileRaceGroupedSection`・
  `MobileRaceNavigation`・`MobileRaceForecastDashboard`・`MobilePaceAnalysisDashboard`）の
  ソースを確認し、`aria-current`・`role="tab"`・`aria-selected`・44px相当のタップ領域
  （`h-11`/`w-11`=44px、日付は`h-14 min-w-14`=56px）の実装状況を具体的に洗い出した。
- [x] タブ実装はアクティブな1パネルのみをDOMへ描画するため、非アクティブタブの
  `aria-controls`が現在DOM未生成のパネルIDを参照するという既知の制約を発見した。
  実害（読み上げ破綻）が無ければ現状維持でよいと判断し、修正はしていない
  （実機で問題が確認されてから対応する、という本プロジェクトの既存方針に合わせた）。
- [x] 上記を`docs/LOCATION_TEST.md`§10「開発者によるモバイル実機QA」として整理し、
  対象画面・表示幅（320/375/390/430px）・確認観点（横はみ出し・タップ領域・スクロール・
  キーボード操作・VoiceOver/TalkBack読み上げ）・既知の制約・記録方法をまとめた。
- [x] `tasks/current.md`の「P2 実端末ロケテスト」「P2 モバイルアクセシビリティ確認」から
  上記チェックリストへ参照を追加した（チェックボックス自体は未実施のまま維持）。
- **このセッションはコード変更なし**（ドキュメントのみ）。実行自体（実機での確認・
  友人からの指摘収集・VoiceOver/TalkBack確認）は引き続き開発者の実機が必要で未実施。

## 2026-07-26 完了: Claude Code — スマホ主要導線の通し確認（静的レンダリング範囲）

- [x] このクラウド環境ではDocker daemonを起動できず（`ulimit`権限制限）、実DB接続の
  開発サーバーでのクリック通し確認はできないと確認した。代替として、主要導線を構成する
  各モバイルコンポーネントを実在の型・現実的なモックデータ（既存`*.test.tsx`のfixtureを再利用・
  拡張）で`renderToStaticMarkup`し、ビルド済みTailwind CSSを適用した静的プレビューを
  Playwrightで390px幅にてスクリーンショット確認した（一時ファイルは確認後に削除）。
- [x] 確認した導線: レースボード（日付ストリップ→競馬場タブ→レース行）→同一開催ナビ→
  レース詳細（ヒーロー→サマリータブ：展開恩恵馬TOP3・評価を下げたい馬・一覧へ戻るリンク）→
  確定後分析（ヒーロー→サマリータブ：上位3頭・ひとこと振り返り）。
- [x] `document.documentElement.scrollWidth === clientWidth === 390`をPlaywrightで機械的に確認し、
  横はみ出しが無いことを裏付けた。目視でも全パネルで折り返し・はみ出しは無かった。
- [x] 確認中に2点、見かけ上の異常に気づき原因を特定した（いずれも自分のモックデータの
  不備で、実装側の不具合ではないと判明）:
  - レース行の展開ラベルが「判断材料が不足」と出た件 → `beginnerPaceLabel()`は
    「ハイ/平均/スロー」の3値しか認識せず、自分のモックが独自の説明文字列
    （「やや速い流れ」）を渡していたのが原因。正しい値に直すと想定どおり表示された。
  - 確定後分析の「実際の流れ」バッジに`H`、各馬結果に`M`が見えた件 →
    `lib/pace.ts`の`PACE_SPEED_META`が意図的に持つ短縮記号（`symbol: "H"`等）で、
    常にフルの日本語ラベル（「ハイ」等）と併記される仕様。内部実数値の露出ではない
    （`docs/PROJECT_RULES.md §5`は引き続き順守されている）。
- [x] **見つかった実装上の不具合は無し**。Web 113 tests（変更なし・確認作業のみ）。
- 未実施（実機・実DBが必要なため範囲外）: 実際のタブ切り替え操作（サマリー/隊列/注目馬/詳細）の
  クリック挙動、実スクロール、iOS/Android実機、キーボード操作・スクリーンリーダー。
  これらは`tasks/current.md`の「実端末ロケテスト」「モバイルアクセシビリティ確認」へ引き続き委ねる。

## 2026-07-26 完了: Claude Code — Codex引き継ぎ検証＋取り込み警告のモバイル要約化

- [x] 117コミット分のCodex作業を検証（`git pull --ff-only`は0 ahead/117 behindで安全に実施）。
  API 596/ingestion-worker 239/Web 111 tests、ruff/lint-imports/mypy --strict/typecheck/build
  すべてこのクラウド環境で再実行し、Codexの記載件数と完全一致を確認した。
- [x] 唯一の見かけ上の不一致（ingestion-worker mypyがこの環境で3件エラー）を調査し、
  `sys.platform != "win32"` 分岐に対するmypyのプラットフォーム推定差（`--platform win32`で
  0エラーに一致）と特定。コードの不具合ではないため対応不要と判断（`docs/HANDOFF.md`参照）。
- [x] 重大な不整合が無いことを確認したため、`tasks/current.md`最優先未完了タスク
  「P1 取り込み警告のモバイル要約化」から作業を再開した。
- [x] `IngestStatusBanner.tsx`へ768px未満専用の要約行（アイコン＋見出しのみ・1行・truncate）を
  追加した。768px以上は既存の見出し＋detail文の2段表示を維持する。
  最優先状態の選定は既存の`ingestStatusMeta()`の判定結果（1つに絞り込み済み）をそのまま使い、
  複数異常時の優先順位を独自に決めていない。
- [x] `IngestStatusBanner.test.tsx`に、モバイル要約行の内容（detail文を含まない）と
  PC表示が従来どおり（見出し＋detail文）であることを確認するテストを追加し、
  既存2件のテストは変更していない。
- [x] Playwrightで実際にレンダリングした静的プレビューを390px・1024pxでスクリーンショット確認し、
  390pxで1行に収まること・1024pxで従来の2段表示が保たれることを目視確認した。
- [x] Web 113 tests、型チェック、本番ビルドを完了した。

## 2026-07-26 完了: Claude Code向け引き継ぎ整理

- [x] ブランチ、最新コミット、Git差分、未追跡ファイルを確認した。
- [x] 直前の機能差分を自己レビューし、デバッグコードや一時ファイルがないことを確認した。
- [x] Web全テスト111件、型チェック、本番ビルドを再実行した。
- [x] Web lintを実行し、`lint`スクリプト未定義のため実行不可であることを記録した。
- [x] 実機確認用Cloudflare Quick Tunnelを停止した。
- [x] `docs/HANDOFF.md`をClaude Code向けに更新した。
- [x] 新しい設計判断がないため`docs/DECISIONS.md`は変更不要と判断した。

## 2026-07-26 完了: Codex向け引き継ぎ整理

- [x] `CLAUDE.md`の終了手順に従い、ブランチ、最新コミット、Git差分を確認した。
- [x] Web全テスト111件と型チェックを再実行した。
- [x] Web lintを実行し、`lint`スクリプト未定義のため実行不可であることを確認した。
- [x] `docs/HANDOFF.md`へ検証結果、暫定事項、既知の問題、再開手順を記録した。
- [x] 新しい設計判断がないため`docs/DECISIONS.md`は変更不要と判断した。

## スマホ画面向け改善の残タスク

### 確定タスク

- [x] **P1 取り込み警告のモバイル要約化**: 完了（上記2026-07-26セクション参照）。

### 検証タスク

- [x] **P2 スマホ主要導線の通し確認（静的レンダリングの範囲で実施済み・詳細下記）**。
- [ ] **P2 実端末ロケテスト**:
  iOS SafariとAndroid Chromeで320px・375px・390px・430px相当の横はみ出し、
  文字欠け、44px未満の操作対象、スクロール位置を確認し、友人からの指摘を記録する。
  実行前の準備（対象画面・チェック項目・実装済みの44px要素の一覧）は
  `docs/LOCATION_TEST.md`§10へ整理済み。実行自体は実機が必要なため未実施。
- [ ] **P2 モバイルアクセシビリティ実機確認**:
  詳細タブと競馬場タブのキーボード操作・フォーカス・`aria-selected`、日付ストリップの
  ナビゲーションランドマークは390pxの実ブラウザと自動テストで確認済み。
  日付ストリップ、競馬場タブ、展開領域の読み上げ順、および詳細タブの読み上げを
  VoiceOver/TalkBack実機で確認する。
  実行前の準備（VoiceOver/TalkBackでの確認項目・既知の制約の洗い出し）は
  `docs/LOCATION_TEST.md`§10へ整理済み。実行自体は実機が必要なため未実施。

### 条件付きタスク

- [x] **P3 日付ストリップのDOM削減**:
  選択日の前4開催・後1開催だけを表示し、離れた日付は月カレンダーから探す構成を実装済み。
- [x] **P2 月カレンダーのタップ領域改善**:
  「他の日程を探す」、年月移動、開催日の操作領域を44px以上へ統一した。

## 2026-07-26 完了: スマホ開催日カレンダーを日付ストリップへ圧縮

- [x] 768px未満は、レースが存在する開催日だけを横スクロールの日付ストリップに表示した。
- [x] 選択日を中央へ自動スクロールし、曜日と月日を一目で確認できるようにした。
- [x] 日付切替時も`performance_days`を維持し、選択日のレース一覧へ遷移できるようにした。
- [x] 768px以上は従来の月間カレンダーを維持した。
- [x] 390px相当でカレンダー高さ149px、最初のレース一覧位置845px、横はみ出しなしを確認した。
- [x] 日付切替、Web 111 tests、型チェック、本番ビルドを完了した。

## 2026-07-26 完了: スマホのレースボード上部を圧縮

- [x] 4つの統計カードを、スマホでは高さ74pxの4列サマリーへ統合した。
- [x] 重複していたスマホのレース一覧フィルターを非表示にし、統計サマリーへ一本化した。
- [x] 予想検証パネルを、初期高さ66pxの折りたたみサマリーへ変更した。
- [x] 展開時は期間切替、検証カバー率、全体・芝・ダート、信頼度別集計など既存内容を維持した。
- [x] 768px以上は従来の4統計カードと予想検証パネルを維持した。
- [x] 390px相当で横はみ出しなし、詳細展開、PC表示の維持を確認した。
- [x] Web 109 tests、型チェック、本番ビルドを完了した。

## 2026-07-26 完了: スマホのレース一覧を競馬場タブへ圧縮

- [x] 768px未満は開催日ごとに競馬場タブを表示し、選択中の競馬場だけを一覧表示するようにした。
- [x] 各レースをレース番号、レース名、条件、頭数、展開ラベル中心のコンパクト行へ変更した。
- [x] 注目・妙味ラベルを残し、推奨理由などの詳細情報はレース詳細画面へ集約した。
- [x] 768px以上は従来の3開催場横並びと詳細カードを維持した。
- [x] 390px相当で3競馬場のタブ切替、選択開催場12R、横はみ出しなしを確認した。
- [x] Web 107 tests、型チェック、本番ビルドを完了した。

## 2026-07-25 完了: スマホ隊列予想を4列ボードへ圧縮

- [x] 先頭・好位・中団・後方の4ゾーンを、横スワイプではなく同時表示するボードへ変更した。
- [x] 初期状態は馬番と馬名だけを表示し、脚質・信頼度・配置理由は選んだ1頭だけ展開する。
- [x] 枠色と公式馬番を維持し、18頭の配置を横はみ出しなしで一覧できるようにした。
- [x] PC版は従来の馬カード・信頼度・理由を含む4列表示を維持した。
- [x] 390px相当の新潟11Rでボード高454px、ページ高990px、詳細1件展開を確認した。
- [x] Web 105 tests、型チェック、本番ビルドを完了した。

## 2026-07-25 完了: 確定後分析をスマホ向けに再構成

- [x] 768px未満を「サマリー・振り返り・全馬」の3タブへ分割した。
- [x] 初期表示を実際の流れ、予想との答え合わせ、上位3頭、一言解説へ絞った。
- [x] 全馬結果を横スクロール表から二段のコンパクト行へ変更した。
- [x] 解説全文と算出根拠は振り返りタブへ移し、情報を失わず初期画面を短縮した。
- [x] PCI・RPCIの内部実数値を追加表示せず、既存の言語ラベルだけを使用した。
- [x] 390px相当で3タブ、全馬18頭、横はみ出しなし、PC版の既存表維持を確認した。
- [x] Web 103 tests、型チェック、本番ビルドを完了した。

## 2026-07-25 完了: スマホに同一開催レースナビを追加

- [x] 出走前・確定後の両詳細画面へ、前R・1R〜12R・次Rのモバイルナビを追加した。
- [x] 同日・同競馬場の一覧API応答から実在するレースキーだけを使い、欠番を推測しない。
- [x] レース状態に応じて出走前は予想、確定後は分析画面へ遷移する。
- [x] 一覧APIだけが失敗した場合はナビを省略し、取得済みの詳細画面は表示を続ける。
- [x] 390px相当で現在Rの中央表示、44pxの前後ボタン、ページ横はみ出しなしを確認した。
- [x] Web 101 tests、型チェック、本番ビルドを完了した。

## 2026-07-25 完了: スマホ注目馬を選択式の詳細表示へ変更

- [x] 注目馬タブの展開恩恵馬TOP5を、高さ64px基準のコンパクト行へ統一した。
- [x] 評価を下げたい馬も同じ操作へ揃え、選んだ馬だけ評価理由を展開するようにした。
- [x] 馬番、脚質、適性評価、役割ラベルは閉じた状態でも確認可能にした。
- [x] 390px実画面で8行すべて閉じた初期高660px、1頭だけの理由展開、横はみ出しなしを確認した。
- [x] Web 97 tests、型チェック、本番ビルドを完了した。

## 2026-07-25 完了: レース詳細のスマホ向け情報階層を再設計

- [x] 768px未満のレース詳細を「サマリー・隊列・注目馬・詳細」の4タブへ分割した。
- [x] モバイルHeroをレース条件・想定展開・信頼度へ圧縮し、初期表示を恩恵馬TOP3と注意馬1頭に絞った。
- [x] 枠順未確定時は暫定番号を公式馬番として表示せず、既存の`horseNumberLabel`契約を維持した。
- [x] 隊列4ゾーンをモバイルでは横スワイプ、デスクトップでは従来の4列表示とした。
- [x] 390px実画面で初期ページ高936px、ページ横はみ出しなし、4タブ切替を確認した。
- [x] 1440pxでは既存デスクトップ詳細だけが表示され、横はみ出しがないことを確認した。
- [x] Web 96 tests、型チェック、本番ビルドを完了した。

## 2026-07-25 完了: 初回ロケテスト公開方式の決定と起動自動化

- [x] 実DB容量が約121MBであることを確認し、無料公開基盤の制約と比較した。
- [x] 3〜5人・開催2週分は、ローカルAPI/DBを維持してNext.jsだけを
  Cloudflare Quick Tunnelへ公開する方針に決定した。
- [x] Git管理外の認証設定例と、API/DB・ビルド・401/200・API疎通を確認する
  `run_location_test_tunnel.ps1`を追加した。
- [x] Quick Tunnelを期間限定テスト専用とし、正式公開時はマネージド構成へ移行する境界を記録した。
- [x] Windows実行機へ署名有効な`cloudflared 2026.7.3`を導入し、実URLの
  未認証401・共有認証200・API疎通と代表3レースを確認した。
  - 芝短距離: 2026-07-25 新潟11R、芝1000m、18頭
  - ダート中距離: 2026-07-25 新潟10R、ダート1800m、15頭
  - 枠順確定後の多頭数: 2026-07-25 新潟7R 新潟日報賞、芝1400m、18頭
  - 確認後に一時公開を停止し、検証用共有パスワードを新しい未使用値へ交換した。
  - 通常の`next dev`と本番`.next`の競合を実地で検出し、ロケテスト出力を
    `.next-location-test`へ分離した。分離後の本番起動とトップ200を確認した。

## 2026-07-25 完了: ロケテスト前の品質・情報階層改善

- [x] ダートRPCI v4の期間外監視を実DBで実行し、採用後データがまだ0件であることを確認した。
- [x] 予想検証APIの対象787レース・照合0件を確認し、係数を変更せず蓄積継続と判断した。
- [x] ingestion-worker全体のRuff・mypy違反を解消し、239 testsを通過させた。
- [x] APIの開発用`seed_dev.py`に残っていたRuff 10件を解消し、API全体Ruffを成功させた。
- [x] レース詳細の判断サマリーを統合順位基準へ統一し、展開適性単独順位との矛盾を解消した。
- [x] 冒頭へ総合上位3頭・注意馬・展開信頼度を集約し、統合順位6位以下を折りたたんだ。
- [x] デスクトップ1440px・モバイル390pxで横はみ出しがないことを実画面確認した。
- [x] 少人数ロケテストの開始条件、停止条件、感想収集項目を`docs/LOCATION_TEST.md`へ整理した。

## 2026-07-25 完了: 少人数ロケテスト用アクセス制限

- [x] Next.js全画面へ任意有効の共有Basic認証を追加した。
- [x] 認証環境変数が片方だけの場合は503で閉じ、設定漏れを無認証公開にしない。
- [x] FastAPIの`/api/v1/*`へ任意有効のBearer認証を追加した。
- [x] api-clientとNext.jsからサーバー間トークンを送信し、参加者へAPIトークンを渡さない構成にした。
- [x] `/health`・`/ready`と`INGEST_TOKEN`認証を既存どおり分離した。
- [x] 単体・契約テストと、401/200を含むHTTP実地確認を完了した。

## 2026-07-25 完了: ロケテスト公開前HTTP点検の自動化

- [x] 公開Web/APIのURLと秘密値を環境変数だけから受け取る点検CLIを追加した。
- [x] WebのBasic認証、APIのBearer認証、readiness、認証済みレース一覧を一括確認する。
- [x] 認証済みトップ画面にレースボードがあり、APIエラー表示がないことを確認する。
- [x] 公開HTTP、短い秘密値、秘密値の使い回し、設定不足を開始前に拒否する。
- [x] HTTPモック8 tests、Web 95 tests、API認証契約5 tests、型チェック、本番ビルドを完了した。

## 次の実施条件つきタスク

- [x] 事前予想63件で、全体・芝・ダートの不一致傾向を初回レビューした。
- [ ] 2026-08-03以降のダート確定100件かつ実績ハイ・平均・スロー各20件に到達したら、
  RPCI v4期間外レビューを実行する。
- [x] `.env.location-test.local`を設定し、Quick Tunnelの実URLで代表3レースを目視確認する。
- [ ] 修正前ログに記録された可能性があるSlack Webhookを再発行し、ローカル`.env`を更新する。

## 2026-07-25 完了: Webhook通知を安全に集約しWindowsで実地確認

- [x] ラッパー配下のPython通知を抑え、全リトライ失敗後の1回だけ通知するようにした。
- [x] httpxのリクエストログと例外から、認証情報を含むWebhook URLを秘匿した。
- [x] HTTPエラーを通知成功として扱わず、TLS 1.2と通常の証明書検証を維持した。
- [x] 取り込みを伴わない`run_batch.ps1 -TestNotification`を追加した。
- [x] Windows実行機からSlackへのテスト通知と、ログへのURL非出力を確認した。

## 2026-07-25 完了: Windows取り込みログの文字化けを解消

- [x] Windows PowerShell、Python、保存ログの文字コードをUTF-8へ統一した。
- [x] `Tee-Object`の既定エンコーディングに依存せず、表示とUTF-8追記を明示的に分離した。
- [x] `sync_mykeibadb.bat`でもコードページ65001とPython UTF-8モードを設定した。
- [x] 実環境の`run_batch.ps1`で予想72件を生成し、コンソールと保存ログの日本語を確認した。

## 2026-07-25 完了: Windows全同期にAPI・DB事前確認を追加

- [x] 全同期開始時に`API_BASE_URL/ready`を確認し、FastAPIとPostgreSQLが利用可能な場合だけ続行する。
- [x] 利用不能時は`mykeibadb.exe`起動前に終了コード1で停止し、具体的な復旧手順を表示する。
- [x] データ更新を伴わない`-PreflightOnly`を追加した。
- [x] 正常系、接続不能系、PowerShell構文解析を確認した。

## 2026-07-25 完了: Windowsでの予想事前生成500を修正

- [x] 同期時の出馬表500はAPI停止ではなく、Docker Desktop停止によりPostgreSQLへ接続できなかったことを特定した。
- [x] Docker Desktop・DBを起動し、Alembic head、`/ready`、レース一覧APIの正常を確認した。
- [x] 出馬表144レース、馬場情報72件、確定成績69レース、特別登録を再同期した。
- [x] 予想事前生成500はWindows環境の`tzdata`欠損による`ZoneInfoNotFoundError`と特定した。
- [x] 予想事前生成の日付判定を、既存機能と同じ外部データ不要のJST固定オフセットへ統一した。
- [x] JSTオフセットの回帰テストを追加した。
- [x] 回帰テスト4件、対象Ruff、mypy strict、実環境の予想72件生成を完了した。

## 2026-07-25 完了: 取り込み警告の詳細表示を集約

- [x] `IngestStatusBanner`の複数の開閉領域を「詳細と復旧手順」1つへ集約し、初期表示の高さを抑えた。
- [x] 失敗、成績未取込、馬場情報未反映、重複レース、再同期、馬場補完の全情報とリンクを維持した。
- [x] 警告時は閉じた状態、正常時は不要な開閉UIを表示しないことをコンポーネントテストで固定した。
- [x] VitestでTSXと`@`エイリアスを扱えるようにし、Web 83 tests、型チェック、本番ビルドを完了した。
- [x] 通常クローンとテスト用API中継を使い、1440px・390pxで警告の初期表示と展開後を実ブラウザ確認した。
- [x] モバイルでエラー文が1文字だけ折り返される表示を`break-words`へ修正し、横はみ出しなし・コンソールエラーなしを確認した。

## 2026-07-24 完了: readinessの文字列列長互換性検査

- [x] ORM必須テーブル・列の存在確認に加え、長さ付き文字列列の実DB長を検査するようにした。
- [x] 実DBがORMの必要長より短い場合は`schema_outdated`を返し、既存のmigration復旧案内へ接続した。
- [x] `TEXT`など長さ無制限の文字列型は互換として扱い、不要な不一致判定を避けた。
- [x] migration 006未適用相当の`predicted_pace.model_version VARCHAR(20)`を単体・PostgreSQL統合で検出した。
- [x] 既存の`/ready`レスポンスとWebエラー表示の契約は変更せず、後方互換を維持した。
- [x] 関連9 tests、PostgreSQL統合3 tests、Ruff、mypy strictを完了した。

## 2026-07-24 完了: ダートRPCI v4期間外監視・再学習レビュー条件

- [x] 採用後のダート確定レースを対象にする`--monitor-dirt-v4`をバックテストCLIへ追加した。
- [x] 全体100件・ハイ・平均・スロー各20件が集まるまでは品質判定を行わず、蓄積中として扱う。
- [x] 判定開始後はMAE、展開一致率、ハイ再現率、絶対バイアスの4条件を機械判定する。
- [x] 条件未達時は本番モデルを自動変更せず、再学習候補と現行モデルの比較開始だけを要求する。
- [x] JSON出力へ監視結果を含め、`--fail-on-monitoring-review`で運用ジョブから検知可能にした。
- [x] 採用時の独立期間197件で`healthy`、採用翌日以降は現時点で`no_data`と確認した。
- [x] v4モデル名がmart列の20文字制限を超える不整合を検出し、migration 006で64文字へ拡張した。
- [x] API非統合588 tests、mart統合4 tests、Ruff、mypy strictを完了した。

## 2026-07-24 完了: RPCI v4前後半3F履歴特徴量・ダートモデル採用

- [x] v3の33特徴量を維持し、対象日より前の各馬最大10走から前後半3F差を集約する6特徴量を追加した。
- [x] 学習SQLとオンライン予測の両方で、履歴保有馬数・標本数・平均差・最小差・幅・カバー率を同じ順序で生成した。
- [x] LightGBMモデルの特徴量数からv1/v2/v3/v4を自動判別し、芝・ダートで異なる世代を同時利用可能にした。
- [x] `--before-date`を追加し、学習期間と独立評価期間を完全分離できるようにした。
- [x] 2026-06-01以降を除外して候補を学習し、同日以降の芝200件・ダート197件で本番v1と比較した。
- [x] 芝v4は展開一致率が62.5%から56.0%へ低下したため不採用とし、本番芝v1を維持した。
- [x] ダートv4はMAE`12.810→4.750`、展開一致率`11.2%→72.6%`、
  ハイ再現率`0%→90.6%`、最上位帯リフト`1.01x→1.19x`へ改善したため採用した。
- [x] 関連90 tests、API非統合577 tests、変更対象Ruff、mypy strict、実モデルスモークテストを完了した。

## 2026-07-24 完了: レース前後半3Fの内部永続化

- [x] `races`へ内部分析用の`race_s3f`・`race_l3f`を追加し、公開Race DTOや画面には露出させない構成にした。
- [x] Alembic migration `005`、Repository保存・復元、結果取り込み、再取り込み時の値保持を実装した。
- [x] wmykeibadbの3F列が1/10秒整数である場合に二重変換される不具合を修正し、秒形式との両方を正規化した。
- [x] 実行用DBをmigration `005`へ更新し、mykeibadb直近1年のresultsを再同期した。
- [x] API非統合570 tests、API統合19 tests、ingestion-worker全237 tests、変更対象Ruff、API mypy strictを完了した。
- [x] ingestion-worker全体Ruff/mypyの既存エラーは今回の対象外として記録した。

## 2026-07-24 完了: mykeibadbラップ利用率診断

- [x] 確定済みJRA平地をレースキーで重複排除し、芝・ダート・距離帯別にラップ保有率を集計するCLIを追加した。
- [x] 前半3F、後半3F、両方、区間ラップの一部・完全取得を分離して診断できるようにした。
- [x] 最低レース数と最低カバー率をCLI引数にし、未確定の採用基準を固定仕様化しない構成にした。
- [x] 集計JSONには件数と割合だけを保存し、生ラップ値・接続情報を含めないようにした。
- [x] 実mykeibadb直近1年を診断し、主要距離帯のS3+L3取得率が芝・ダートとも100%であることを確認した。
- [x] 区間ラップはダート1400-1799mが66.7%、その他の主要帯は86.3-100%であることを確認した。
- [x] 新規7 tests、ingestion-worker全237 tests、変更対象Ruff、変更対象mypy strictを完了した。

## 2026-07-24 完了: RPCI v3過去前付けペース特徴量・独立比較

- [x] 全出走馬について、対象レースより前に前で運んだ最大10走のPCI傾向を予測コンテキストへ追加した。
- [x] 当該レースの確定脚質や結果を使わず、1角2番手以内を優先して前付け実績を判定する。
- [x] v2の27特徴量を維持し、履歴保有馬数・標本数・平均・最小・幅・カバー率の6特徴量をv3へ追加した。
- [x] LightGBMモデルの特徴量数からv1/v2/v3を自動判別し、既存モデルとの互換性を維持した。
- [x] 学習SQLを`pr.race_date < r.race_date`へ限定し、未来情報の混入を防止した。
- [x] 芝・ダート各200レースで独立比較し、重要区分の再現率悪化・欠落により候補モデルを不採用とした。
- [x] 候補モデル3ファイルを削除し、本番v1モデルを維持した。
- [x] 対象83 tests、API非統合569 tests、変更対象Ruff、mypy strictを完了した。

## 2026-07-24 完了: RPCI v2特徴量基盤・独立比較

- [x] v1の8特徴量を維持し、頭数・逃げ比率・前方頭数・自在比率・逃げ競合をv2へ追加した。
- [x] 距離4帯とJRA10場のone-hotを追加し、競馬場コードの順序値だけに依存しない構成にした。
- [x] LightGBMモデルの特徴量数からv1/v2を自動判別し、芝・ダートで異なる世代も同時利用可能にした。
- [x] 未対応の特徴量数をロード時に拒否し、v2候補は専用モデル世代として記録する。
- [x] `--feature-set v2`は明示的な`--output`を必須とし、本番モデルの誤上書きを防止した。
- [x] 芝・ダート各200レースで比較し、PAI順位指標は改善したが展開区分の悪化により候補モデルを不採用とした。
- [x] 候補モデル2ファイルを削除し、本番v1モデルを維持した。
- [x] 対象49 tests、API非統合563 tests、変更対象Ruff、mypy strictを完了した。

## 2026-07-24 完了: RPCI少数展開区分の加重学習・独立比較

- [x] `none`・`sqrt-inverse`・`inverse`のラベル重みプロファイルを追加した。
- [x] 芝・ダート別の展開3区分を使い、平均重み1.0へ正規化して既存回帰モデルへ適用した。
- [x] 加重学習は明示的な`--output`を必須とし、本番モデルの誤上書きを防止した。
- [x] 芝・ダートの候補を検証セットと直近200レースのas-ofバックテストで比較した。
- [x] 芝は平均再現率とMAEが改善したがハイ・総合一致率が悪化し、ダートは独立期間のハイ再現率0%のため不採用とした。
- [x] 候補モデル4ファイルを削除し、本番モデルと既定学習プロファイルを維持した。
- [x] 学習テスト8件、API非統合553件、変更対象Ruff、mypy strictを完了した。

## 2026-07-24 完了: RPCI学習データの脚質特徴量・時系列検証を修正

- [x] 実DBの`running_style`が正規表記（逃げ・先行・差し・追込・自在）であることを確認した。
- [x] 学習SQLの短縮表記（逃・先・差・追）を正規表記へ直し、常にゼロだった脚質特徴量を復旧した。
- [x] 最新20%を検証、古い80%を学習に使う時系列分割へ修正し、未来データ参照を防いだ。
- [x] 学習時に芝・ダート別の展開3分類再現率を表示するようにした。
- [x] バックテストへ任意の芝・ダートモデルパスを追加し、本番モデルを置換せず比較可能にした。
- [x] 候補モデルを実DBで比較し、芝の分類悪化とダートのハイ再現率0%を理由に本番採用を見送った。
- [x] API非統合548 tests、変更対象Ruff、mypy strictを完了した。

## 2026-07-24 完了: PAIの距離適性・道悪弱点を実履歴へ接続

- [x] 予想日より前の同一馬場種別の確定成績から、距離適性と道悪傾向を保守的に生成する。
- [x] 距離は好走2件以上、今回に最も近い好走距離を採用し、前後200m以内は適合として扱う。
- [x] 道悪弱点は今回距離の前後400m、良・道悪各3件以上、頭数補正着順差0.30以上に限定する。
- [x] 1回の予想内で最大20走の履歴を共用し、脚質・前付け・ペース相性・能力・コース適性の重複照会を除いた。
- [x] PAIモデル世代を`pai-v2`へ更新し、内部数値を出さない既存説明を維持した。
- [x] 2025年後半424頭・2026年前半413頭で実DB診断し、導入前と同水準の相関・上位帯リフトを確認した。
- [x] 対象54 tests、API非統合545 tests、PostgreSQL統合6 tests、Ruff、mypy strictを完了した。

## 2026-07-23 完了: PaiWeights候補の実DB比較基盤

- [x] `--compare-pai-weights`を追加し、現行・RPCI差減点2候補・脚質好ペース幅2候補を比較可能にした。
- [x] 同一レース・同一馬を検証し、全体・芝・ダート別のPAI相関と最上位帯リフトをCLI/JSONへ出力する。
- [x] 2025年後半30件・424頭と2026年前半30件・413頭で実DB診断を行った。
- [x] 全指標を両期間で安定改善する候補がなかったため、本番`PaiWeights`は現行値を維持した。
- [x] 距離適性・道悪弱点が現行プロファイル生成へ未接続であることを別タスクへ分離した。
- [x] 関連38 tests、API非統合536 tests、Ruff、mypy strictを完了した。

## 2026-07-23 完了: TestClientをhttpx2へ移行

- [x] APIの開発用依存へ`httpx2>=2.7,<3`を追加し、Starlette TestClientの正式移行経路を有効化した。
- [x] Gemini通信などアプリ本体は従来の`httpx`を維持し、変更範囲をテスト環境へ限定した。
- [x] API契約90 tests、非統合533 tests、PostgreSQL API統合6 tests、Ruff、mypy strictを完了した。
- [x] TestClient非推奨警告を解消し、残る警告をCodexワークスペースのpytestキャッシュ権限だけにした。

## 2026-07-23 完了: 自コード由来の非推奨警告を解消

- [x] FastAPIルーターのHTTP 422定数を`HTTP_422_UNPROCESSABLE_CONTENT`へ更新した。
- [x] Alembicへ`path_separator = os`を追加し、旧区切り動作の非推奨警告を解消した。
- [x] API非統合533 tests、ingest契約42 tests、PostgreSQL統合2 tests、Ruff、mypy strictを完了した。
- [x] 全体警告を5件から2件へ削減した。TestClient通知は次のタスクで解消済み。

## 2026-07-23 完了: Alembic実行によるロガー状態漏れの解消

- [x] `test_database_readiness.py`へ統合テストマーカーを追加し、通常テストへのDocker/Alembic混入を防止した。
- [x] Alembicの`fileConfig`で`disable_existing_loggers=False`を指定し、同一プロセスの既存ロガーを保持する。
- [x] マイグレーション後もアプリロガーが有効であることをPostgreSQL統合テストへ追加した。
- [x] API非統合533 tests、PostgreSQL統合2 tests、Ruff、mypy strictを完了した。

## 2026-07-23 完了: RuleWeights候補の実DB比較基盤

- [x] `--compare-rule-weights`を追加し、現行・脚質影響2候補・前付け実績混合2候補を同一レースで比較可能にした。
- [x] 全体・芝・ダート別のMAEと展開分類一致率、現行差をCLIとJSONへ出力する。
- [x] 候補間で対象レースが一致しない場合は比較を拒否し、本番`DEFAULT_WEIGHTS`は自動変更しない。
- [x] 2025年後半50件と2026年前半30件で実DB診断を行った。
- [x] `evidence-heavy`は両期間でMAEが改善したが、2026年ダートの分類一致率が22.2ポイント低下したため不採用とした。
- [x] 関連35 tests、Ruff、mypy strictを完了した。API全体は531 passed・3 failedで、
  失敗3件は既存のログ捕捉テストの順序依存（単独再実行3 passed）。

## 2026-07-23 完了: 予想検証の専用画面

- [x] `GET /api/v1/forecast-performance/misses`を追加し、30/90/180日、芝/ダート、予想・実績区分で検索可能にした。
- [x] 25件単位のページングと最大100件のAPI上限を設け、PCI/RPCI実数値と生の信頼度は公開しない。
- [x] `/forecast-review`を追加し、URLに絞り込み状態を保持して確定後分析へ遷移可能にした。
- [x] トップの直近不一致一覧と共通ヘッダーから専用画面へ移動できるようにした。
- [x] API 21 tests、Web 81 tests、Ruff、mypy strict、型チェック、production buildを完了した。

## 2026-07-23 完了: 予想検証から不一致レースを確認できる導線

- [x] 選択期間内で予想区分と実績区分が異なる直近5レースをAPIへ追加した。
- [x] 開催日・競馬場・R番号・コース・レース名・「予想→実際」を表示し、確定後分析へ遷移可能にした。
- [x] PCI/RPCI実数値と生の信頼度は公開せず、表示ロジックと内部計算を分離した。
- [x] API単体・契約・PostgreSQL統合、Ruff、mypy strict、Web 76 tests、型チェック、buildを完了した。

## 2026-07-23 完了: 重複レース450組の正規キー統合完了

- [x] 予想martを持つ旧キーは、正規側の同一モデル世代・全頭分カバレッジを検証する。
- [x] 残存していた2026-02-01東京9Rを正規キーへ再同期し、旧キーを削除した。
- [x] 統合後も正規キーの12頭、`predicted_pace` 1件、`pace_fit` 12件を維持した。
- [x] 直近1年の重複レース組数が0件になったことを実DBで確認した。

## 2026-07-23 完了: 重複レース449組の安全な正規キー統合

- [x] 正規キーの出走表・成績登録成功後だけ旧キーを削除する順序へ修正した。
- [x] 旧キー監査署名、正規頭数・確定頭数、中核成績一致、旧martゼロをAPIで再検証する。
- [x] 専用CLIはdry-runを既定とし、適用時は確認済み対象組数の指定を必須にした。
- [x] 実DBで449組を再同期し、旧キー449件を失敗0件で削除した。
- [x] 重複450組から1組へ削減。予想martを持つ東京9Rは安全側で保留した。

## 2026-07-23 完了: 重複レース統合前のdry-run監査

- [x] 認証付き`GET /internal/ingest/duplicate-race-audit`で、重複キーごとの状態・頭数・
  確定頭数・出走馬署名・中核成績署名・予想mart件数を読み取り専用で取得可能にした。
- [x] `python -m ingestion.audit_duplicate_races`でmykeibadbの`RACE_CODE`を正規キー候補とし、
  5区分へ分類するJSON監査CLIを追加した。
- [x] 2025-07-23〜2026-07-23を実DBで監査し、450組すべてで正規キーを一意に特定した。
- [x] 450組すべてが中核成績一致・旧キー側予想martなしの`removable_after_resync`だった。
- [x] 450組すべてで出走馬構成差があるため、正規キー再同期前の旧キー削除は禁止する。

## 2026-07-23 完了: 重複レースのデータ完全性監視

- [x] 直近365日のJRA平地について、同一開催日・競馬場・R番号に複数キーがあるレース組をDB集計する。
- [x] `/api/v1/ingest-status`へ重複組数と代表20組を追加し、Web警告バナーで各キーを確認可能にする。
- [x] 実DBで450組を検出し、監視が実データを捉えることを確認した。
- [x] FKを伴う誤削除を防ぐため、自動削除・自動統合は実装しない。

## 2026-07-23 完了: 小倉芝1200mの馬場状態別再検証

- [x] 確定値診断へ `distance-track-condition` 内訳を追加し、距離と馬場状態の交差条件を同時比較できるようにした。
- [x] 実DBの2025-07-01〜2026-07-31、小倉芝199レース・1534頭を再検証した。
- [x] 小倉芝1200mは良-22.4pt（45R）、稍重-10.5pt（12R）、重-9.0pt（6R）で、確認できた全馬場状態で逆転傾向が継続した。
- [x] `style-advantage-v3`の「7月・小倉・芝・1200m」を`reference`とする条件と仮係数は変更しない。
- [x] 参考表示の理由へ、馬場状態別でも同じ傾向であることを追記した。

## 2026-07-23 完了: 検証サマリー余白・馬場情報補完

- [x] 「事前の展開想定と実際の流れ」に `px-4 sm:px-6` を追加し、左右の見切れを解消した。
- [x] mykeibadb の `race-metadata` を定期フル同期と `--step all --mode mykeibadb` に組み込んだ。
- [x] コース種別・馬場状態・天候を既存レースへ補完できるようにした。
- [x] 旧形式レースキーと正規キーが併存する場合、日付・競馬場・R番号が一致する重複レースへ同じ補足情報を反映するようにした。
- [x] TrackCD の正式区分を mykeibadb の `track_code` マスタに合わせ、芝=10〜22、ダート=23〜29、障害=51〜59へ修正した。
- [x] 2025-07-23〜2026-07-23を再補完し、馬場情報未反映を564件から0件へ削減した。
- [x] mykeibadbに存在しない開発用シード `2026061805010101` をローカルDBから削除した。

現在の進行中タスク: なし。

> 進行中・直近着手のタスクをチェックボックスで管理する。着手/完了のたびに更新する。
> 状態: ⬜未着手 / 🔄進行中 / ✅完了 / ⏸保留。優先度: P0(必須) / P1(高) / P2(中) / P3(低)。
> 単なる改善案・未着手の候補は `tasks/backlog.md` に置く。

最終更新: 2026-07-24（RPCI v2特徴量基盤・独立比較） / 担当: OpenAI Codex / ブランチ `claude/sweet-einstein-ilnaov`

詳しい状態は `docs/HANDOFF.md` を参照（このファイルはタスクの一覧管理に専念する）。

---

## 進行中

なし。

---

## 最近完了したタスク

- [x] ✅ **P2 予想検証を直前の同期間と比較する**
  - 選択中の30日・90日・180日の直前にある同じ日数を、重複しない比較期間として集計する。
  - 全体・芝・ダートについて前期間の一致率と母数を公開APIへ追加し、内部PCI/RPCI値は返さない。
  - Webトップの各指標へ前期比をポイント差・アイコン・色で表示し、比較元0件は「前期比較なし」とする。
  - 任意の良否閾値や自動評価は導入しない。
  - 検証: API対象11 passed、OpenAPIスナップショット2 passed、Web 74 passed、
    API Ruff・mypy strict、api-client/Web typecheck、Web build成功。First Load JSは110KBを維持。
- [x] ✅ **P2 予想検証の集計期間を30日・90日・180日で切り替える**
  - `GET /api/v1/forecast-performance?days=`へ許容期間を追加し、未指定時は従来どおり90日とした。
  - 一致率・信頼度別集計・外れ方向・カバー率は選択期間で再集計し、週次グラフは比較軸を揃えるため
    期間にかかわらず直近8完了週を維持する。
  - Webトップへセグメント切り替えを追加し、期間変更と開催日変更の双方でURL状態を保持する。
  - 検証: API対象11 passed、OpenAPIスナップショット2 passed、Web 74 passed、
    API Ruff・mypy strict、api-client/Web typecheck、Web build成功。First Load JSは110KBを維持。
- [x] ✅ **P1 事前予想を照合できた母集団カバー率を表示する**
  - 直近90日の確定済みJRA平地かつ実績展開を判定できるレース総数を、予想保存とは独立に集計する。
  - 照合済み件数／評価対象総数とカバー率を公開APIへ追加し、一致率の母集団偏りを判断可能にした。
  - Webトップでは完全なら緑、未保存が残る場合は黄の進捗バーで表示する。任意の品質閾値は設けない。
  - 検証: 新規API単体・契約7 passed、PostgreSQL統合1 passed、Web 74 passed、
    API Ruff・mypy strict、api-client/Web typecheck、Web build成功。First Load JSは110KBを維持。
- [x] ✅ **P2 予想と実績の展開3区分を比較し、外れ方の方向を可視化する**
  - 速い・平均・落ち着くの予想3区分を行、実績3区分を列とする混同行列をAPIへ追加した。
  - 各セルは件数と予想区分内割合だけを返し、PCI/RPCIの内部実数値は公開しない。
  - Webトップでは「外れ方の傾向」として既定で閉じ、一致セルと不一致セルを色分けする。
  - 検証: 新規API単体・契約7 passed、Web 74 passed、API Ruff・mypy strict、
    api-client/Web typecheck、Web build成功。First Load JSは110KBを維持。
- [x] ✅ **P2 表示上の信頼度区分ごとに展開予想一致率を検証する**
  - 既存UIと同じ「読みやすい（70%以上）」「標準（50%以上）」「変動注意（50%未満）」で集計する。
  - 各区分の一致率・的中数・母数を公開APIへ追加し、内部実数値は公開しない。
  - Webトップへ3本の比較バーを追加し、週次推移とデスクトップ2列・モバイル縦並びで表示する。
  - 検証: 新規API単体・契約7 passed、Web 74 passed、API Ruff・mypy strict、
    api-client/Web typecheck、Web build成功。First Load JSは110KBを維持。
- [x] ✅ **P2 展開予想一致率を直近8完了週の推移で可視化する**
  - 進行中の週を除外し、月曜から日曜までの完了週を8区間集計する。
  - 各週の一致率・的中数・母数を公開APIへ追加し、PCI/RPCI内部実数値は公開しない。
  - WebトップへRecharts棒グラフと最新週の母数を追加。グラフは遅延読み込みし、
    一覧の初期JSを110KBに維持した。
  - 検証: 新規API単体・契約7 passed、Web 74 passed、API Ruff・mypy strict、
    api-client/Web typecheck、Web build成功。
- [x] ✅ **P1 保存済み事前予想の展開一致率をレース一覧へ表示する**
  - 直近90日の確定済みJRA平地を対象に、レースごとの最新の有効な事前予想と実際の展開区分を比較する。
  - 全体・芝・ダートの一致率と検証レース数を`GET /api/v1/forecast-performance`で返す。
  - Webトップへ期間・母数付きの検証サマリーを追加し、PCI/RPCIの内部実数値は公開しない。
  - 同日内のレース終了後生成は時刻情報がないため完全には除外できず、運用上は事前生成を前提とする。
  - 検証: 新規API単体・契約7 passed、PostgreSQL統合1 passed、Web 74 passed、
    API Ruff・mypy strict、api-client/Web typecheck、Web build成功。
- [x] ✅ **P2 馬場状態バックフィルの完了判定をデータ完全性監視へ統合する**
  - 直近365日の確定済みJRA平地で`track_condition`欠損をDB集計し、代表20件を返す。
  - Webトップで成績未取込と区別して警告し、専用`race-metadata`コマンドをコピー可能にした。
  - `run_batch.ps1`へ`-ChunkDays`を追加し、1年分を7日単位で安定して処理できるようにした。
  - 実DBへのバックフィル実行と馬場別再検証は引き続きWindows実行機で行う。
- [x] ✅ **P2 mykeibadbの馬場状態・天候を永続化し、既存レースを安全にバックフィル可能にする**
  - `race_shosai`の芝/ダート馬場状態・天候コードを、固定長位置を介さない補足情報として読む。
  - 通常のentries/results同期へ自動反映し、既存レース専用`--step race-metadata`も追加。
  - 専用APIはレース状態・成績・出走馬・RPCI/PCI3を保持し、補足情報だけを更新する。
  - 実DBバックフィルと馬場別再検証はWindows実行機での次作業として分離した。
- [x] ✅ **P2 7月小倉芝の脚質別有利度を距離・馬場状態別に再検証する**
  - 確定値診断へ`--style-breakdown year|distance|track-condition`を追加し、CLI/JSONで内訳を保存可能にした。
  - 小倉芝1200mの好走率差は2022年-21.8pt、2024年-11.6pt、2025年-28.4pt、
    2026年-26.9pt。1800m以上は方向が一貫しなかった。
  - `style-advantage-v3`では「7月・小倉・芝・1200m」だけを`reference`とし、他距離の過剰警告を解除。
  - 馬場状態は対象全件が未登録。RAパーサーの既知制約として別タスクへ分離した。
- [x] ✅ **P2 函館・小倉の夏開催における脚質別展開有利度を複数年で検証する**
  - 7月1〜22日の芝を2022〜2026年で診断し、函館は年ごとに方向が変わる一方、小倉はデータのある
    2022・2024・2025・2026年すべてで確定RPCI×確定脚質の有利群が逆転すると確認した。
  - 係数・PAI・順位は変更せず、7月の小倉芝を`style-advantage-v2`で`reference`とした。
    その後の距離別検証で1200mだけへ限定し、`style-advantage-v3`へ更新した。
  - Webの展開分析カードへ「参考」と根拠を表示し、強い推奨として誤読されないようにした。
- [x] ✅ **P2 芝の脚質別展開有利度が予測時だけ逆転する原因を切り分ける**
  - 予測RPCI/実績RPCI × 予測脚質/確定脚質の4パターンを同一母集団で比較する診断CLIを追加。
  - 2025年後半は予測同士+5.2pt、2026年前半は+4.4ptで、期間全体では逆転せず。
  - 2026年7月は全体-11.9pt。福島は想定RPCIが主因、函館・小倉は確定値同士でも逆転した。
  - 短期・開催場別の標本だけで本番係数は変更せず、過去年の同開催比較を次の検証候補とした。
- [x] ✅ **P2 脚質別展開有利度を実DBで検証可能にする**
  - 通常バックテストへ有利群・不利群の好走率、リフト、好走率差、相関、JSON明細を追加。
  - 確定RPCI・確定脚質だけで係数の方向性を切り分ける`--validate-style-advantage`を追加。
  - 1000レース診断では、芝は有利群26.4%／不利群19.3%、ダートは33.0%／14.0%で方向性は妥当。
  - 予測込み100レースでは芝だけ逆転したため、仮係数は変更せず、芝の想定RPCIまたは脚質予測を
    次の調査対象とした。
- [x] ✅ **P1 DBマイグレーション不足を検出し、正しい復旧手順を表示**
  - `/health`をlivenessとして維持し、`/ready`でDB接続とORM必須テーブル・列を検査する。
  - Webは一覧APIの500発生時だけreadinessを確認し、スキーマ不足なら
    `python -m alembic upgrade head`、DB停止なら接続確認を案内する。
  - 前タスクで追加したRepositoryメソッドをバックテスト用ラッパーにも委譲し、全体mypyを修復。
  - 検証: API非統合482 passed、関連33 passed、PostgreSQL統合1 passed、Web71 passed、
    API Ruff・mypy 62ファイル、api-client/Web typecheck、Web build成功。

- [x] ✅ **P0 成績未取り込み警告と2026-07-19小倉11Rの実データ不整合を修復**
  - 確定出馬表を完全スナップショットとして置換し、結果送信前に必ず同じ出馬表を登録する。
  - 特別登録は確定枠順・結果を上書きせず、JRA外会場・海外行・障害戦を平地の未取り込み監視から除外。
  - `--only-incomplete`と認証付き全対象キーAPIを追加し、旧開催回キーを正規キーへ置換可能にした。
  - 実DB: 未取り込み343件→0件。小倉11Rは18頭、17頭着順反映。小倉1Rは障害へ再分類。
  - 検証: API非統合477 passed、worker204 passed、変更対象Ruff、API変更対象mypy、
    OpenAPIスナップショット、api-client typecheck成功。

- [x] ✅ **P1 同期完了後に今後のレース予想を事前生成**
  - 認証付き`POST /internal/ingest/forecasts/precompute`と`--step forecasts`を追加。
  - 自動同期をentries/results/special-entries/forecastsの順にし、一覧初回表示での全レース計算を回避。
  - 過去レースは後付け予想せず、今日以降・出走前・出走馬ありだけを再生成する。
  - ボードAPIの欠損時フォールバックは残し、事前生成失敗時も閲覧可能性を維持。
  - 検証: API非統合472 passed、worker194 passed、Web68 passed/build、API Ruff、
    変更対象mypy strict、api-client/Web typecheck成功。worker全体Ruff/mypyには既知違反が残る。

- [x] ✅ **P2 予想martのモデル世代選択を生成日時ベースへ変更**
  - Alembic `004`で`predicted_pace`と`pace_fit`へ`generated_at`を追加。
  - 回顧は最新の想定展開、レースボードは最新の想定展開と最新PAI世代内の最上位馬を選択。
  - 同一モデルの再計算でも生成日時を更新し、モデル名の辞書順へ依存しない。
  - 検証: API非統合467 passed、PostgreSQL統合21 passed、Web68 passed、Ruff、
    変更対象mypy strict、api-client/Web typecheck成功。

- [x] ✅ **P1 レース一覧の予想取得N+1を一括APIへ移行**
  - `GET /api/v1/races/board?date=YYYY-MM-DD`で、レース情報と軽量予想を一括返却。
  - 一覧契約は展開ラベル・信頼度・最上位候補に限定し、PCI/RPCI/PAI実数値を非公開。
  - 初回だけ未作成予想を計算し、以後は`predicted_pace`/`pace_fit`を一括読取。成功時の
    リクエスト単位commitを追加し、従来は破棄されていた予想martを永続化。
  - 出走馬・馬番・枠の変更時だけキャッシュを無効化し、結果更新時は答え合わせ用に保持。
  - 検証: API非統合467 passed、契約33 passed、mart統合2 passed、Web68 passed、Ruff、
    api-client/Web typecheck、Web build成功。全体mypyは既知のNumPy型定義問題、import-linter未導入。

- [x] ✅ **P2 成績未取込警告から安全な手動再同期コマンドを提示**
  - DBで最古の未取込日を集計し、標準10日以上の`recommended_sync_days_back`をAPIへ追加。
  - Web警告バナーに`run_mykeibadb_full_sync.ps1 -DaysBack N`のコピーUIを追加。正常時は非表示。
  - APIからWindowsプロセスを直接起動せず、認証・多重実行・配置差のリスクを回避。
  - OpenAPIとapi-client型を再生成。
  - 検証: API非統合464 passed、Repository統合1 passed、Web67 passed、Ruff、変更対象mypy、
    api-client/Web typecheck、Web build成功。

- [x] ✅ **P1 展開コメント生成をゼロコスト既定モードへ変更**
  - `COMMENT_GENERATOR_MODE=rule`を既定とし、APIキーが環境に残っていても外部APIを呼ばない。
  - Geminiは`COMMENT_GENERATOR_MODE=gemini`と`GEMINI_API_KEY`の両方を明示した場合だけ有効化。
  - Gemini指定時のキー欠損・初期化失敗は`comment-v2`へフォールバックする。
  - 公開APIスキーマ・コメント計算・Web表示は変更なし。
  - 検証: API非統合463 passed、関連29 passed、ruff成功、変更対象2ファイルのmypy strict成功。
    全体mypyは既知のNumPy型定義とPython 3.11設定の不整合で解析前に停止。import-linterは未導入。

- [x] ✅ **P2 Gemini既定モデルを3.5 Flashへ移行し、環境変数化**
  - 提供終了した`gemini-2.0-flash`から`gemini-3.5-flash`へ既定モデルを更新。
  - `GEMINI_MODEL`でモデルを上書き可能にし、DIから`GeminiCommentGenerator`へ渡す。
  - Gemini出力を`comment-gemini-v3`へ更新し、`reasons`には実際に使用したモデル名を記録。
  - APIキー未設定・呼出失敗時の`comment-v2`フォールバックは維持。実APIは呼び出していない。
  - 検証: API非統合457 passed、関連23 passed、ruff成功、変更対象3ファイルのmypy strict成功。

- [x] ✅ **P2 枠順未確定時の展開コメント馬番号表示を修正（comment-v2）**
  - `formation-v1`と同じ枠順確定判定をscenario・ルールコメント・Geminiプロンプトへ結線。
  - 共通の`horse_number_label.py`で、未確定時は「登録順 N（馬番未確定）」、確定後だけ
    「N番」と表示する。API公開スキーマは変更していない。
  - 出力変更を追跡するため、ルール版を`comment-v2`、Gemini版を`comment-gemini-v2`へ更新。
  - 検証: API非統合454 passed、関連74 passed、ruff成功、変更対象5ファイルのmypy strict成功。
    import-linterはローカル環境に未導入。全体mypyは既知のNumPy型定義不整合で解析前に停止。

- [x] ✅ **P1 LightGBMモデルのWindows改行破損修正・AbilityWeights実DB採用判断**
  - `.gitattributes`で`apps/api/models/*.txt`をLF固定。LightGBMの`tree_sizes`がCRLF変換で
    壊れ、実モデルを読めなくなる問題を修正した。既存clone向けにローダーでもLFへ自己修復する。
  - モデル読込失敗時のフォールバックを警告ログへ記録し、追跡中の芝・ダートモデルを実際に
    ロード・予測する回帰テストを追加。
  - 実DBを2025年後半212レース、2026年前半97レースに分けて4候補を比較。
    全3指標が両期間で改善する候補はなく、現行重み（0.55/0.30/0.15）を維持する。
  - 検証: API非統合449 passed、LightGBM関連31 passed、ruff、実DBバックテストCLI成功。
    mypyはローカルNumPy型定義とPython 3.11設定の不整合で対象コード解析前に停止。

- [x] ✅ **P2 取り込み監視をデータ完全性へ拡張**（本セッション・OpenAI Codex）
  - 前日以前のレースが `status=entries` のまま残っている件数と代表20件を、
    `GET /api/v1/ingest-status` で返すようにした。
  - Webトップの `IngestStatusBanner` に成績未取込警告と対象レースへのリンクを追加。
    取り込みログが無い環境でも、未取込レースがあれば警告する。
  - API非統合テスト447件、関連SQL統合テスト1件、Webテスト66件、ruff、Web型チェック・本番ビルド成功。
    mypyはローカルNumPy型定義とPython 3.11設定の不整合で依存型解析前に停止（新規コード原因ではない）。

- [x] ✅ **P1 AbilityWeightsの同一期間比較CLI**（本セッション・OpenAI Codex）
  - `backtest_forecast.py --compare-ability-weights`を追加し、現行・近走のみ・近走重視・
    市場支持重視の4候補を同じ対象レースで比較。
  - 1位馬勝率・1位馬好走率・TOP3好走捕捉率と現行差を表示し、`--output`のJSONにも保存。
  - 候補は検証専用で、`DEFAULT_WEIGHTS`は自動更新しない。実DB実行と採用判断は上記タスクで完了。
  - 検証: API unit+contract 444 passed（関連は40 passed）、変更対象Ruff、
    mypy strict 58ファイル成功、CLI `--help`成功。

- [x] ✅ **P1 統合順位予想 Phase 2 完成（grade・確定馬体重・検証指標）**（本セッション・OpenAI Codex）
  - grade: RA `GradeCD[615]` を公式コードから名称へ変換し、mykeibadb→API→`races.grade`へ保存。
    ability-v3はgradeをクラス補正へ優先利用し、欠損時だけrace_class推定へ縮退。
  - 馬体重: SE `BaTaijyu[324:327]` をresults payloadにも追加し、results単独再取込でも既存
    `race_entries.weight`を確定馬体重で更新。体格と能力を直結させる根拠がないため能力加点は見送り。
  - 検証: バックテストへ統合順位の1位馬勝率・1位馬好走率・TOP3好走捕捉率とJSON出力を追加。
  - テスト: API unit+contract 440 passed、ingestion 191 passed、Web 65 passed、API/ingestion変更対象Ruff、
    lint-imports、api-client/web typecheck、Web build成功。mypy strictは既存lgbm unused-ignore 1件のみ。

- [x] ✅ **P0 Claude Code リモート更新の統合と引き継ぎ資料更新**（本セッション・OpenAI Codex）
  - 状況: Codex が `tasks/current.md` の最優先候補（バックテスト結果のJSON保存）に着手し
    `144ebfd feat(backtest): export reports as json` を作成したが、push前にリモートが16コミット進んでいた。
  - 判断: リモート側 `03bc005 feat(backtest): persist backtest reports to JSON via --output` が同じ目的を
    既に実装済みで、後続コミット（統合順位予想Phase2等）も含まれていたため、マージ競合では
    `apps/api/src/pci/application/backtest.py`・`apps/api/scripts/backtest_forecast.py`・
    `apps/api/tests/unit/application/test_backtest.py`・各ドキュメントをリモート版で採用。
    重複実装で新しいClaude Code作業を上書きしないことを優先した。
  - 検証: 競合マーカーなし、API backtest関連の `py_compile` 成功、api-client typecheck成功、
    Web test 65 passed・typecheck/build成功。API/ingestion-workerの全量pytest/ruff/mypyは
    このCodex環境でPython dev toolingが未構築のため再実行不可（Claude Code側の全量結果は
    `docs/HANDOFF.md` に保持）。

- [x] ✅ **P1 統合順位予想: 印→タグUI・順位明確化 ＋ Phase2（人気・本賞金→ability-v2）**（本セッション）
  - UI（ユーザーFB）: `IntegratedRankingView` を刷新。◎○▲△の印を廃止し、総合順位（1位…）を主役に、
    分類は言葉タグ（本命/対抗/穴（妙味）/人気でも注意/能力上位・中位/展開が向く・向きにくい）で表示。
    プレゼン層のみ（ドメイン/スキーマ不変）。
  - Phase2 データ: 人気(TANSHO_NINKIJUN)・獲得本賞金(KAKUTOKU_HONSHOKIN)を永続化。
    ingestion（models/se_parser/jv_spec 予約offset/mykeibadb_client/ingest_api）→ API（ingest schema・
    ResultInput・RaceEntry・use case・ORM・repository・migration 003）→ ability-v2。
  - ability-v2: form0.55＋本賞金(対数正規化)0.30＋人気0.15 を新しさ加重ブレンド。データ無し成分は
    除外し再正規化 → 旧データは form のみ＝v1相当へ安全に縮退。本賞金がクラス係数 best-effort の限界を緩和。
  - **運用（ユーザー作業）**: `alembic upgrade head`（migration 003）＋過去 results の再取込が必要
    （`MANUAL_SYNC_GUIDE §7.5`）。やらなくても壊れない（縮退）。
  - 検証: API 436 passed（+ability-v2 4件・parser round-trip等）、ingestion 185 passed（+2）、
    Web 65 passed・typecheck・build clean、ruff/lint-imports/mypy clean（既存lgbm/tuple-concat debtのみ・新規0）。
  - 後続で馬体重・gradeを追加済み（上記）。残るのは実JV-Dataの人気/賞金オフセット検証。

- [x] ✅ **P1 統合順位予想（能力×展開）Phase1 実装**（本セッション・ユーザー選択のB節要望を再開）
  - 判断: データ範囲=現データのみ / 統合=2軸分類（本命/対抗/穴/危険）（ユーザー選択・`docs/DECISIONS.md` 2026-07-21）。
  - domain: `pace/ability.py`（ability-v1: 出走頭数正規化着順×クラス係数の新しさ加重平均）、
    `pace/integrated_ranking.py`（integrated-v1: 能力相対順位×展開適性の2軸分類・決定的表示順）。
    いずれも純粋・reasons付き・model_version付き。🧪仮係数は `AbilityWeights`（`docs/SPEC.md §9`-16）。
  - 結線: `forecast_use_cases`（近走+過去レースからability構築→統合）, `dto.py`, `schemas.py`,
    OpenAPI再生成（`openapi.json`+`schema.d.ts`）, `api-client/src/index.ts`（型追加）。
  - web: `IntegratedRankingView.tsx`（新規・◎○▲△と能力上位/中位・展開向く/向きにくいを言葉表示、
    実数値は非表示）を `RaceForecastDashboard` の隊列予想の前に配置。
  - 当時の既知の限界: `grade`未永続化のためクラス係数は`race_class`文字列のbest-effortだった。
    本セッションのability-v3でgrade・人気・本賞金・確定馬体重の永続化まで完了済み。
  - 検証: API 432 passed（+新規domain16・contract1）、Web 65 passed、api/web typecheck・build・
    ruff・lint-imports・mypy --strict すべてclean（既存のlgbm 1件は当環境固有・無関係）。

- [x] ✅ **P0 確定成績未反映を解決**（本セッション・§9-15）
  - 診断で「解析は正常（453件解析可・DATA_KUBUN='7'）」→原因は解析より下流と特定。`batch.py`が
    `record_results`失敗をexit 0に握りつぶしていた欠陥を修正し、送信成功/失敗の件数ログを常設。
    ユーザーが最新コードでresultsステップを再実行→全レース送信成功しアプリに反映（解決）。
  - 残: 障害競走の成績が別途未反映（ユーザー保留）。`DaysBack`既定 7→10 済み。

- [x] ✅ **P0 確定成績未反映の切り分け診断ツール＋件数ログ＋日付窓修正**（本セッション2巡目）
  - 契機: DATA_KUBUN修正（`af922a5`）投入後もユーザー環境で確定成績が反映されず、同期ログは
    全ステップexit 0（件数ではない）。「exit 0で0件」だけでは原因層を特定できないと判明。
  - 対応:
    - `batch.py`: `ingest_results`に「SE何行読込／確定成績何行解析／何レース記録」の件数INFOログ、
      「SE行>0だが確定成績0行」なら診断コマンドを促すWARNINGを追加。`ingest_entries`にも件数ログ。
    - `ingestion/diagnose_results.py`(新規): 実DBのSE行を読み、着順/タイム/上り3F列の有無・
      DATA_KUBUN分布・parse_se_result成功数・SEテーブルの実列名一覧を出力。判定セクションで
      (A)mykeibadb未取得 / (B)列名不一致 / (C)バイト配置バグ を切り分ける。個人・馬名系列は既定で伏せる。
    - `run_mykeibadb_full_sync.ps1`: `DaysBack`既定 7→10（月曜実行時に前々週土曜が窓外へ漏れる
      問題を緩和。7/12の欠落はこれで説明可能）。長期バックフィル用に`-DaysBack 21`例を明記。
  - 対象: `apps/ingestion-worker/src/ingestion/batch.py`,
    `apps/ingestion-worker/src/ingestion/diagnose_results.py`(新規),
    `apps/ingestion-worker/tests/test_diagnose_results.py`(新規3件),
    `apps/ingestion-worker/scripts/run_mykeibadb_full_sync.ps1`
  - 検証: pytest 182 passed（+3）、ruff clean（新規ファイル）、mypy --strict clean（diagnose_results/
    batch.pyとも新規エラーなし。batch.pyの既存4件はingest_masters内・未編集で対象外）。
  - **未解決**: 診断ツールの実行はユーザー環境（Windows+MySQL）でのみ可能。出力を受け取り次第
    (A)/(B)/(C)を確定して次の手を打つ。上記「進行中」に残置。

- [x] 🧪 **P0（未検証・空振り） 確定成績1週間以上未反映の原因調査とDATA_KUBUN修正**（本セッション）
  - 調査: entries/special-entriesは動いているのに確定成績だけが1週間以上反映されないという
    ユーザー報告を受け、`mykeibadb_client._build_se_record()`を確認。着順・タイム・上り3Fは
    無条件にバイト列へ書き込まれる一方、DataKubun（確定判定に使う値）は`DATA_KUBUN`列が
    存在すればその値をそのまま採用し、存在しない場合のみ着順等の有無から推測していた。
    実環境の`DATA_KUBUN`列がJV-Data本来の確定コードを正しく反映していない場合、着順等が
    揃っていても`parse_se_result`が`None`を返し続ける、という一貫した説明がつく。
  - 対応: 着順・タイム・上り3Fが全て揃っていれば`DATA_KUBUN`列の値に関わらず'7'（確定）とする
    よう変更（単調な修正で、揃っていない行を誤って確定扱いにする副作用はない）。
    `apps/ingestion-worker/tests/test_mykeibadb_client.py`に回帰テストを追加。
  - 検証: `apps/ingestion-worker`は3.12専用のためこの環境にpython3.12でvenvを作成し
    `pip install -e ".[dev]"`後に実行。pytest 179 passed（+1）、ruff/mypy（該当ファイルは
    差分前後で0/25エラーのまま=新規エラーなし）clean。
  - **結果（空振り）**: ユーザーが再pull＆再同期したが確定成績は反映されず、この仮説は外れ
    （または原因の一部でしかない）と判明。修正自体は単調で無害なため残置。真因は別（診断ツールで
    切り分け中。上記の2巡目タスク参照）。

---

- [x] ✅ **P0 展開恩恵馬カードが枠順未確定の馬番を確定情報のように表示するバグを修正**（本セッション）
  - ユーザー報告: 枠順確定前のレースなのに展開予想画面の「展開恩恵馬TOP5」等に馬番が出ている。
  - 原因: `formation-v1`（隊列予想）は`frame_no`（枠番）で確定/未確定を判定し未確定時は
    `formation: null`にする設計だったが、同じ画面のPAI系出力`HorseFitOutput`にはそもそも
    `frame_no`が無く、この判定が一切されていなかった。特別登録段階の`horse_no`は
    `ingest_entries()`がUMABAN=0時に割り当てる暫定連番で、公式馬番ではない。
  - 対応: `HorseFitOutput`/`HorseFitSchema`に`frame_no`を追加（`forecast_use_cases.py`で
    `RaceEntry.frame_no`から供給）。web側`lib/pace.ts`に`horseNumberLabel()`を新設し、
    `frame_no>0`なら「馬番 N」、`frame_no=0`なら「登録順 N（馬番未確定）」を表示。
    `RaceForecastDashboard.tsx`・`HorseFitTable.tsx`・`app/page.tsx`の計5箇所を統一。
    PAIスコア自体は枠順確定前でも意味があるため、カードごと非表示にはせずラベルのみ是正。
  - 対象: `application/dto.py`, `application/forecast_use_cases.py`, `presentation/schemas.py`,
    `openapi.json`+`schema.d.ts`(再生成), `apps/web/src/lib/pace.ts`(+テスト2件),
    `apps/web/src/components/RaceForecastDashboard.tsx`, `apps/web/src/components/HorseFitTable.tsx`,
    `apps/web/src/app/page.tsx`, `tests/unit/application/test_forecast_use_cases.py`(+1件),
    `tests/contract/test_races_api.py`(HORSE_KEYS更新)
  - 検証: API 415 passed（+1）、Web 65 passed（+2）、ruff/mypy --strict/lint-imports/
    typecheck/build すべてclean。
  - 後続対応: `scenario.py`を含む自然文コメントの同種問題は2026-07-22の`comment-v2`で解決済み。

- [x] ✅ **P0 自動同期が来週の特別登録を一度も取り込んでいなかったバグを修正**（本セッション）
  - ユーザー報告「月曜なのに土日の結果・来週の特別登録馬が未反映」を受けて
    `run_mykeibadb_full_sync.ps1`（Task Scheduler「PCI_Sync_Mykeibadb」金/土10:00・日18:00が実行）の
    中身を確認。`batch.py --step entries`/`--step results` のみを呼んでおり、`--step special-entries`
    （mykeibadbの`TOKUBETSU_TOROKUBA`系テーブルを読む独立ステップ）を一度も呼んでいなかったと判明。
    `setup_task_scheduler.ps1`自身のdocstringには「日曜18:00は来週の重賞特別登録取り込みも兼ねる」と
    明記されており、実装漏れ（2026-07-11のTask Scheduler自動化切替時に追加し忘れたと推測）と判断。
  - 対応: `run_mykeibadb_full_sync.ps1`に3番目の呼び出し（`-Step special-entries`、同じ過去7日〜
    未来14日の日付窓）を追加。`sync_mykeibadb.bat`・`MANUAL_SYNC_GUIDE.md`（手順・注意書き・
    6.8節トラブルシューティング新設）・`docs/SPEC.md §6`・`docs/DECISIONS.md`を更新。
  - コード上の修正のみでは今週分の取りこぼしは遡って埋まらないため、ユーザーには
    `--step special-entries`の手動実行コマンドを別途案内。
  - 未解決: 「土日の確定成績が反映されていない」側は自動実行の対象内（`--step results`）のはずで、
    「取りこぼし」ではなく「実行自体の失敗/未発火」の可能性が高いが、このクラウド環境からは
    Task Scheduler実行履歴・ログ・MySQL80状態を確認できないため、ユーザー自身の診断が必要
    （`MANUAL_SYNC_GUIDE.md §6.8`に診断手順を用意）。上記「進行中」に残置。

- [x] ✅ **P3 JV-Dataバイトオフセットの JV-Link新バージョン追従手順の明文化（技術的負債）**（本セッション）
  - 背景: `jv_spec.py`（RA/SE）のオフセットは実データ校正済みだが、JV-Linkが仕様バージョンを
    上げた場合の再検証手順が文書化されていなかった。
  - 調査で判明: UM/KS/CH（`master_parsers.py`）は既に Ver.3.0.0→Ver.4.9 の実データ差分
    （日付フィールド群24byte追加による名前位置シフト）を確認・反映済みという実例が存在した。
    一方RA/SE側は README.md/common.py が「Ver.3.0準拠」と書いたままで、実際にどのバージョンの
    出力を元に校正したかは未確認と判明（独断で確定せず `docs/SPEC.md §9`-8 に記録）。
  - 対応: `apps/ingestion-worker/JV_SPEC_MAINTENANCE_GUIDE.md` を新規作成。UM/KS/CHの実例を
    土台に、`dump_records.py`→`verify_layout.py`（アンカー検証→フィールド目視確認）→
    `locate_haron.py`/`locate_corners.py`（新オフセット特定）→`jv_spec.py`更新→テスト更新→
    記録、の手順と安全策（1レースだけでCONFIRMED昇格しない等）を明文化。
    `README.md`・`docs/SPEC.md`（§6, §9-8）から相互参照を追加。
  - コード変更なし（ドキュメントのみ）。実際の再検証はWindows実行機（JV-Link必須）が必要なため、
    ガイドの実施自体は引き続き未着手（`docs/SPEC.md §9`-8に残置）。

- [x] ✅ **P3 旧handoffファイルの整理（技術的負債）**（本セッション）
  - `docs/handoff-claude-code-2026-06-25.md` の内容を精査。全項目が (a) 現構成と食い違う
    誤情報（`domain/services.py`・`infrastructure/repositories.py`は現存しない旧パス、
    「次に推奨する作業」は全項目完了済み）か、(b) 既存資料で完全に上書き済み
    （ローカル起動→`apps/api|web/README.md`、mykeibadb `.env`→`.env.example`、
    同期手順→`MANUAL_SYNC_GUIDE.md`、ディレクトリ構成→`docs/ARCHITECTURE.md`）と判明。
    「吸収すべき未収録の情報」が残っていなかったため削除（Git履歴には残るため復元可能）。
  - 対応: ファイル削除、`tasks/backlog.md` 更新。他ドキュメントからの参照なし（削除前に確認済み）。

- [x] ✅ **P3 `mypy --strict` 全体化（技術的負債）**（本セッション、ドキュメント訂正のみ）
  - `tasks/backlog.md` C節に着手したところ、既存の「infrastructure/presentationはスタブ未導入で
    多数エラー・環境要因」という長年の記載が**誤りだったと判明**。
    `python -m mypy src/ --strict` を実行（`.mypy_cache`削除後も再現）すると
    **56ファイル全体で0エラー**。原因は素の`mypy`コマンドが`uv tool`等の隔離環境
    （プロジェクト依存関係が入っていない）を指していたこと（前セッションで発見した
    `pytest`の問題と同根）。fastapi/sqlalchemy/pydanticはいずれも`py.typed`同梱で型情報あり。
  - 対応: `CLAUDE.md`, `AGENTS.md`, `docs/PROJECT_RULES.md`, `docs/ARCHITECTURE.md`,
    `apps/api/README.md`, `tasks/backlog.md` の誤記載をすべて訂正。Definition of Doneも
    「domain・applicationのみ0エラー」から「全体で0エラー」へ引き上げ（実態を反映）。
  - コード変更なし。次回以降は `python -m mypy src/ --strict` を標準コマンドとして使うこと。

- [x] ✅ **P1 データ取り込みの鮮度監視（推奨1・「監視・鮮度表示」）**（コミット `2b83d75`）
  - 目的: `ingest_log` は書き込み専用で、自動同期が静かに失敗し続けても気づく手段が無かった。
  - 対応: 新規 `domain/ops/ingest_log.py`（`IngestLogRepository` Protocol +
    純粋関数 `evaluate_freshness()`）。判定は「直近試行の失敗有無」「直近成功からの経過日数
    （暫定閾値 `STALE_AFTER_DAYS=4`）」のみで、Task Schedulerの具体的cronはハードコードしない。
    `GET /api/v1/ingest-status`（公開GET、`/internal/ingest/*`の認証とは別）を新設し、
    web トップに `IngestStatusBanner` を追加（正常時は控えめ、鮮度低下・失敗時のみ目立つ表示、
    失敗一覧は開閉式）。ログが1件も無い環境（開発/fixture等）は「異常」ではなく「監視対象外」
    として扱い誤警告を防ぐ。
  - 対象: `domain/ops/ingest_log.py`(新規), `infrastructure/repositories/ingest_log_repository.py`(新規),
    `application/dto.py`, `application/ingest_status_use_cases.py`(新規), `presentation/schemas.py`,
    `presentation/routers/status.py`(新規), `presentation/dependencies.py`, `presentation/app.py`,
    `packages/api-client`(型+クライアントメソッド追加), `apps/web/src/lib/ingestStatus.ts`(新規),
    `apps/web/src/components/IngestStatusBanner.tsx`(新規), `apps/web/src/app/page.tsx`
  - 検証: API 414 passed（+18: domain 10・application 5・contract 3）、Web 63 passed（+6）、
    ruff/mypy --strict/lint-imports/typecheck/build すべてclean。
  - 未実施（ユーザー環境でのみ確認可能）: `NOTIFY_WEBHOOK_URL` によるWebhook通知が実際に届くかの
    実地確認。このクラウド環境からは検証不可。

- [x] ✅ **P1 脚質別有利度の差が出ない問題を修正（style-advantage-v1）**（コミット `3d3131e`）
  - ユーザー指摘: 展開分析の有利度が 71/76/91/96 のように高止まりし、機能していると言い難い。
  - 原因: web が「その脚質の最大PAI」を有利度として流用（脚質自体の有利さではない）。
  - 対応: domain に `style_advantage.py` を新設し、想定RPCIの中立点（classify_pace と同じ閾値中点:
    芝50/ダート43）からの乖離を 50=互角の対称スコアへ写像。逃げ競合減点あり。reasons/model_version 付き。
    DTO→schema→OpenAPI→api-client→web（`styleAdvantageScores` + 有利/互角/不利の言葉ラベル）まで結線。
  - 対象: `domain/pace/style_advantage.py`(新規), `application/dto.py`, `forecast_use_cases.py`,
    `presentation/schemas.py`, `tests/`(domain 12件+app 1件+contract 1件), `openapi.json`+`schema.d.ts`(再生成),
    `packages/api-client/src/index.ts`, `apps/web/src/lib/pace.ts`(+テスト2件),
    `apps/web/src/components/RaceForecastDashboard.tsx`
  - 検証: API 396 passed（+14）、Web 57 passed（+2）、ruff/mypy --strict/lint-imports/typecheck/build clean。
  - 関連: ユーザー要望②（展開＋絶対能力の統合順位予想）は `tasks/backlog.md` B節に P1 で記録
    （能力指数の定義が必要なため着手時に仕様合意から）。

- [x] ✅ **P2 バックテスト結果の可視化/保存**（コミット `03bc005`）
  - 対象: `apps/api/src/pci/application/backtest.py`（`report_to_dict`等の変換関数を追加）,
    `apps/api/scripts/backtest_forecast.py`（`--output <path>` オプション追加）,
    `apps/api/tests/unit/application/test_backtest.py`（`TestReportToDict` 2件追加）
  - 結果: `--output` 指定時、混合集計＋（`--track-type`未指定なら）track別内訳をJSONに保存。
    既存の `print` 出力は変更なし。DBテーブル化は見送り（推移ダッシュボードが要る段階で再検討、
    `tasks/backlog.md` A節に記録）。
  - 検証: `python -m pytest tests/unit/ tests/contract/ -q` 382 passed（+2）、ruff/mypy/lint-imports
    clean。`_write_output`の実ファイル書き込み・JSON往復読み込みを手動スモークテストで確認
    （スクリプト層はプロジェクト方針上ユニットテスト対象外のため）。

- [x] ✅ **P1 隊列予想の「自在」過多を距離対応の脚質予測で改善**（コミット `2b083ba`）
  - 原因調査: 直近20レース266頭で自在139頭のうち、混在履歴99頭・履歴なし40頭。
  - 明確な従来脚質は維持し、混在型だけを対象距離・過去走距離・近走順で再判定。
  - 先行・差し同数時は、先行歴が今回より短距離中心なら先行、長距離中心なら差しを優先。
  - 実DB: 自在139頭（52.3%）から40頭（15.0%）へ減少。履歴なしは参考表示を維持。
  - 検証: API unit+contract 380 passed、Web 55 passed、mypy/ruff/import-linter/typecheck/build成功。

- [x] ✅ **P1 枠順確定後の隊列予想機能**（コミット `c679e09`）
  - 特別登録の `frame_no=0` を除外し、全馬に実枠番がある場合だけ `formation-v1` を生成。
  - 脚質と近走序盤位置から、先頭・好位・中団・後方の4ゾーンへ配置。各馬に根拠と信頼度ラベルを付与。
  - OpenAPI型を更新し、展開予想画面にJRA枠色を使ったレスポンシブ隊列ビューを追加。
  - 実DB: entries 278件中、枠順確定112件で生成、未確定166件で非生成を確認。
  - 検証: API unit+contract 374 passed、Web 55 passed、mypy/ruff/import-linter/typecheck/build成功。

- [x] ✅ **P1 レース分析画面のモダンUI刷新**（コミット `4d9e5b5`）
  - 共通ヘッダーとデザイントークンを追加し、レース一覧・展開予想・確定後回顧を同じUIへ統一。
  - 一覧は開催日カレンダーをデスクトップのサイドバーへ移し、レースカードの状態・導線を明確化。
  - 予想・回顧はダークヒーロー、要約カード、解説、的中表示を情報優先度に沿って再構成。
  - 回顧画面の表面からPCIという専門用語を外し、数値を増やさずペース傾向として表示。
  - 検証: Web 55 tests / typecheck / production build がすべて成功。実データの一覧・予想・回顧URLがHTTP 200。

- [x] ✅ **P1 想定RPCI 受入基準の判定方針を決める（製品判断）**（`docs/DECISIONS.md` 2026-07-11）
  - 判断: 現行モデル（lgbm-turf-v1/lgbm-dirt-v1）のまま運用継続。MAE≤1.5 を追う追加投資は今は行わない。
  - 理由: MAEの未達幅は2回の独立計測で一貫した構造差。UIは実数値非表示のため、製品価値に直結する
    ラベル一致率(33%ランダムを上回る)とPAIリフト(track別1.18〜1.32x)は実効性ありと判断。
  - 見直し条件: `forecast_accuracy` 蓄積増加、またはダートの外れに偏りが見えた場合に再検討。
  - 注記: 受入基準の文言自体（blended/track別のどちらで判定するか）は基準を定めた側の確認が必要な
    別問題として `docs/SPEC.md §9`-3 に残置（この決定の範囲外）。

- [x] ✅ **P2 `backtest_forecast.py` の既定出力に track 別内訳を追加**
  - 対象: `apps/api/src/pci/application/backtest.py`(`group_races_by_track`追加),
    `apps/api/scripts/backtest_forecast.py`(`_print_track_breakdown`追加),
    `apps/api/tests/unit/application/test_backtest.py`(テスト2件追加)
  - 結果: `--track-type` 未指定時、混合集計に加え芝/ダート別内訳も自動表示。再予測はせず、
    既存の `ForecastBacktester.run()` を track 別サブセットで再実行するのみ（application層は
    グルーピングのみ純粋関数化しテスト、DB配線はスクリプト層のまま）。
  - 検証: `python -m pytest tests/unit/ tests/contract/ -q` 364 passed（+2）、ruff/mypy/lint-imports clean。

- [x] ✅ **P1 想定RPCI 精度の検証（受入基準の達成度確認）**（コード変更なし、ドキュメント更新のみ）
  - 対象: `docs/SPEC.md §8/§9`, `docs/adr/0005-rpci-forecast-strategy.md §5.4`
  - 実行: ユーザーが本番相当DB（mykeibadb蓄積データ）で `python -m scripts.backtest_forecast --limit 200`
    を実行（混合／芝のみ／ダートのみ）。結果をこちらで分析・記録。
  - 結果概要:
    - MAE≤1.5 は **未達（構造的）**。混合7.848 / 芝8.861 / ダート8.332 — 15,440レースの過去
      バックテストと整合する安定した値で、単発の外れ値ではない。
    - 展開ラベル一致率≥60% は **芝(73.5%)・混合(61.0%)は達成、ダート(42.5%)は未達**。
      受入基準文言が track 区別を明記していないため、判定基準（blended/track別）は未確定事項化。
    - **新知見:** 混合サンプルで PAI point-biserial を見ると +0.009（無相関）に見えるが、
      track別に分けると +0.084(芝)/+0.032(ダート) と過去記録どおりの正相関に戻る
      （母集団混在による希釈）。今後 PAI 検証は必ず `--track-type` を使うこと。
  - 未解決: 上記2件を `tasks/backlog.md` に追記（受入基準の判定方針決定、backtest ツールの
    track別内訳表示）。

- [x] ✅ **P1 AI 引き継ぎ基盤の整備**（コミット `004aead`）
  - 対象: `docs/PROJECT_RULES.md`, `docs/ARCHITECTURE.md`, `docs/SPEC.md`, `docs/DECISIONS.md`,
    `docs/HANDOFF.md`, `AGENTS.md`, `tasks/current.md`, `tasks/backlog.md`, `CLAUDE.md`(追記)

- [x] ✅ **P1 `forecast_accuracy` の UI 表示**（コミット `e65f919`）
  - 対象: `packages/api-client/src/index.ts`(型追加) + `schema.d.ts`(再生成),
    `apps/web/src/lib/pace.ts`(`forecastAccuracyMeta`), `apps/web/src/lib/pace.test.ts`,
    `apps/web/src/components/ForecastAccuracyBadge.tsx`(新規),
    `apps/web/src/app/races/[raceKey]/pace-analysis/page.tsx`, `apps/web/src/app/globals.css`
  - 結果: pace-analysis 画面に「想定が的中/外れ」を**言葉・色**で表示（実数値は出さない）。
    予測未保存レースではバッジ非表示。vitest 3件追加（web計55件 green）。
  - 副次対応: `schema.d.ts` 再生成で、前回セッション（ingest_log 追加時）に反映漏れだった
    `IngestLogBody`/`IngestLogResponse`/`/internal/ingest/log` の型ドリフトも解消。

---

## 次に着手する候補（今スプリントの当面・優先順位順）

（進行中タスクはなし。以下は `tasks/backlog.md` と実DB確認から整理した残タスク。）

- [x] **P1 直近1年の重複レース450組を安全に統合する**
  - 状態: ✅完了。mykeibadbで正規キーを判定し、再同期・署名照合・mart代替確認後に統合した。
  - 結果: 2026-07-23の実DB確認で重複0組。詳細はファイル先頭と`docs/HANDOFF.md`を参照。

- [ ] **P2 実JV-Dataの人気・賞金予約オフセットを検証する**
  - 状態: ⏸Windows JV-Link実機待ち
  - 対象: `apps/ingestion-worker/JV_SPEC_MAINTENANCE_GUIDE.md`

- [ ] **P2 暫定定数の検証と正式化**（`_NEIGHBOR_BLEED_RATIO`・上がり3F妥当範囲・`RuleWeights`・`PaiWeights`・
  `FormationWeights`・`DistanceStyleWeights`・`StyleAdvantageWeights`・`AbilityWeights`）
  - 状態: 🔄一部完了。`AbilityWeights`・`RuleWeights`・`PaiWeights`の候補比較は完了し、
    いずれも現行値を維持。
  - 背景: `docs/SPEC.md §9` に記載の仮仕様。実データ検証後に確定する方針（独断で確定しない）。
    `FormationWeights`（脚質70%/近走序盤位置30%・4ゾーン境界）と`DistanceStyleWeights`
    （距離スケール・新しさ減衰・先行距離補正）は2026-07-12にCodexが追加した仮係数
    （`docs/SPEC.md §9`-10, `docs/DECISIONS.md` 2026-07-12参照）。`AbilityWeights`
    （近走内容0.55/本賞金0.30/人気0.15のブレンド比・新しさ減衰・本賞金対数レンジ等）は
    2026-07-21にability-v2として追加（`docs/SPEC.md §9`-16）。
  - 完了条件: 対象定数ごとに実データでの妥当性検証結果を記録し、確定 or 調整の判断を
    `docs/DECISIONS.md` に残す。formation-v1については隊列ゾーン一致率（実際の後方カメラ等の
    確定データがあれば）での再検証が望ましいが、現状データで可能な範囲でよい。`AbilityWeights`は
    migration 003 適用＋過去成績再取込（`MANUAL_SYNC_GUIDE §7.5`）後でないと人気/本賞金データが
    無く検証できない点に注意。
  - ブロック要因: 対象定数ごとの評価指標・受入条件は未確定。独断で正式化しない。

- [x] **P2 Webhook通知のWindows実地確認**
  - 2026-07-25、`run_batch.ps1 -TestNotification`でSlackへの送信成功とURL非出力を確認した。

---

## 保留・ブロック中

- [ ] ⏸ **ダート特徴量追加・学習データ拡張の検討**（`docs/DECISIONS.md` 2026-07-11参照）
  - 状態: ⏸保留（製品判断済み: 現時点では追加投資しない）
  - 見直し条件: `forecast_accuracy` の蓄積データが増える（track毎に100件超など）、
    または実運用でダートの外れ方に偏り（例: 常にハイ側へ外す）が見えた場合に着手を再検討。

- [x] ✅ **P2 統合テスト（testcontainers-postgres）の実行環境整備**
  - CIとローカルDockerで実行可能。今回も重複集計のPostgreSQL統合テスト1件が成功した。
