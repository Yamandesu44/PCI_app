...
- [ ] **ラベル構成比を確認する。** 55/45 は規則を当てただけで構成比は未測定。
      `--diagnose-pai` の構成比表で「合致」の割合と、合致/不利の好走率の順序を見る。
- [ ] **オフセットを期間外で確認する。** −1.17 / +0.47 は 2026-06-01以降の500レースからの
      当てはめ値。別期間で解き直して安定を見る。`--compare-pai-weights` の `uncentered` が
      中心合わせの寄与を継続監視する。
