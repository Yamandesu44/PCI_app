...
## 2026-08-02 完了: スマホの取り込み警告を要約表示へ圧縮

- [x] `IngestStatusBanner`で、モバイルに状態チップと異常種別ごとの件数を常時表示した。
- [x] 説明文、対象レース、失敗詳細、再同期・馬場情報補完コマンドは既存の`details`内に維持した。
- [x] 768px以上は従来の説明文と「詳細と復旧手順」表示を維持した。
- [x] 既存の`ingestStatusMeta()`が持つ異常優先順位を変更していない。
- [x] `IngestStatusBanner.test.tsx`へモバイル要約の回帰テストを追加した。
- [x] 対象テスト、Web全112 tests、型チェック、本番ビルドを完了した。
...
- 現在、スマホ画面向けの確定済み実装タスクはありません。

### 検証タスク

- [ ] **P2 スマホ主要導線の通し確認**:
  390px相当でレースボード、日付切替、競馬場切替、レース詳細、同一開催ナビ、
  サマリー・隊列・注目馬・詳細タブ、一覧へ戻る操作を連続して確認する。
- [ ] **P2 実端末ロケテスト**:
