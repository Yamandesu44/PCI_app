...
- [ ] **`sortByPai` が ADR と矛盾している。** `apps/web/src/lib/pace.ts` が馬を PAI 降順で
      並べており「脚質をまたいだ順位付けにPAIを使わない」に反する。

### CI（未着手・仕様判断が要る）

- [ ] `mypy --strict` が CI で失敗（私の変更とは無関係）。CIは Python 3.12 で numpy 2.5.1 が
      入るが `[tool.mypy] python_version = "3.11"` のため落ちる。候補: (a) Type/Lintジョブを
      3.11 で走らせる、(b) `numpy<2.5` 固定、(c) 3.11サポートを捨てる。
...
- [ ] **`mypy --strict` が CI で失敗**（私の変更とは無関係）。CIは Python 3.12 で
      numpy 2.5.1 が入るが `[tool.mypy] python_version = "3.11"` のため numpy の
      PEP695 構文で落ちる。候補: (a) Type/Lintジョブを 3.11 で走らせる、
