...
- [x] Webhook通知を最終失敗時の1回へ集約し、URL秘匿とWindows実地確認を完了する。
- [x] Windows同期のコンソールと保存ログの日本語文字化けを解消する。
- [x] JST固定オフセット修正の回帰テストと実環境予想生成を完了する。
- [x] Windows全同期の開始前にFastAPI・PostgreSQL readinessを確認し、連続500と部分実行を防止する。
- [x] Windowsの`tzdata`未導入環境でも予想事前生成の日付判定を実行できるようにする。
- [x] 取り込み警告の詳細・対象一覧・復旧コマンドを1つの開閉領域へ集約し、初期表示をコンパクトにする。
- [x] `IngestStatusBanner`のTSXコンポーネントテストを追加する。

## 2026-07-24 完了

- [x] readinessでmigration 006のmartモデル世代列長不足を検出する。
- [x] ダートRPCI v4の期間外監視と再学習レビュー条件を実装する。
- [x] RPCI v4へ対象日前の前後半3F履歴特徴量を追加し、ダート専用モデルへ採用する。
- [x] RPCI v4候補用に前半3F・後半3Fを内部永続化し、直近1年をバックフィルする。
- [x] mykeibadbの前半3F・後半3F・区間ラップ利用率を芝ダート・距離帯別に診断する。
- [x] RPCI候補へ、対象レースより前の各馬の前付けペース履歴を集約したv3特徴量を追加する。
- [x] v1/v2/v3モデルを特徴量数で安全に自動判別し、モデル世代を正しく記録する。
- [x] v3候補を芝・ダート各200レースで比較し、本番モデルの採否を判断する。
- [x] RPCI候補へ頭数・逃げ競合・自在比率・距離帯・競馬場one-hotのv2特徴量を追加する。
- [x] v1/v2モデルを特徴量数で安全に自動判別し、モデル世代を正しく記録する。
- [x] v2候補を芝・ダート各200レースで比較し、本番モデルの採否を判断する。
- [x] RPCI学習へ少数展開区分の平方根逆頻度・完全逆頻度プロファイルを追加する。
- [x] 加重候補を検証セットと独立200レースで比較し、本番モデルの採否を判断する。
- [x] 加重学習時の本番モデル誤上書きをCLIで防止する。
...
- [x] ~~**P1 RPCI v4候補用のS3/L3永続化とバックフィル**~~ → 完了（2026-07-24）。
  `races`へ前半3F・後半3Fを内部分析値として保存し、ingest resultsから更新する。
  Alembic migration、Repository往復、API契約、mykeibadb直近1年の再同期を完了した。
  画面や公開Race DTOへ実数値は表示しない。
  区間ラップはダート1400-1799mの完全取得率が66.7%のため、v4初期候補では必須にしない。
- [x] ~~**P1 RPCI v4候補のS3/L3履歴特徴量と独立比較**~~ → 完了（2026-07-24）。
  対象レースより前の履歴だけを使う学習SQLとオンライン特徴量を同じ条件で実装する。
  独立期間で本番v1と比較し、芝候補は不採用、ダート候補だけを採用した。
- [x] ~~**P1 ダートRPCI v4の期間外監視と再学習条件の確定**~~ → 完了（2026-07-24）。
  採用時の独立評価は2026-06-01以降197件。今後の確定レースで展開一致率・ハイ再現率・
  バイアスを継続監視する。2026-08-02の後続ADRにより、100件かつハイ・平均・スロー各20件から
...
- [x] ~~**P2 実JV-Dataの人気/賞金予約オフセット検証**~~ → **2026-08-03完了**。
  実COM SE 50件とmykeibadbを照合し、合成予約位置は人気1/50（偶然一致）・本賞金0/50だった。
  実位置は確定できなかったため、予約拡張はmykeibadb合成時だけ解析し、jvlinkでは常に欠損へ縮退する。
- [ ] **P2 実JV-Link SE全体の再校正**。血統登録番号を追加アンカーとして同一馬対応を検証し、
  確定着順・人気・本賞金の実位置を複数レースで再特定する。現行mykeibadb運用には影響しない。
...
- [x] ~~**P1 DBマイグレーション不足のreadiness検出と復旧案内**~~ → **2026-07-22完了**。
  ...
  2026-07-24に長さ付き文字列列の不足も検出対象へ追加し、migration 006未適用を判別可能にした。
...
- [ ] 修正前のローカルログに記録されたSlack Webhookを再発行し、`.env`を新URLへ更新する。
