...
ルートの `vercel.json` がビルド設定を持つ。**環境変数はここに書かない。**
以前は `"env": {"API_BASE_URL": "@api_base_url"}` と書いていたが、`@` 記法は廃止された
Vercel Secrets を指すもので、**存在しない秘密情報を要求してビルド開始前に弾かれる**
（プロジェクトを作り直しても直らず、デプロイが1件も作られない形で現れる）。
値はダッシュボードの Environment Variables で設定する。

Vercel の環境変数へ次の2つを入れる。
