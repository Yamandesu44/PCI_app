...
class PaiWeights:
    ...
    # 合致ラベル閾値（pai-v5 で（コース×脚質）別へ、pai-v6 で目標を30%→20%へ）。
    #
    # pai-v4 までは全脚質・両コース共通の 55.0 だった。PAI は脚質内の相対量なので、
    # 共通の絶対値と比べると脚質ごとに実効的な厳しさが変わる。実測では芝の自在が
    # 602頭中0頭で**構造的に合致へ到達できず**、一方ダートの差しは48.1%が合致だった。
    #
    # 値は各セルの合致割合を目標へ揃える最小の閾値を実測から解いたもの
    # （`--diagnose-pai` の「脚質ごとの合致割合と推奨閾値」）。
    #
    # **目標は20%。** pai-v5 は30%だったが、それは「芝の現状」というだけで、
    # 良いという検証ではなかった。実機で「展開では絞りにくいレースです」が
    # 頻繁に出ると指摘され、測り直したら**約4分の1のレース**で出ていた
    # （芝21.6% / ダート30.4%）。フォールバックが4回に1回出るならフォールバックではない。
    #
    # 目標を振った実測（500レース・`--diagnose-pai` の「目標割合を振ったとき…」）:
    #
    #   目標   芝: 半数以上   ダート: 半数以上
    #   30%    21.6%         30.4%   ← pai-v5
    #   25%    13.5%         23.5%
    #   20%    10.8%         22.1%   ← 採用。芝はここが底
    #   15%    12.2%(汚染)   20.1%
    #   10%     9.1%(汚染)   17.2%
    #
    # **芝は20%より絞れない。** 15%以下では差し・逃げが PAI 上限 75.0 に当たって
    # 分割不能になり、据え置きになる分かえって悪化する（10.8% → 12.2%）。
    #
    # **ダートは閾値では直らない。** 目標10%でも17.2%までしか下がらない。
    # 差し（757頭・ダートの28%）が分割不能で48.1%のまま残り、これが床を決めている。
    #
    # **ダートの差しと追込は据え置き。** 感応度0の脚質は base_pai が中立の50で固定され
    # PAI = 0.5×50 + 0.5×affinity、affinity は自分の最良レベルを100へ正規化するので
    # **PAI の上限が 75.0**。ダートはほぼ全レースが同じペース区分に入るため多くの馬が
    # そこへ並ぶ（75.0 に48.1%・その上は0頭）。塊ごと入れるか丸ごと落とすかしかない。
    # 根治は affinity の正規化を母集団基準へ変えること（pai-v7 で扱う）。
    #
    # **芝の自在は中立点50を上回る値に保つ。** 解は分布だけを見るので、50を下回っても
    # 気付かない。下回る値を合致にすると「普段どおりより悪い流れ」の馬に「向く」と言う
    # ことになる（`__post_init__` で弾いている）。このセルは実測でも
    # **合致25.0% < 中立26.2%** で、脚質内の効きも +2.0%・±8.7% と全10セル中もっとも
    # 弱い。**そもそも PAI が芝の自在を判別できていない。** 閾値では直らない。
    #
    # **2026-06-01以降の500レースから取った当てはめ値。期間外で再確認すること。**
    matched_threshold_turf_escape: float = 70.5
    matched_threshold_turf_front: float = 72.5
    matched_threshold_turf_flexible: float = 52.0
    matched_threshold_turf_stalker: float = 73.5
    matched_threshold_turf_closer: float = 59.0
    matched_threshold_dirt_escape: float = 74.0
    matched_threshold_dirt_front: float = 74.0
    matched_threshold_dirt_flexible: float = 51.0
