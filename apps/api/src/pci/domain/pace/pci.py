...
PCI の方向性（不変・ゴールデンテストで固定）:
    PCI > 50: スロー（前半が相対的に遅い / 後半速い）
    PCI = 50: イーブンペース
    PCI < 50: ハイ（前傾ラップ）

どの脚質が有利かは、上の定義から自明ではない。実測（2026-08-04・21万頭）では
**スローで逃げ・先行が有利**、ハイで前が苦しくなる。

    芝 スロー時   逃げ 1.21x / 先行 1.12x / 差し 1.04x / 追込 0.98x
    ダート スロー時 逃げ 1.17x / 先行 1.12x / 差し 0.94x / 追込 0.96x
    （各脚質自身の平均に対する比。`--pace-style-matrix` で再現できる）

前半が緩めば前の馬は脚を溜められるため残りやすい、という素直な関係。
以前ここには「スロー → 差し・追込有利」と逆の記述があったが、データに反していた
