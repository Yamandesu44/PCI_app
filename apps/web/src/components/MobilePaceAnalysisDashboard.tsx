  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-3 pb-8 pt-3 md:hidden">
      ...
      <div className="sticky top-0 z-20 -mx-3 mt-3 border-y border-slate-200 bg-white/95 px-3 py-2 shadow-sm backdrop-blur">
        <div
          role="tablist"
          aria-label="確定後分析の表示切り替え"
          className="grid h-12 grid-cols-3 overflow-hidden rounded-md border border-slate-200 bg-slate-50"
        >
          {TABS.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                key={tab.id}
                id={`mobile-analysis-tab-${tab.id}`}
                type="button"
                role="tab"
                aria-selected={isActive}
                aria-controls={`mobile-analysis-panel-${tab.id}`}
